...
void CPrivManager::AddCharPriv(DWORD pid, BYTE type, int value)
{
	...
	PrivCharDataMap::const_iterator it;
...
//
// @version 05/06/07	Bang2ni - АМ№М єёіКЅє°Ў Аыїл µИ ±жµеїЎ єёіКЅє јіБ¤
//
void CPrivManager::AddGuildPriv(DWORD guild_id, BYTE type, int value, time_t duration_sec)
{
	...
	PrivGuildDataMap::iterator it = m_aPrivGuild[type].find(guild_id);
	...
	// ADD_GUILD_PRIV_TIME
	// АМ№М єёіКЅє°Ў јіБ¤µЗ АЦґЩёй map АЗ value ё¦ °»ЅЕЗШ БШґЩ.
	// АМАь value АЗ ЖчАОЕНґВ priority queue їЎј­ »иБ¦µЙ ¶§ ЗШБ¦µИґЩ.
...
void CPrivManager::AddEmpirePriv(BYTE empire, BYTE type, int value, time_t duration_sec)
{
	...
	// АМАь јіБ¤°Є №«ИїИ­
	// priority_queueїЎ µйѕоАЦґВ pointer == m_aaPrivEmpire[type][empire]
	{
...
void CPrivManager::SendPrivOnSetup(CPeer* peer)
{
	for (int i = 1; i < MAX_PRIV_NUM; ++i)
	{
		...
		for (PrivGuildDataMap::const_iterator it = m_aPrivGuild[i].begin(); it != m_aPrivGuild[i].end(); ++it)
		{
			// ADD_GUILD_PRIV_TIME
			FSendChangeGuildPriv(it->first, i, it->second->value, it->second->end_time_sec)(peer);
			// END_OF_ADD_GUILD_PRIV_TIME
		}
		for (PrivCharDataMap::const_iterator it = m_aPrivChar[i].begin(); it != m_aPrivChar[i].end(); ++it)
		{
