...
{
    unsigned int	cutAt;                            //АЪёЈґВА§ДЎ
    int				index     = 0;                    //№®АЪї­АОµ¦Ѕє
    string* strResult = new string[30];		  //°б°ъreturn ЗТєЇјц

    //strTokА»ГЈА»¶§±оБц№Эє№
    while ((cutAt = strOrigin.find_first_of(strTok)) != strOrigin.npos)
    {
       if (cutAt > 0)  //АЪёЈґВА§ДЎ°Ў0єёґЩЕ©ёй(јє°шЅГ)
       {
            strResult[index++] = strOrigin.substr(0, cutAt);  //°б°ъ№иї­їЎГЯ°Ў
       }
       strOrigin = strOrigin.substr(cutAt+1);  //їшє»АєАЪёҐєОєРБ¦їЬЗСіЄёУБц
    }

    if(strOrigin.length() > 0)  //їшє»АМѕЖБчіІѕТАёёй
    {
        strResult[index++] = strOrigin.substr(0, cutAt);  //іЄёУБцё¦°б°ъ№иї­їЎГЯ°Ў
    }
...
int get_Item_SubType_Value(unsigned int type_value, string inputString) 
{
	...
	//ѕЖАМЕЫ ЕёАФАЗ ј­єкЕёАФ ѕо·№АМ°Ў БёАзЗПґВБц ѕЛѕЖєё°н, ѕшАёёй 0 ё®ЕП
...
int get_Item_AntiFlag_Value(string inputString) 
{
	...
	int retValue = 0;
	string* arInputString = StringSplit(inputString, "|");				//ЗБ·ОЕд Б¤єё і»їлА» ґЬѕоє°·О ВЙ°µ №иї­.
	for(unsigned int i =0;i<sizeof(arAntiFlag)/sizeof(arAntiFlag[0]);i++) {
		string tempString = arAntiFlag[i];
		for (unsigned int j=0; j<30 ; j++)		//ГЦґл 30°і ґЬѕо±оБц. (ЗПµеДЪµщ)
		{
			string tempString2 = arInputString[j];
			if (tempString2.compare(tempString)==0) {				//АПДЎЗПґВБц И®АО.
...
//ёч ЕЧАМєнА» јВЖГЗШБШґЩ.
bool Set_Proto_Mob_Table(TMobTable *mobTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
	int col = 0;
	str_to_number(mobTable->dwVnum, csvTable.AsStringByIndex(col++));
	enhance_strlcpymt(mobTable->szName, csvTable.AsStringByIndex(col++), sizeof(mobTable->szName));

	//3. БцїЄє° АМё§ іЦѕоБЦ±в.
	map<int,const char*>::iterator it;
	it = nameMap.find(mobTable->dwVnum);
	if (it != nameMap.end()) {
		const char * localeName = it->second;
		enhance_strlcpymt(mobTable->szLocaleName, localeName, sizeof (mobTable->szLocaleName));
	} else {
		enhance_strlcpymt(mobTable->szLocaleName, mobTable->szName, sizeof (mobTable->szLocaleName));
	}
	...
	enhance_strlcpymt(mobTable->szFolder, csvTable.AsStringByIndex(col++), sizeof(mobTable->szFolder));
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
	...
	// vnum №Ч vnum range АР±в.
	{
		std::string s(csvTable.AsStringByIndex(0));
		unsigned int pos = s.find("~");
		// vnum ЗКµеїЎ '~'°Ў ѕшґЩёй ЖРЅє
	...
	enhance_strlcpymt(itemTable->szName, csvTable.AsStringByIndex(1), sizeof(itemTable->szName));
	//БцїЄє° АМё§ іЦѕоБЦ±в.
	map<int,const char*>::iterator it;
	it = nameMap.find(itemTable->dwVnum);
	if (it != nameMap.end()) {
		const char * localeName = it->second;
		enhance_strlcpymt(itemTable->szLocaleName, localeName, sizeof (itemTable->szLocaleName));
	} else {
		enhance_strlcpymt(itemTable->szLocaleName, itemTable->szName, sizeof (itemTable->szLocaleName));
	}
