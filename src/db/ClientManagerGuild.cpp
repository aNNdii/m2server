...
void CClientManager::GuildWar(CPeer* peer, TPacketGuildWar* p)
{
	switch (p->bWar)
	{
		...
		case GUILD_WAR_RESERVE:	// ±жµеАь ї№ѕа
		...
		case GUILD_WAR_ON_WAR:		// ±жµеАьА» ЅГАЫ ЅГЕІґЩ. (ЗКµеАьАє №Щ·О ЅГАЫ µК)
		...
		case GUILD_WAR_OVER:		// ±жµеАь Б¤»у Бѕ·б
		...
		case GUILD_WAR_END:		// ±жµеАь єсБ¤»у Бѕ·б
			...
			return; // NOTE: RecvWarEndїЎј­ ЖРЕ¶А» єёі»№З·О µы·О єк·ОµеДіЅєЖГ ЗПБц ѕКґВґЩ.
