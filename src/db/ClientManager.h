...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	// MYSHOP_PRICE_LIST
	/// ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® їдГ» Б¤єё
	/**
	 * first: Peer handle
	 * second: їдГ»ЗС ЗГ·№АМѕоАЗ ID
	 */
	...
	DWORD	GetUserCount();	// БўјУµИ »зїлАЪ јцё¦ ё®ЕП ЗСґЩ.
	...
	// MYSHOP_PRICE_LIST
	/// °Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГё¦ °ЎБ®їВґЩ.
	/**
	 * @param [in]	dwID °Ў°ЭБ¤єё ё®ЅєЖ®АЗ јТАЇАЪ.(ЗГ·№АМѕо ID)
	 * @return	°Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГАЗ ЖчАОЕН
	 */
	CItemPriceListTableCache*	GetItemPriceListCache(DWORD dwID);

	/// °Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГё¦ іЦґВґЩ.
	/**
	 * @param [in]	pItemPriceList ДіЅГїЎ іЦА» ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ®
	 *
	 * ДіЅГ°Ў АМ№М АЦАёёй Update °Ў ѕЖґС replace ЗСґЩ.
	 */
	void			PutItemPriceListCache(const TItemPriceListTable* pItemPriceList);


	/// Flush ЅГ°ЈАМ ёё·бµИ ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГё¦ Flush ЗШБЦ°н ДіЅГїЎј­ »иБ¦ЗСґЩ.
	...
	char*			GetCommand(char* str);					//µ¶АПј±№°±вґЙїЎј­ ён·Йѕо ѕтґВ ЗФјц
	void			ItemAward(CPeer * peer, char* login);	//µ¶АП ј±№° ±вґЙ
	...
	// mob_proto.txt, item_proto.txtїЎј­ АРАє mob_proto, item_protoё¦ real dbїЎ №Эїµ.
	//	item_proto, mob_protoё¦ dbїЎ №ЭїµЗПБц ѕКѕЖµµ, °ФАУ µ№ѕЖ°ЎґВµҐґВ №®Б¦°Ў ѕшБцёё,
	//	їоїµЕш µоїЎј­ dbАЗ item_proto, mob_protoё¦ АРѕо ѕІ±в ¶§№®їЎ №®Б¦°Ў №Я»эЗСґЩ.
	...
	// MYSHOP_PRICE_LIST
	/// °Ў°ЭБ¤єё ·Оµе Дхё®їЎ ґлЗС Result Гіё®
	/**
	 * @param	peer °Ў°ЭБ¤єёё¦ їдГ»ЗС Game server АЗ peer °ґГј ЖчАОЕН
	 * @param	pMsg Дхё®АЗ Result ·О №ЮАє °ґГјАЗ ЖчАОЕН
	 *
	 * ·ОµеµИ °Ў°ЭБ¤єё ё®ЅєЖ®ё¦ ДіЅГїЎ АъАеЗП°н peer їЎ°Ф ё®ЅєЖ®ё¦ єёі»БШґЩ.
	 */
	void		RESULT_PRICELIST_LOAD(CPeer* peer, SQLMsg* pMsg);

	/// °Ў°ЭБ¤єё ѕчµҐАМЖ®ё¦ А§ЗС ·Оµе Дхё®їЎ ґлЗС Result Гіё®
	/**
	 * @param	pMsg Дхё®АЗ Result ·О №ЮАє °ґГјАЗ ЖчАОЕН
	 *
	 * ·ОµеµИ Б¤єё·О °Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГё¦ ёёµй°н ѕчµҐАМЖ® №ЮАє °Ў°ЭБ¤єё·О ѕчµҐАМЖ® ЗСґЩ.
	 */
	...
	/// ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® ѕчµҐАМЖ® ЖРЕ¶(HEADER_GD_MYSHOP_PRICELIST_UPDATE) Гіё®ЗФјц
	/**
	 * @param [in]	pPacket ЖРЕ¶ µҐАМЕНАЗ ЖчАОЕН
	 */
	void		MyshopPricelistUpdate(const TPacketMyshopPricelistHeader* pPacket);

	/// ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® їдГ» ЖРЕ¶(HEADER_GD_MYSHOP_PRICELIST_REQ) Гіё®ЗФјц
	/**
	 * @param	peer ЖРЕ¶А» єёіЅ Game server АЗ peer °ґГјАЗ ЖчАОЕН
	 * @param [in]	dwHandle °Ў°ЭБ¤єёё¦ їдГ»ЗС peer АЗ ЗЪµй
	 * @param [in]	dwPlayerID °Ў°ЭБ¤єё ё®ЅєЖ®ё¦ їдГ»ЗС ЗГ·№АМѕоАЗ ID
	 */
    ...
    private:
	int					m_looping;
	socket_t				m_fdAccept;	// БўјУ №ЮґВ јТДП
	...
	// Login LoginData pair (ЅЗБ¦ ·О±ЧАО µЗѕоАЦґВ °иБ¤)
	...
	TPlayerTableCacheMap			m_map_playerCache;  // ЗГ·№АМѕо id°Ў key

	TItemCacheMap				m_map_itemCache;  // ѕЖАМЕЫ id°Ў key
	TItemCacheSetPtrMap			m_map_pkItemCacheSetPtr;  // ЗГ·№АМѕо id°Ў key, АМ ЗГ·№АМѕо°Ў ѕо¶І ѕЖАМЕЫ ДіЅ¬ё¦ °ЎБц°н АЦіЄ?

	// MYSHOP_PRICE_LIST
	/// ЗГ·№АМѕоє° ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® map. key: ЗГ·№АМѕо ID, value: °Ў°ЭБ¤єё ё®ЅєЖ® ДіЅГ
	TItemPriceListCacheMap m_mapItemPriceListCache;  ///< ЗГ·№АМѕоє° ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ®
	// END_OF_MYSHOP_PRICE_LIST
