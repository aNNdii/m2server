...
bool CMonarch::AddCandidacy(DWORD pid, const char * name)
{
	...
	info.pid = pid;
	enhance_strlcpymt(info.name, name, sizeof(info.name));
...
bool CMonarch::LoadMonarch()
{
    ...
    for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
    {
		...
		enhance_strlcpymt(p->name[Empire], row[idx++], sizeof(p->name[Empire]));

        str_to_number(p->money[Empire], row[idx++]);
		enhance_strlcpymt(p->date[Empire], row[idx++], sizeof(p->date[Empire]));
