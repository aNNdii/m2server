...
void CGuildManager::ParseResult(SQLResult * pRes)
{
	...
	while ((row = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		enhance_strlcpymt(r_info.szName, row[1], sizeof(r_info.szName));
...
{
	ProcessReserveWar(); // ї№ѕа АьАп Гіё®
...
//
// ±жµеАь єсБ¤»у Бѕ·б №Ч ЗКµеАь Бѕ·б
//
void CGuildManager::WarEnd(DWORD GID1, DWORD GID2, bool bForceDraw)
{
	...
	if (!bForceDraw) // °­Б¦ №«ЅВєО°Ў ѕЖґТ °жїмїЎґВ БЎјцё¦ ГјЕ©ЗСґЩ.
	{
	...
	else // °­Б¦ №«ЅВєОАП °жїмїЎґВ №«Б¶°З №«ЅВєО
		bDraw = true;
	...
	// DB ј­№цїЎј­ АЪГјАыАё·О іЎіѕ ¶§µµ АЦ±в ¶§№®їЎ µы·О ЖРЕ¶А» єёі»БаѕЯ ЗСґЩ.
...
void CGuildManager::RecvWarEnd(DWORD GID1, DWORD GID2)
{
	...
	WarEnd(GID1, GID2, true); // №«Б¶°З єсБ¤»у Бѕ·б ЅГДСѕЯ ЗСґЩ.
}
...
//
// ї№ѕа ±жµеАь(°ьАьАЪ°Ў №иЖГЗТ јц АЦґЩ)
//
...
void CGuildManager::BootReserveWar()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		while ((row = mysql_fetch_row(pmsg->Get()->pSQLResult)))
		{
			...
			// i == 0 АМёй ±жµеАь µµБЯ DB°Ў ЖЁ±д °НАМ№З·О №«ЅВєО Гіё®ЗСґЩ.
			// ¶ЗґВ, 5єР АМЗП іІАє ї№ѕа ±жµеАьµµ №«ЅВєО Гіё®ЗСґЩ. (°ўАЪАЗ №иЖГѕЧА» µ№·ББШґЩ)
...
void CGuildManager::ProcessReserveWar()
{
	...
	while (it != m_map_kWarReserve.end())
	{
		...
		if (!r.bStarted && r.dwTime - 1800 <= dwCurTime) // 30єР АьєОЕН ѕЛё°ґЩ.
		{
			...
			else
			{
				if (iMin != pk->GetLastNoticeMin())
				{
					...
					if (!g_stLocale.compare("euckr"))
						CClientManager::instance().SendNotice("%s ±жµеїН %s ±жµеАЗ АьАпАМ ѕа %dєР ИД ЅГАЫ µЛґПґЩ!", r_1.szName, r_2.szName, iMin);
					else if (!g_stLocale.compare("gb2312"))
						CClientManager::instance().SendNotice("%s °п»бєН %s °п»бµД°п»бХЅХщЅ«ФЪ %d·ЦЦУєуїЄКј!", r_1.szName, r_2.szName, iMin);
				}
			}
		}
	}
...
void CGuildWarReserve::Initialize()
{
	...
	if (msgbet->Get()->uiNumRows)
	{
		...
		{
			dwGuild = dwGold = 0;
			enhance_strlcpymt(szLogin, row[0], sizeof(szLogin));
...
{
	if (m_data.bStarted) // АМ№М ЅГАЫµИ °НАє єёі»Бц ѕКґВґЩ.
		return;
	...
	while (it != mapBet.end())
	{
		enhance_strlcpymt(pckBet.szLogin, it->first.c_str(), sizeof(pckBet.szLogin));
...
bool CGuildWarReserve::Bet(const char * pszLogin, DWORD dwGold, DWORD dwGuild)
{
	...
	TPacketGDGuildWarBet pckBet;
	pckBet.dwWarID = m_data.dwID;
	enhance_strlcpymt(pckBet.szLogin, pszLogin, sizeof(pckBet.szLogin));
...
//
// №«ЅВєО Гіё®: ґлєОєР ЅВєО°Ў іЄѕЯ Б¤»уАМБцёё, ј­№ц №®Б¦ µо ЖЇБ¤ »уИІАП °жїмїЎґВ
//              №«ЅВєО Гіё®°Ў АЦѕоѕЯ ЗСґЩ.
//
