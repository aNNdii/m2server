...
bool CClientManager::Initialize()
{
	...
	if (!CConfig::instance().GetValue("BIND_IP", szBindIP, 128))
		enhance_strlcpymt(szBindIP, "0", sizeof(szBindIP));
	...
	// database character-setА» °­Б¦·О ёВГг
...
void CClientManager::MainLoop()
{
	...
	// MYSHOP_PRICE_LIST
	//
	// °іАО»уБЎ ѕЖАМЕЫ °Ў°Э ё®ЅєЖ® Flush
	//
...
{
	const BYTE bPacketVersion = 6; // BOOT ЖРЕ¶АМ №ЩІр¶§ё¶ґЩ №шИЈё¦ їГё®µµ·П ЗСґЩ.
...
void CClientManager::QUERY_SAFEBOX_LOAD(CPeer * pkPeer, DWORD dwHandle, TSafeboxLoadPacket * packet, bool bMall)
{
	...
	enhance_strlcpymt(pi->safebox_password, packet->szPassword, sizeof(pi->safebox_password));
	pi->account_id = packet->dwID;
	pi->account_index = 0;
	pi->ip[0] = bMall ? 1 : 0;
	enhance_strlcpymt(pi->login, packet->szLogin, sizeof(pi->login));
...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	// ї©±вїЎј­ »зїлЗПґВ account_indexґВ Дхё® јшј­ё¦ ё»ЗСґЩ.
	// Г№№шВ° ЖРЅєїцµе ѕЛѕЖі»±в А§ЗШ ЗПґВ Дхё®°Ў 0
	// µО№шВ° ЅЗБ¦ µҐАМЕНё¦ ѕтѕоіхґВ Дхё®°Ў 1

	if (pi->account_index == 0)
	{
		char szSafeboxPassword[SAFEBOX_PASSWORD_MAX_LEN + 1];
		enhance_strlcpymt(szSafeboxPassword, pi->safebox_password, sizeof(szSafeboxPassword));
	...
	else
	{
		...
		// Дхё®їЎ їЎ·Ї°Ў АЦѕъАё№З·О ААґдЗТ °жїм Гў°н°Ў єсѕоАЦґВ °Н Гі·і
		// єёАМ±в ¶§№®їЎ Гў°н°Ў ѕЖѕк ѕИї­ё®ґВ°Ф іЄАЅ
				...
				while (it != pSet->end())
				{
					...
					else
					{
						switch (dwItemVnum)
						{
							...
							// №«ЅГ№«ЅГЗПБцёё АМАьїЎ ЗПґш °Й °нДЎ±вґВ №«ј·°н...
							// ±Ч·Ўј­ ±ЧіЙ ЗПµе ДЪµщ. ј±№° »уАЪїл АЪµї№°ѕа ѕЖАМЕЫµй.
...
{
	ClientHandleInfo * pi = new ClientHandleInfo(dwHandle);
	pi->account_index = p->bSize;	// account_indexё¦ »зАМБо·О АУЅГ·О »зїл
...
{
	ClientHandleInfo * pi = new ClientHandleInfo(dwHandle);
	enhance_strlcpymt(pi->safebox_password, p->szNewPassword, sizeof(pi->safebox_password));
	enhance_strlcpymt(pi->login, p->szOldPassword, sizeof(pi->login));
...
void CClientManager::RESULT_PRICELIST_LOAD(CPeer* peer, SQLMsg* pMsg)
{
	...
	//
	// DB їЎј­ ·ОµеЗС Б¤єёё¦ Cache їЎ АъАе
	//
	...
	//
	// ·ОµеЗС µҐАМЕНё¦ Game server їЎ АьјЫ
	//
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	//
	// ѕо¶І ёКАМ ѕо¶І ј­№цїЎ АЦґВБц єёі»±в
	//
	TMapLocation kMapLocations;

	enhance_strlcpymt(kMapLocations.szHost, peer->GetPublicIP(), sizeof(kMapLocations.szHost));
	...
	if (peer->GetChannel() == 1)
	{
		for (TPeerList::const_iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
		{
			...
			if (tmp->GetChannel() == GUILD_WARP_WAR_CHANNEL || tmp->GetChannel() == peer->GetChannel())
			{
				...
				enhance_strlcpymt(kMapLocation2.szHost, tmp->GetPublicIP(), sizeof(kMapLocation2.szHost));
	...
	//
	// јВѕч : БўјУЗС ЗЗѕоїЎ ґЩёҐ ЗЗѕоµйАМ БўјУЗП°Ф ёёµзґЩ. (P2P ДБіШјЗ »эјє)
	// 
	...
	enhance_strlcpymt(p2pSetupPacket.szHost, peer->GetPublicIP(), sizeof(p2pSetupPacket.szHost));
	...
	for (TPeerList::const_iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
	{
		...
		// Г¤іОАМ 0АМ¶уёй ѕЖБч SETUP ЖРЕ¶АМ їАБц ѕКАє ЗЗѕо ¶ЗґВ auth¶у°н °ЈБЦЗТ јц АЦАЅ
		if (0 == tmp->GetChannel())
			continue;
	...
	for (DWORD c = 0; c < p->dwLoginCount; ++c, ++pck)
	{
		...
		enhance_strlcpymt(r.social_id, pck->szSocialID, sizeof(r.social_id));
		enhance_strlcpymt(r.passwd, "TEMP", sizeof(r.passwd));
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	// Гў°нёй ДіЅ¬ЗПБц ѕК°н, ДіЅ¬їЎ АЦґш °Нµµ »©№ц·БѕЯ ЗСґЩ.
	// auctionАє АМ ·зЖ®ё¦ ЕёБц ѕКѕЖѕЯ ЗСґЩ. EnrollInAuctionА» ЕёѕЯЗСґЩ.
...
{       
	...
	else
	{
		...
		{
			// АМ№М АМ ѕЖАМЕЫА» °ЎБц°н АЦѕъґш АЇАъ·О єОЕН ѕЖАМЕЫА» »иБ¦ЗСґЩ.
			TItemCacheSetPtrMap::iterator it = m_map_pkItemCacheSetPtr.find(c->Get()->owner);
	...
	{
		// ЗцАз јТАЇАЪ°Ў ѕшАё№З·О №Щ·О АъАеЗШѕЯ ґЩАЅ БўјУАМ їГ ¶§ SQLїЎ Дхё®ЗПї©
		// №ЮА» јц АЦАё№З·О №Щ·О АъАеЗСґЩ.
		if (g_log)
...
void CClientManager::QUERY_ITEM_DESTROY(CPeer * pkPeer, const char * c_pData)
{
	...
	if (!DeleteItemCache(dwID))
	{
		...
		if (dwPID == 0) // ѕЖ№«µµ °ЎБш »з¶чАМ ѕшѕъґЩёй, єсµї±в Дхё®
...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	else
	{
		...
		enhance_strlcpymt(r.social_id, p->szSocialID, sizeof(r.social_id));
		enhance_strlcpymt(r.passwd, "TEMP", sizeof(r.passwd));
...
//
// ДіЅГїЎ °Ў°ЭБ¤єё°Ў АЦАёёй ДіЅГё¦ ѕчµҐАМЖ® ЗП°н ДіЅГїЎ °Ў°ЭБ¤єё°Ў ѕшґЩёй
// їмј± ±вБёАЗ µҐАМЕНё¦ ·ОµеЗС µЪїЎ ±вБёАЗ Б¤єё·О ДіЅГё¦ ёёµй°н »х·О №ЮАє °Ў°ЭБ¤єёё¦ ѕчµҐАМЖ® ЗСґЩ.
//
...
// MYSHOP_PRICE_LIST
// ДіЅГµИ °Ў°ЭБ¤єё°Ў АЦАёёй ДіЅГё¦ АРѕо №Щ·О АьјЫЗП°н ДіЅГїЎ Б¤єё°Ў ѕшАёёй DB їЎ Дхё®ё¦ ЗСґЩ.
//
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_RMCANDIDACY:		//< ИДєё Б¦°Е (їоїµАЪ)
				RMCandidacy(peer, dwHandle, data);
				break;

			case HEADER_GD_SETMONARCH:		///<±єБЦјіБ¤ (їоїµАЪ)
				SetMonarch(peer, dwHandle, data);
				break;

			case HEADER_GD_RMMONARCH:		///<±єБЦ»иБ¦
...
// DB ёЕґПАъ·О єОЕН №ЮАє °б°ъё¦ Гіё®ЗСґЩ.
//
// @version	05/06/10 Bang2ni - °Ў°ЭБ¤єё °ь·Г Дхё®(QID_ITEMPRICE_XXX) ГЯ°Ў
...
{   
	...
	int         avg = g_dwUsageAvg / 3600; // 60 ГК * 60 єР
...
int CClientManager::Process()
{
	...
	while (pulses--)
	{
		...
		*/
		...
		if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * 60)))	// 60ГКїЎ ЗС№ш
		{
			// АЇґПЕ© ѕЖАМЕЫА» А§ЗС ЅГ°ЈА» єёіЅґЩ.
	...
	for (idx = 0; idx < num_events; ++idx) // АОЗІ
	{
...
bool CClientManager::InitializeLocalization() 
{
	...
	for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
	{
		...
		enhance_strlcpymt(locale.szValue, row[col++], sizeof(locale.szValue));
		enhance_strlcpymt(locale.szKey, row[col++], sizeof(locale.szKey));
...
{
	//szIP == NULL АП°жїм  ёрµзј­№цїЎ їоїµАЪ ±ЗЗСА» °®ґВґЩ.
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery),
	...
	while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		enhance_strlcpymt(Info.m_szName, row[idx++], sizeof(Info.m_szName));
		enhance_strlcpymt(Info.m_szContactIP, row[idx++], sizeof(Info.m_szContactIP));
		enhance_strlcpymt(Info.m_szServerIP, row[idx++], sizeof(Info.m_szServerIP));
...
void CClientManager::RMCandidacy(CPeer * peer, DWORD dwHandle, const char * data)
{
	...
	enhance_strlcpymt(szName, data, sizeof(szName));
...
void CClientManager::ChangeMonarchLord(CPeer * peer, DWORD dwHandle, TPacketChangeMonarchLord* info)
{
	...
	if (pMsg->Get()->uiNumRows != 0)
	{
		...
		enhance_strlcpymt(ack.szName, row[0], sizeof(ack.szName));
		enhance_strlcpymt(ack.szDate, row[1], sizeof(ack.szDate));
...
void CClientManager::EnrollInAuction (CPeer * peer, DWORD owner_id, AuctionEnrollProductInfo* data)
{
	...
	{
		// ѕЖАМЕЫ ДЙЅГё¦ AuctionїЎ µо·П ЗЯАёґП ClientManagerїЎј­ґВ »«ґЩ.
...
// ReBidґВ АМАь АФВы±ЭѕЧїЎ ґхЗШј­ АФВыЗСґЩ.
// ReBidїЎј± data->bid_price°Ў АМАь АФВы°ЎїЎ ґхЗШБ®ј­
// ±Ч ±ЭѕЧАё·О rebidЗПґВ °Н.
// АМ·ё°Ф ЗС АМАЇґВ rebidїЎ ЅЗЖР ЗЯА» ¶§,
// АЇАъАЗ ИЈБЦёУґПїЎј­ »« µ·А» µ№·ББЦ±в ЖнЗП°Ф ЗП±в А§ЗФАМґЩ.
...
void CClientManager::AuctionReBid (CPeer * peer, DWORD bidder_id, AuctionBidInfo* data)
{
	...
	// АМ°З FAILАМ ¶°ј­ґВ ѕИµЕ.
	// FAILАМ ¶г јц°Ў ѕшґВ°Ф, MyBidїЎ АЦґВ bidder_idїЎ ґлЗС ДБЕЩГчґВ bidder_idёёАМ Бў±Щ ЗТ јц АЦ°Еµз?
	// ±Ч·Ї№З·О ґЩёҐ °НАМ ґЩ Б¤»уАыАё·О АЫµїЗСґЩ°н °ЎБ¤ ЗСґЩёй
	// ЗС °ФАУ ј­№ц і»їЎј­ bidder_id·О MyBidё¦ јцБ¤ЗСґЩ ЗТ Бц¶уµµ, ±Ч°З µї±вИ­ №®Б¦°Ў ѕшѕо.
	// ґЩёҐ °ФАУ ј­№цїЎ ¶И°°Ає bidder_idё¦ °ЎБш ірАМ АЦА» јц°Ў ѕшАёґП±о.
	// ±Ч·Ї№З·О ±Ч °ФАУ ј­№цїЎј­ BidCancel ён·ЙА» dbїЎ іЇ·ИґЩґВ °НАє,
	// АМ№М ±Ч єОєРїЎ ґлЗШј­ґВ °Л»з°Ў їПє®ЗПґЩґВ °НАМѕЯ.
	// ±Ч·Ўµµ И¤ЅГіЄ ЅНѕој­, µр№ц±лА» А§ЗШ fail ДЪµеё¦ іІ°ЬµРґЩ.
	if (result <= AUCTION_FAIL)
	{
		TPacketDGResultAuction enroll_result;
