...
bool CreateItemTableFromRes(MYSQL_RES * res, std::vector<TPlayerItem> * pVec, DWORD dwPID)
{
	...
	if ((rows = mysql_num_rows(res)) <= 0)	// µҐАМЕН ѕшАЅ
	{
		pVec->clear();
		return true;
	}
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	//
	// ЗС °иБ¤їЎ јУЗС ёрµз ДіёЇЕНµй ДіЅ¬Гіё®
	//
	...
	//----------------------------------------------------------------
	// 1. АЇАъБ¤єё°Ў DBCache їЎ БёАз : DBCacheїЎј­ 
	// 2. АЇАъБ¤єё°Ў DBCache їЎ ѕшАЅ : DBїЎј­ 
	// ---------------------------------------------------------------
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		//--------------------------------------------
		// ѕЖАМЕЫ & AFFECT & QUEST ·Оµщ : 
		//--------------------------------------------
		// 1) ѕЖАМЕЫАМ DBCache їЎ БёАз : DBCache їЎј­ °ЎБ®їИ
		// 2) ѕЖАМЕЫАМ DBCache їЎ ѕшАЅ : DB їЎј­ °ЎБ®їИ 
		...
		if (pSet)
		{
			...
			while (it != pSet->end())
			{
				...
				if (p->vnum) // vnumАМ ѕшАёёй »иБ¦µИ ѕЖАМЕЫАМґЩ.
	...
	else
	{
		...
		//--------------------------------------------------------------
		// ДіёЇЕН Б¤єё ѕтѕоїА±в : №«Б¶°З DBїЎј­ 
		//--------------------------------------------------------------
		...
		//µ¶АП ј±№° ±вґЙїЎј­ item_awardЕЧАМєнїЎј­ login Б¤єёё¦ ѕт±вА§ЗШ account idµµ іС°ЬБШґЩ
		//--------------------------------------------------------------
		// AFFECT °ЎБ®їА±в 
		//--------------------------------------------------------------
		snprintf(queryStr, sizeof(queryStr),
...
void CClientManager::ItemAward(CPeer * peer,char* login)
{
	...
	std::set<TItemAward *>::const_iterator it = pSet->begin();	//taken_timeАМ NULLАО°Нµй АРѕоїИ	
	while(it != pSet->end() )
	{				
		TItemAward * pItemAward = *(it++);		
		char* whyStr = pItemAward->szWhy;	//why ДЭ·л АР±в
		...
		strcpy(command,GetCommand(cmdStr));	// command ѕт±в		
		if( !(strcmp(command,"GIFT") ))	// command °Ў GIFTАМёй
		{
			TPacketItemAwardInfromer giftData;
			strcpy(giftData.login, pItemAward->szLogin);	//·О±ЧАО ѕЖАМµр є№»з
			strcpy(giftData.command, command);					//ён·Йѕо є№»з
			giftData.vnum = pItemAward->dwVnum;				//ѕЖАМЕЫ vnumµµ є№»з
...
{
	if (mysql_num_rows(res) == 0)	// µҐАМЕН ѕшАЅ
		return false;
	...
	// reset sub_skill_point
	{
		...
		if (pkTab->level > 9)
		{
			...
			int skill_point = 
				MIN(20, pkTab->skills[121].bLevel) +	// SKILL_LEADERSHIP			ЕлјЦ·В
				MIN(20, pkTab->skills[124].bLevel) +	// SKILL_MINING				Г¤±¤
				MIN(10, pkTab->skills[131].bLevel) +	// SKILL_HORSE_SUMMON		ё»јТИЇ
				MIN(20, pkTab->skills[141].bLevel) +	// SKILL_ADD_HP				HPєё°­
				MIN(20, pkTab->skills[142].bLevel);		// SKILL_RESIST_PENETRATE	°ьЕлАъЗЧ
...
void CClientManager::RESULT_ITEM_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID)
{
	...
	if (dwCount)
	{
		...
		for (DWORD i = 0; i < dwCount; ++i)
			PutItemCache(&s_items[i], true); // ·ОµеЗС °НАє µы·О АъАеЗТ ЗКїд ѕшАё№З·О, АОАЪ bSkipQueryїЎ trueё¦ іЦґВґЩ.
	}
}
...
void CClientManager::RESULT_AFFECT_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle)
{
	...
	if ((iNumRows = mysql_num_rows(pRes)) == 0) // µҐАМЕН ѕшАЅ
		return;
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
	...
	// ЗС °иБ¤їЎ XГК і»·О ДіёЇЕН »эјєА» ЗТ јц ѕшґЩ.
...
//
// @version	05/06/10 Bang2ni - ЗГ·№АМѕо »иБ¦ЅГ °Ў°ЭБ¤єё ё®ЅєЖ® »иБ¦ ГЯ°Ў.
//
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		// ЗГ·№АМѕо ЕЧАМєнА» ДіЅ¬їЎј­ »иБ¦ЗСґЩ.
...
void CClientManager::RESULT_HIGHSCORE_REGISTER(CPeer * pkPeer, SQLMsg * msg)
{
	...
	// TODO: АМ°чїЎј­ ЗПАМЅєДЪѕо°Ў ѕчµҐАМЖ® µЗѕъґВБц ГјЕ©ЗПї© °шБцё¦ »С·БѕЯЗСґЩ.
	delete pi;
}
...
void CClientManager::InsertLogoutPlayer(DWORD pid)
{
	...
	// БёАзЗПБц ѕКА»°жїм ГЯ°Ў
	if (it != m_map_logout.end())
	{
		// БёАзЗТ°жїм ЅГ°Јёё °»ЅЕ
		if (g_log)
