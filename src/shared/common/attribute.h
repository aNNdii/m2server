...
//
// ёК јУјєµйА» Гіё®ЗТ ¶§ »зїл
//
class CAttribute
{
    public:
	CAttribute(DWORD width, DWORD height); // dword ЕёАЩАё·О ёрµО 0А» Г¤їоґЩ.
	CAttribute(DWORD * attr, DWORD width, DWORD height); // attrА» АРѕој­ smartЗП°Ф јУјєА» АРѕоїВґЩ.
	~CAttribute();
	void Alloc();
	int GetDataType();
	void * GetDataPtr();
