...
CAttribute::CAttribute(DWORD width, DWORD height) // dword ЕёАЩАё·О ёрµО 0А» Г¤їоґЩ.
{
    Initialize(width, height);
    Alloc();
}

CAttribute::CAttribute(DWORD * attr, DWORD width, DWORD height) // attrА» АРѕој­ smartЗП°Ф јУјєА» АРѕоїВґЩ.
{
    Initialize(width, height);
    ...
    // јУјєАМ АьєО °°Аёёй ґЬБц defaultAttrёё јіБ¤ЗСґЩ.
    if (i == size)
	defaultAttr = attr[0];
    else
    {
	...
	// ЗПА§ 8єсЖ®ёё »зїлЗТ °жїм D_BYTE
	if (!(allAttr & 0xffffff00))
	    dataType = D_BYTE;
	// ЗПА§ 16єсЖ®ёё »зїлЗТ °жїм D_WORD
	else if (!(allAttr & 0xffff0000))
	    dataType = D_WORD;
	else // ±Ч АМїЬїЎґВ D_DWORD
	    dataType = D_DWORD;

	Alloc();

	if (dataType == D_DWORD) // D_DWORDАП ¶§ґВ їшє» јУјє°ъ °°Аё№З·О ґЬБц є№»з.
	    thecore_memcpy(data, attr, sizeof(DWORD) * width * height);
	else
	{
	    // ѕЖґПёй ДБ№цЖ® ЗШѕЯ ЗСґЩ.
...
void CAttribute::Remove(DWORD x, DWORD y, DWORD attr)
{
    ...
    if (!data) // јУјєА» »иБ¦ЗТ ¶§ ёёѕа µҐАМЕН°Ў ѕшАёёй ±ЧіЙ ё®ЕПЗСґЩ.
	return;
