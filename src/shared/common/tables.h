...
/**
 * @version 05/06/10	Bang2ni - Myshop Pricelist °ь·Г ЖРЕ¶ HEADER_XX_MYSHOP_PRICELIST_XXX ГЯ°Ў
 */
enum
{
	HEADER_GD_LOGIN				= 1,
	...
	HEADER_GD_MYSHOP_PRICELIST_UPDATE	= 108,		///< °Ў°ЭБ¤єё °»ЅЕ їдГ»
	HEADER_GD_MYSHOP_PRICELIST_REQ		= 109,		///< °Ў°ЭБ¤єё ё®ЅєЖ® їдГ»

	HEADER_GD_BLOCK_CHAT				= 110,

	HEADER_GD_HAMMER_OF_TOR			= 114,
	HEADER_GD_RELOAD_ADMIN			= 115,			///<їоїµАЪ Б¤єё їдГ»
	HEADER_GD_BREAK_MARRIAGE		= 116,			///< °бИҐ ЖД±в
	HEADER_GD_ELECT_MONARCH			= 117,			///< ±єБЦ ЕхЗҐ
	HEADER_GD_CANDIDACY				= 118,			///< ±єБЦ µо·П
	HEADER_GD_ADD_MONARCH_MONEY		= 119,			///< ±єБЦ µ· Бх°Ў 
	HEADER_GD_TAKE_MONARCH_MONEY	= 120,			///< ±єБЦ µ· °ЁјТ 
	HEADER_GD_COME_TO_VOTE			= 121,			///< ЗҐ°б
	HEADER_GD_RMCANDIDACY			= 122,			///< ИДєё Б¦°Е (їоїµАЪ)
	HEADER_GD_SETMONARCH			= 123,			///<±єБЦјіБ¤ (їоїµАЪ)
	HEADER_GD_RMMONARCH			= 124,			///<±єБЦ»иБ¦
	HEADER_GD_DEC_MONARCH_MONEY = 125,

	HEADER_GD_CHANGE_MONARCH_LORD = 126,
	HEADER_GD_BLOCK_COUNTRY_IP		= 127,		// ±¤ґлїЄ IP-Block
	HEADER_GD_BLOCK_EXCEPTION		= 128,		// ±¤ґлїЄ IP-Block ї№їЬ
	...
	HEADER_GD_DC					= 133,		// Login Keyё¦ Бцїт
	...
	HEADER_DG_MYSHOP_PRICELIST_RES	= 157,		///< °Ў°ЭБ¤єё ё®ЅєЖ® ААґд
	HEADER_DG_RELOAD_ADMIN = 158, 				///< їоїµАЪ Б¤єё ё®·Оµе 
	HEADER_DG_BREAK_MARRIAGE = 159,				///< °бИҐ ЖД±в
	HEADER_DG_ELECT_MONARCH			= 160,			///< ±єБЦ ЕхЗҐ
	HEADER_DG_CANDIDACY				= 161,			///< ±єБЦ µо·П
	HEADER_DG_ADD_MONARCH_MONEY		= 162,			///< ±єБЦ µ· Бх°Ў 
	HEADER_DG_TAKE_MONARCH_MONEY	= 163,			///< ±єБЦ µ· °ЁјТ 
	HEADER_DG_COME_TO_VOTE			= 164,			///< ЗҐ°б
	HEADER_DG_RMCANDIDACY			= 165,			///< ИДєё Б¦°Е (їоїµАЪ)
	HEADER_DG_SETMONARCH			= 166,			///<±єБЦјіБ¤ (їоїµАЪ)
	HEADER_DG_RMMONARCH			= 167,			///<±єБЦ»иБ¦
	HEADER_DG_DEC_MONARCH_MONEY = 168,

	HEADER_DG_CHANGE_MONARCH_LORD_ACK = 169,
	HEADER_DG_UPDATE_MONARCH_INFO	= 170,
	HEADER_DG_BLOCK_COUNTRY_IP		= 171,		// ±¤ґлїЄ IP-Block
	HEADER_DG_BLOCK_EXCEPTION		= 172,		// ±¤ґлїЄ IP-Block ї№їЬ account
...
typedef struct SPlayerItem
{
	...
	DWORD	vnum;
	long	alSockets[ITEM_SOCKET_MAX_NUM];	// јТДП№шИЈ
...
typedef struct SShopItemTable
{
    ...
    TItemPos	pos;			// PC »уБЎїЎёё АМїл
	DWORD		price;	// PC, shop_table_ex.txt »уБЎїЎёё АМїл
	BYTE		display_pos; // PC, shop_table_ex.txt »уБЎїЎёё АМїл, єёАП А§ДЎ.
} TShopItemTable;
...
typedef struct SItemTable : public SEntityTable
{
	...
	short int	sAddonType; // ±вє» јУјє

	// ѕЖ·Ў limit flagµйАє realtimeїЎ ГјЕ© ЗТ АПАМ ё№°н, ѕЖАМЕЫ VNUMґз °нБ¤µИ °ЄАОµҐ,
	// ЗцАз ±ёБ¶ґл·О ёЕ№ш ѕЖАМЕЫё¶ґЩ ЗКїдЗС °жїмїЎ LIMIT_MAX_NUM±оБц ·зЗБµ№ёйј­ ГјЕ©ЗПґВ єОЗП°Ў Дїј­ №Мё® АъАе ЗШ µТ.
	char		cLimitRealTimeFirstUseIndex;		// ѕЖАМЕЫ limit ЗКµе°Є БЯїЎј­ LIMIT_REAL_TIME_FIRST_USE ЗГ·Ў±ЧАЗ А§ДЎ (ѕшАёёй -1)
	char		cLimitTimerBasedOnWearIndex;		// ѕЖАМЕЫ limit ЗКµе°Є БЯїЎј­ LIMIT_TIMER_BASED_ON_WEAR ЗГ·Ў±ЧАЗ А§ДЎ (ѕшАёёй -1) 
...
typedef struct SPlayerLoadPacket
{
	DWORD	account_id;
	DWORD	player_id;
	BYTE	account_index;	/* account їЎј­АЗ А§ДЎ */
} TPlayerLoadPacket;
...
typedef struct SPacketGDSetup
{
	...
	BYTE	bChannel;	// Г¤іО
	WORD	wListenPort;	// Е¬¶уАМѕрЖ®°Ў БўјУЗПґВ ЖчЖ® №шИЈ
	WORD	wP2PPort;	// ј­№ціўё® ї¬°б ЅГЕ°ґВ P2P ЖчЖ® №шИЈ
	long	alMaps[32];
	DWORD	dwLoginCount;
	BYTE	bAuthServer;
} TPacketGDSetup;
...
/// °іАО»уБЎ °Ў°ЭБ¤єёАЗ Змґх. °ЎєЇ ЖРЕ¶Аё·О АМ µЪїЎ byCount ёёЕ­АЗ TItemPriceInfo °Ў їВґЩ.
typedef struct SPacketMyshopPricelistHeader
{ 
	DWORD	dwOwnerID;	///< °Ў°ЭБ¤єёё¦ °ЎБш ЗГ·№АМѕо ID 
	BYTE	byCount;	///< °Ў°ЭБ¤єё °№јц
} TPacketMyshopPricelistHeader;

/// °іАО»уБЎАЗ ґЬАП ѕЖАМЕЫїЎ ґлЗС °Ў°ЭБ¤єё
typedef struct SItemPriceInfo
{
	DWORD	dwVnum;		///< ѕЖАМЕЫ vnum
	DWORD	dwPrice;	///< °Ў°Э
} TItemPriceInfo;

/// °іАО»уБЎ ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® ЕЧАМєн
typedef struct SItemPriceListTable
{
	DWORD	dwOwnerID;	///< °Ў°ЭБ¤єёё¦ °ЎБш ЗГ·№АМѕо ID
	BYTE	byCount;	///< °Ў°ЭБ¤єё ё®ЅєЖ®АЗ °№јц

	TItemPriceInfo	aPriceInfo[SHOP_PRICELIST_MAX_NUM];	///< °Ў°ЭБ¤єё ё®ЅєЖ®
} TItemPriceListTable;
...
{
	int m_ID;				//°нАЇID
	char m_szAccount[32];	//°иБ¤
	char m_szName[32];		//ДіёЇЕНАМё§
	char m_szContactIP[16];	//Бў±ЩѕЖАМЗЗ
	char m_szServerIP[16];  //ј­№цѕЖАМЗЗ
	int m_Authority;		//±ЗЗС
} tAdminInfo;
//END_ADMIN_MANAGER
...
{
	DWORD pid[4];  // ±єБЦАЗ PID
	int64_t money[4];  // ±єБЦАЗ є°°і µ·	
	char name[4][32];  // ±єБЦАЗ АМё§	
	char date[4][32];  // ±єБЦ µо·П іЇВҐ
} MonarchInfo;

typedef struct TMonarchElectionInfo
{
	DWORD pid;  // ЕхЗҐ ЗС»з¶ч PID
	DWORD selectedpid; // ЕхЗҐ ґзЗС PID ( ±єБЦ Вь°ЎАЪ )
	char date[32]; // ЕхЗҐ іЇВҐ
} MonarchElectionInfo;
...
//µ¶АП ј±№° ѕЛёІ ±вґЙ ЕЧЅєЖ®їл ЖРЕ¶ Б¤єё
typedef struct tItemAwardInformer
{
	char	login[LOGIN_MAX_LEN + 1];
	char	command[20];		//ён·Йѕо
	unsigned int vnum;			//ѕЖАМЕЫ
} TPacketItemAwardInfromer;
// ј±№° ѕЛёІ ±вґЙ »иБ¦їл ЖРЕ¶ Б¤єё
