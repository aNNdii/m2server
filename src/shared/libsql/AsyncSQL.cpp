...
bool CAsyncSQL::Connect()
{
	...
	// db cacheґВ common dbАЗ LOCALE ЕЧАМєнїЎј­ localeА» ѕЛѕЖїА°н, АМИД character setА» јцБ¤ЗСґЩ.
	// µы¶уј­ ГЦГК ConnectionА» ёОА» ¶§їЎґВ localeА» ёрёЈ±в ¶§№®їЎ character setА» Б¤ЗТ јц°Ў ѕшАЅїЎµµ єТ±ёЗП°н,
	// °­Б¦·О character setА» euckr·О Б¤ЗПµµ·П µЗѕоАЦѕо АМ єОєРА» БЦј®Гіё® ЗПїґґЩ.
	// (ѕЖ·Ў БЦј®А» З®ёй mysqlїЎ euckrАМ ѕИ ±т·БАЦґВ µрєсїЎ Бў±ЩЗТ јц°Ў ѕшґЩ.)
...
{
	cProfiler profiler(500000); // 0.5ГК
	...
	while (!m_bEnd)
	{
		...
		while (count--)
		{
			...
			// 0.5ГК АМ»у °Й·ИАёёй ·О±ЧїЎ іІ±в±в
			if (!profiler.IsOk())
...
size_t CAsyncSQL::EscapeString(char* dst, size_t dstSize, const char *src, size_t srcSize)
{
	...
	{
		// \0АМ ѕИєЩѕоАЦА» ¶§ё¦ ґлєсЗШј­ 256 №ЩАМЖ®ёё є№»зЗШј­ ·О±Ч·О Гв·В
		char tmp[256];
		size_t tmpLen = sizeof(tmp) > srcSize ? srcSize : sizeof(tmp); // µС БЯїЎ АЫАє Е©±в
