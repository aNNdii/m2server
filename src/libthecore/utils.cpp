...
void trim_and_lower(const char * src, char * dest, size_t dest_size)
{
	...
	*dest = '\0';
    ...
    {
	*(dest++) = LOWER(*tmp); // LOWERґВ ёЕЕ©·О¶у ++ ѕІёй ѕИµК
	++tmp;
	++len;
    }
...
/* arg1 АМ arg2 АЗ ѕХґЬ°ъ °°А» ¶§ 1 А» ё®ЕПЗСґЩ. */
...
/* "Name : єсї±" °ъ °°АМ "ЗЧёс : °Є" Аё·О АМ·зѕоБш №®АЪї­їЎј­ 
   ЗЧёсА» token Аё·О, °ЄА» value ·О є№»зЗПї© ё®ЕПЗСґЩ. */
