...
int socket_write(socket_t desc, const char *data, size_t length)
{
	...
	if ((bytes_written = socket_write_tcp(desc, data, total)) < 0)
	{
		...
		sys_err("write to desc error");   // 'єёЕл' »уґлЖнАё·О єОЕН БўјУАМ Іч±д °НАМґЩ.
...
//А©µµїм ј­№цґВ °і№ЯїлАё·Оёё ѕІ±в ¶§№®їЎ BIND ipё¦ INADDR_ANY·О °нБ¤
//(ЕЧЅєЖ®АЗ ЖнАЗјєА» А§ЗШ)
#ifndef _WIN32
