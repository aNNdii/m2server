	...
	if (pInfo != NULL)
	{
		switch (pInfo->state)
		{
			case 0:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("ёуЅєЕНµйАЗ °ш°Э±оБц 5єР іІѕТЅАґПґЩ!!!"));
				}
			...
			case 1:
				{
					++pInfo->state;
					BroadcastNotice(LC_TEXT("ёуЅєЕНµйАЗ °ш°Э±оБц 1єР іІѕТЅАґПґЩ!!!"));
				}
			...
			case 2:
				{
					...
					BroadcastNotice(LC_TEXT("ёуЅєЕНµйАМ јєА» °ш°ЭЗП±в ЅГАЫЗЯЅАґПґЩ."));
			...
			case 3 :
				{
					...
					{
						pInfo->state = 6;
						SendNoticeMap(LC_TEXT("БЯѕУ Б¦ґЬїЎ ѕЗАЗ ±вїоАМ ёрї©µмґПґЩ."), pInfo->nMapIndex, false);
					}
					else
					{
						pInfo->wait_count++;
						...
						{
							pInfo->state++;
							SendNoticeMap(LC_TEXT("ёуЅєЕНµйАМ №°·Ї°Ґ Б¶БьА» єёАФґПґЩ."), pInfo->nMapIndex, false);
						}
						else
						{
			...
			case 4 :
				{
					pInfo->state++;
					SendNoticeMap(LC_TEXT("ёуЅєЕНµйАМ №°·Ї°Ў±в ЅГАЫЗЯЅАґПґЩ."), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("АбЅГ ИД ё¶А»·О µ№ѕЖ°©ґПґЩ."), pInfo->nMapIndex, false);
			...
			case 6 :
				{
					...
					SendNoticeMap(LC_TEXT("ёуЅєЕНµйАЗ ґлАеАМ іЄЕёіµЅАґПґЩ."), pInfo->nMapIndex, false);
					SendNoticeMap(LC_TEXT("30єР і»·О ±Нёс·ЙБЦё¦ №°ё®ГДБЦјјїд."), pInfo->nMapIndex, false);
			...
			case 7 :
				{
					if ( SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0 )
					{
						SendNoticeMap(LC_TEXT("±Нёс·ЙБЦїН ±ЧАЗ єОЗПµйА» ёрµО №°ё®ГЖЅАґПґЩ."), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("АбЅГ ИД ё¶А»·О µ№ѕЖ°©ґПґЩ."), pInfo->nMapIndex, false);
					...
					{
						SendNoticeMap(LC_TEXT("±Нёс·ЙБЦ°Ў Ер°ўЗПїґЅАґПґЩ."), pInfo->nMapIndex, false);
						SendNoticeMap(LC_TEXT("АбЅГ ИД ё¶А»·О µ№ѕЖ°©ґПґЩ."), pInfo->nMapIndex, false);
...
bool CBattleArena::Start(int nEmpire)
{
	...
	char szBuf[1024];
	snprintf(szBuf, sizeof(szBuf), LC_TEXT("%sАЗ јєАё·О ёуЅєЕНµйАМ Бш±єЗП°н АЦЅАґПґЩ."), EMPIRE_NAME(m_nEmpire));
	BroadcastNotice(szBuf);
	BroadcastNotice(LC_TEXT("10єР µЪ јєА» °ш°ЭЗТ ї№Б¤АФґПґЩ."));
