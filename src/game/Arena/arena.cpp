...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		if (chA != NULL)
		{
			chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·Г »уґл°Ў »з¶уБ® ґл·ГА» Бѕ·бЗХґПґЩ."));
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerAPID(), pArena->GetPlayerBPID());
		}

		if (chB != NULL)
		{
			chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·Г »уґл°Ў »з¶уБ® ґл·ГА» Бѕ·бЗХґПґЩ."));
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerBPID(), pArena->GetPlayerAPID());
		}

		pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("ґл·Г »уґл°Ў »з¶уБ® ґл·ГА» Бѕ·бЗХґПґЩ."));
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("№°ѕа Б¦ЗСАМ ѕшЅАґПґЩ."));
					chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("№°ѕа Б¦ЗСАМ ѕшЅАґПґЩ."));
				}
				else
				{
					...
					chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("№°ѕаА» %d °і ±оБц »зїл °ЎґЙЗХґПґЩ."), chA->GetPotionLimit());
					chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("№°ѕаА» %d °і ±оБц »зїл °ЎґЙЗХґПґЩ."), chB->GetPotionLimit());
				}
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґл·ГАМ ЅГАЫµЛґПґЩ."));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґл·ГАМ ЅГАЫµЛґПґЩ."));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґл·ГАМ ЅГАЫµЛґПґЩ."));
		...
		case 1:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАМ ЅГАЫµЗѕъЅАґПґЩ."));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАМ ЅГАЫµЗѕъЅАґПґЩ."));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАМ ЅГАЫµЗѕъЅАґПґЩ."));

				TPacketGCDuelStart duelStart;
				duelStart.header = HEADER_GC_DUEL_START;
				duelStart.wSize = sizeof(TPacketGCDuelStart) + 4;

				DWORD dwOppList[8]; // ГЦґл ЖДЖјїш 8ён АМ№З·О..
		...
		default:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАе №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
				chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАе №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАе №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
...
EVENTFUNC(duel_time_out)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("ґл·Г »уґл°Ў »з¶уБ® ґл·ГА» Бѕ·бЗХґПґЩ."));
	...
	else
	{
		...
		{
			case 0:
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("ґл·Г ЅГ°Ј ГК°ъ·О ґл·ГА» БЯґЬЗХґПґЩ."));
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("10ГКµЪ ё¶А»·О АМµїЗХґПґЩ."));

				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("ґл·Г ЅГ°Ј ГК°ъ·О ґл·ГА» БЯґЬЗХґПґЩ."));
				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("10ГКµЪ ё¶А»·О АМµїЗХґПґЩ."));

				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("ґл·Г ЅГ°Ј ГК°ъ·О ґл·ГА» БЯґЬЗХґПґЩ."));
				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("10ГКµЪ ё¶А»·О АМµїЗХґПґЩ."));
...
{
	// 1:1 Аьїл ґЩґлґЩ ЗТ °жїм јцБ¤ ЗКїд
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		// µСґЩ БўјУАМ ІчѕоБіґЩ ?!
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("ґл·ГАЪ №®Б¦·О АОЗПї© ґл·ГА» БЯґЬЗХґПґЩ."));
		restart = false;
	}
	else if (pCharA == NULL && pCharB != NULL)
	{
		pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("»уґл№ж ДіёЇЕНАЗ №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("ґл·ГАЪ №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
		pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("»уґл№ж ДіёЇЕНАЗ №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("ґл·ГАЪ №®Б¦·О АОЗПї© ґл·ГА» Бѕ·бЗХґПґЩ."));
		restart = false;
	}
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharA->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharA->GetName());
			...
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ґФАМ ЅВё®ЗПїґЅАґПґЩ."), pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ґФАМ ЅВё®ЗПїґЅАґПґЩ."), pCharA->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharB->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("%s ґФАМ ґл·ГїЎј­ ЅВё®ЗПїґЅАґПґЩ."), pCharB->GetName());
			...
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ґФАМ ЅВё®ЗПїґЅАґПґЩ."), pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ґФАМ ЅВё®ЗПїґЅАґПґЩ."), pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ё¶А»·О µЗµ№ѕЖ°©ґПґЩ."));

		if (	pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ё¶А»·О µЗµ№ѕЖ°©ґПґЩ."));

		SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ё¶А»·О µЗµ№ѕЖ°©ґПґЩ."));
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґЩАЅ ЖЗА» ЅГАЫЗХґПґЩ."));

		if (pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґЩАЅ ЖЗА» ЅГАЫЗХґПґЩ."));

		SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("10ГКµЪ ґЩАЅ ЖЗА» ЅГАЫЗХґПґЩ."));
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB() != NULL)
			GetPlayerB()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»уґл№ж ДіёЇЕН°Ў БўјУА» Бѕ·бЗПї© ґл·ГА» БЯБцЗХґПґЩ."));
	...
	{
		if (GetPlayerA() != NULL)
			GetPlayerA()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»уґл№ж ДіёЇЕН°Ў БўјУА» Бѕ·бЗПї© ґл·ГА» БЯБцЗХґПґЩ."));
