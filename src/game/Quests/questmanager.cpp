...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("click", QUEST_CLICK_EVENT));		// NPCё¦ Е¬ёЇ
		m_mapEventName.insert(TEventNameMap::value_type("kill", QUEST_KILL_EVENT));		// MobА» »зіЙ
		m_mapEventName.insert(TEventNameMap::value_type("timer", QUEST_TIMER_EVENT));		// №Мё® БцБ¤ЗШµР ЅГ°ЈАМ БціІ
		m_mapEventName.insert(TEventNameMap::value_type("levelup", QUEST_LEVELUP_EVENT));	// ·№є§ѕчА» ЗФ
		m_mapEventName.insert(TEventNameMap::value_type("login", QUEST_LOGIN_EVENT));		// ·О±ЧАО ЅГ
		m_mapEventName.insert(TEventNameMap::value_type("logout", QUEST_LOGOUT_EVENT));		// ·О±ЧѕЖїф ЅГ
		m_mapEventName.insert(TEventNameMap::value_type("button", QUEST_BUTTON_EVENT));		// ДщЅєЖ® №цЖ°А» ґ©ё§
		m_mapEventName.insert(TEventNameMap::value_type("info", QUEST_INFO_EVENT));		// ДщЅєЖ® Б¤єёГўА» ї°
		m_mapEventName.insert(TEventNameMap::value_type("chat", QUEST_CHAT_EVENT));		// ЖЇБ¤ Е°їцµе·О ґлИ­ё¦ ЗФ
		m_mapEventName.insert(TEventNameMap::value_type("in", QUEST_ATTR_IN_EVENT));		// ёКАЗ ЖЇБ¤ јУјєїЎ µйѕо°Ё
		m_mapEventName.insert(TEventNameMap::value_type("out", QUEST_ATTR_OUT_EVENT));		// ёКАЗ ЖЇБ¤ јУјєїЎј­ іЄїИ
		m_mapEventName.insert(TEventNameMap::value_type("use", QUEST_ITEM_USE_EVENT));		// ДщЅєЖ® ѕЖАМЕЫА» »зїл
		m_mapEventName.insert(TEventNameMap::value_type("server_timer", QUEST_SERVER_TIMER_EVENT));	// ј­№ц ЕёАМёУ (ѕЖБч ЕЧЅєЖ® ѕИµЖАЅ)
		m_mapEventName.insert(TEventNameMap::value_type("enter", QUEST_ENTER_STATE_EVENT));	// ЗцАз ЅєЕЧАМЖ®°Ў µК
		m_mapEventName.insert(TEventNameMap::value_type("leave", QUEST_LEAVE_STATE_EVENT));	// ЗцАз ЅєЕЧАМЖ®їЎј­ ґЩёҐ ЅєЕЧАМЖ®·О №ЩІс
		m_mapEventName.insert(TEventNameMap::value_type("letter", QUEST_LETTER_EVENT));		// ·О±д ЗП°ЕіЄ ЅєЕЧАМЖ®°Ў №ЩІё »х·О Б¤єёё¦ јјЖГЗШБаѕЯЗФ
		m_mapEventName.insert(TEventNameMap::value_type("take", QUEST_ITEM_TAKE_EVENT));	// ѕЖАМЕЫА» №ЮАЅ
		m_mapEventName.insert(TEventNameMap::value_type("target", QUEST_TARGET_EVENT));		// Её°Щ
		m_mapEventName.insert(TEventNameMap::value_type("party_kill", QUEST_PARTY_KILL_EVENT));	// ЖДЖј ёв№ц°Ў ёуЅєЕНё¦ »зіЙ (ё®ґхїЎ°Ф їИ)
		m_mapEventName.insert(TEventNameMap::value_type("unmount", QUEST_UNMOUNT_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("pick", QUEST_ITEM_PICK_EVENT));	// ¶іѕоБ®АЦґВ ѕЖАМЕЫА» ЅАµжЗФ.
		m_mapEventName.insert(TEventNameMap::value_type("sig_use", QUEST_SIG_USE_EVENT));		// Special item groupїЎ јУЗС ѕЖАМЕЫА» »зїлЗФ.
		m_mapEventName.insert(TEventNameMap::value_type("item_informer", QUEST_ITEM_INFORMER_EVENT));	// µ¶АПј±№°±вґЙЕЧЅєЖ®
	...
	void CQuestManager::Select(unsigned int pc, unsigned int selection)
	{
		...
		if ((pPC = GetPC(pc)) && pPC->IsRunning() && pPC->GetRunningQuestState()->suspend_state==SUSPEND_STATE_SELECT)
		{
			...
			{
				// Г¤ЖГ АМєҐЖ®АО °жїм
				// ЗцАз ДщЅєЖ®ґВ ѕоґА ДщЅєЖ®ё¦ ЅЗЗаЗТ °НАО°Ўё¦ °нёЈґВ ДщЅєЖ® АМ№З·О
				// іЎі»°н ј±ЕГµИ ДщЅєЖ®ё¦ ЅЗЗаЗСґЩ.
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			/* [hyo] ёч killЅГ БЯє№ Д«їоЖГ АМЅґ °ь·ГЗС јцБ¤»зЗЧ
			   quest scriptїЎ when 171.kill begin ... µоАЗ ДЪµе·О АОЗПї© ЅєЕ©ёіЖ®°Ў Гіё®µЗѕъґх¶уµµ
			   №Щ·О returnЗПБц ѕК°н ґЩёҐ °Л»зµµ јцЗаЗПµµ·П єЇ°жЗФ. (2011/07/21)
			*/   
			// call script
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ДщЅєЖ®ё¦ ·ОµеЗПґВ БЯАФґПґЩ. АбЅГёё ±вґЩ·Б БЦЅКЅГїА."));
	...
	// Speical Item GroupїЎ Б¤АЗµИ Group Use
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	// END OF ДщЅєЖ® АМєҐЖ® Гіё®
	///////////////////////////////////////////////////////////////////////////////////////////

	///////////////////////////////////////////////////////////////////////////////////////////
	void CQuestManager::LoadStartQuest(const string& quest_name, unsigned int idx)
	{
		for (std::set<string>::const_iterator it = g_setQuestObjectDir.begin(); it != g_setQuestObjectDir.end(); ++it)
		{
	...
	void CQuestManager::RegisterQuest(const string & stQuestName, unsigned int idx)
	{
		...
		THashMapQuestName::const_iterator it;
	...
	{
		QuestNameIndexMap::const_iterator it;
	...
	void CQuestManager::Reload()
	{
		...
		for (std::set<unsigned int>::const_iterator it = m_registeredNPCVnum.begin(); it != m_registeredNPCVnum.end(); ++it)
		{
			char buf[256];
			DWORD dwVnum = *it;
	...
	void CQuestManager::WriteRunningStateToSyserr()
	{
		...
		for (TEventNameMap::const_iterator it = m_mapEventName.begin(); it != m_mapEventName.end(); ++it)
		{
	...
	{
		ServerTimerMap::const_iterator it = m_mapServerTimer.find(make_pair(name, arg));
	...
	{
		ServerTimerMap::const_iterator it = m_mapServerTimer.begin();
