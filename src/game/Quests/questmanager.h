...
namespace quest
{
	...
	class CQuestManager : public singleton<CQuestManager>
	{
		...
		public:
			...
			PC *		GetPCForce(unsigned int pc);	// ЗцАз PCё¦ №ЩІЩБц ѕК°н PC ЖчАОЕНё¦ °ЎБ®їВґЩ.
			...
			void		ItemInformer(unsigned int pc, unsigned int vnum);	//µ¶АПј±№°±вґЙ
		...
		// begin_other_pc_blcok, end_other_pc_blockА» А§ЗС °ґГјµй.
		public:
