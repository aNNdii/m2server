...
namespace quest
{
	...
	void NPC::Set(unsigned int vnum, const string & script_name)
	{
		...
		{
			CQuestManager::TEventNameMap::iterator it = itEventName;
			++itEventName;

			for (std::set<string>::const_iterator itObjectDir = g_setQuestObjectDir.begin(); itObjectDir != g_setQuestObjectDir.end(); ++itObjectDir)
			{	
	...
	void NPC::LoadStateScript(int event_index, const char* filename, const char* script_name)
	{
		...
		else
		{
			...
			///////////////////////////////////////////////////////////////////////////
			// јшј­ Index (ї©·Ї°і АЦА» јц АЦАё№З·О АЦґВ °НАУ, ЅЗБ¦ index °ЄАє ѕІБц ѕКАЅ)
			j = i;
			i = s.find('.', i + 1);
	...
	bool NPC::HandleEvent(PC& pc, int EventIndex)
	{
		...
		if (fMatch.Matched())
		{
			...
			{
				if ( i != 0 ) {
					//2012.05.14 <±иїлїн> : ДщЅєЖ® ёЕґПАъАЗ m_pCurrentPC°Ў №ЩІоґВ °жїм°Ў №Я»эЗПї©,
					//µО°і АМ»уАЗ ЅєЕ©ёіЖ®ё¦ ЅЗЗаЅГ, µО№шВ° єОЕНґВ ДщЅєЖ® ёЕґПАъАЗ PC °ЄА» »х·О јВЖГЗСґЩ.
	...
	struct FuncMissHandleReceiveAllEvent
	{
		...
		void operator() (PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap)
		{
			...
			if (NPC::HasStartState(itQuestMap->second) && CQuestManager::instance().CanStartQuest(dwQuestIndex))
			{
				...
				NPC::AQuestScriptType::const_iterator it = QuestScript.find(QUEST_START_STATE_INDEX);
	...
	struct FuncMatchHandleReceiveAllEvent
	{
		...
		void operator() (PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap)
		{
			...
			NPC::AQuestScriptType::const_iterator itQuestScript = QuestScript.find(iPCState);
	...
	bool NPC::ExecuteEventScript(PC& pc, int EventIndex, DWORD dwQuestIndex, int iState)
	{
		...
		QuestMapType::iterator itQuest = rQuest.find(dwQuestIndex);
		...
		AQuestScriptType::const_iterator itState = rScript.find(iState);
