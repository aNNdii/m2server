...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґЩёҐ °Е·ЎГўАМ ї­ё°»уЕВїЎј­ґВ »уБЎ°Е·Ўё¦ ЗТјц °Ў ѕшЅАґПґЩ."));
		return false;
	}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (SHOP_SUBHEADER_GC_OK != ret) // №®Б¦°Ў АЦѕъАёёй єёіЅґЩ.
	{
		TPacketGCShop pack;
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Вшїл БЯАО ѕЖАМЕЫАє ЖЗёЕЗТ јц ѕшЅАґПґЩ."));
		return;
	}
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20ѕпіЙАМ ГК°ъЗПї© №°З°А» ЖИјц ѕшЅАґПґЩ."));
		return;
	}

	// 20050802.myevan.»уБЎ ЖЗёЕ ·О±ЧїЎ ѕЖАМЕЫ ID ГЯ°Ў
	sys_log(0, "SHOP: SELL: %s item name: %s(x%d):%u price: %u", ch->GetName(), item->GetName(), bCount, item->GetID(), dwPrice);

	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЖЗёЕ±ЭѕЧАЗ %d %% °Ў јј±ЭАё·О іЄ°Ў°ФµЛґПґЩ"), iVal);

	DBManager::instance().SendMoneyLog(MONEY_LOG_SHOP, item->GetVnum(), dwPrice);

	if (bCount == item->GetCount())
	{
		// ЗС±№їЎґВ ѕЖАМЕЫА» №цё®°н є№±ёЗШґЮ¶уґВ Бш»уАЇАъµйАМ ё№ѕЖј­
		// »уБЎ ЖЗёЕЅГ јУјє·О±Чё¦ іІ±дґЩ.
		if (LC_IsYMIR())
			item->AttrLog();
...
{
	// file АЇ№« ГјЕ©.
	// ѕшґВ °жїмґВ їЎ·Ї·О Гіё®ЗПБц ѕКґВґЩ.
