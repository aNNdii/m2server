...
static bool get_word(FILE *fp, char *buf) // їцµеґЬА§·О №ЮґВґЩ.
{
	int i = 0;
	int c;
...
bool regen_load(const char* filename, long lMapIndex, int base_x, int base_y)
{
	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			//NO_REGEN
			// Desc: 	regen.txt (їЬ ё®БЁ°ь·Г ЕШЅєЖ® ) їЎј­ ё®БЁ ЅГ°ЈА» 0Аё·О јјЖГЗТЅГ 
			// 			ё®БЁА» ЗПБц ѕИЗСґЩ.
			if (regen->time != 0)
			{
				// ГіАЅїЈ №«Б¶°З ё®БЁ ЗШБШґЩ.
...
void regen_reset(int x, int y)
{
	...
	for (regen = regen_list; regen; regen = regen->next)
	{
		...
		// БВЗҐ°Ў АЦАёёй БВЗҐ і»їЎ АЦґВ ё®БЁ ё®ЅєЖ®ёё ё®БЁ ЅГЕІґЩ.
		if (x != 0 || y != 0)
		{
