...
void DESC::Destroy()
{
	...
	if (!g_bAuthServer)
	{
		if (m_accountTable.login[0] && m_accountTable.passwd[0])
		{
			...
			enhance_strlcpymt(pack.login, m_accountTable.login, sizeof(pack.login));
			enhance_strlcpymt(pack.passwd, m_accountTable.passwd, sizeof(pack.passwd));
...
bool DESC::Setup(LPFDWATCH _fdw, socket_t _fd, const struct sockaddr_in & c_rSockAddr, DWORD _handle, DWORD _handshake)
{
	...
	//else
	//NOTE: АМ°Й іЄ¶ує°·О ґЩёЈ°Ф АвѕЖѕЯЗТ АМАЇ°Ў АЦіЄ?
	m_lpOutputBuffer = buffer_new(DEFAULT_PACKET_BUFFER_SIZE * 2);
...
int DESC::ProcessInput()
{
	...
	else
	{
		...
		// false°Ў ё®ЕП µЗёй ґЩёҐ phase·О №ЩІп °НАМ№З·О ґЩЅГ ЗБ·ОјјЅє·О µ№АФЗСґЩ!
	...
	else
	{
		...
		// 8№ЩАМЖ® ґЬА§·Оёё Гіё®ЗСґЩ. 8№ЩАМЖ® ґЬА§їЎ єОБ·ЗПёй АЯёшµИ ѕПИЈИ­ №цЖЫё¦ є№ИЈИ­
		// ЗТ °ЎґЙјєАМ АЦАё№З·О В©¶уј­ Гіё®ЗП±в·О ЗСґЩ.
		if (iSizeBuffer & 7) // & 7Ає % 8°ъ °°ґЩ. 2АЗ ЅВјцїЎј­ёё °ЎґЙ
...
void DESC::Packet(const void * c_pvData, int iSize)
{
	...
	if (m_iPhase == PHASE_CLOSE) // ІчґВ »уЕВёй єёі»Бц ѕКґВґЩ.
		return;

	if (m_stRelayName.length() != 0)
	{
		// Relay ЖРЕ¶Ає ѕПИЈИ­ЗПБц ѕКґВґЩ.
		TPacketGGRelay p;

		p.bHeader = HEADER_GG_RELAY;
		enhance_strlcpymt(p.szName, m_stRelayName.c_str(), sizeof(p.szName));
		p.lSize = iSize;
	...
	else
	{
		...
		else
		{
			...
			{
				// ѕПИЈИ­їЎ ЗКїдЗС ГжєРЗС №цЖЫ Е©±вё¦ И®єёЗСґЩ.
...
void DESC::SetPhase(int _phase)
{
	...
	switch (m_iPhase)
	{
		...
		case PHASE_SELECT:
			// ёЮЅЕАъ°Ў ДіёЇЕНґЬА§°Ў µЗёйј­ »иБ¦
			//MessengerManager::instance().Logout(GetAccountTable().login); // АЗµµАыАё·О break ѕИ°Л
		case PHASE_LOGIN:
		case PHASE_LOADING:
#ifndef _IMPROVED_PACKET_ENCRYPTION_
			m_bEncrypted = true;
...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		...
		GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґЩёҐ ДДЗ»ЕНїЎј­ ·О±ЧАО ЗПї© БўјУА» Бѕ·б ЗХґПґЩ."));
		DelayedDisconnect(5);
	}
	else
	{
...
void DESC::SendLoginSuccessPacket()
{
	...
	{   
		...
		{   
			...
			enhance_strlcpymt(p.guild_name[i], g->GetName(), sizeof(p.guild_name[i]));
		}   
		else
		{
			p.guild_id[i] = 0;
