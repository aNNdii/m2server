...
void CInputDB::LoginSuccess(DWORD dwHandle, const char *data)
{
	...
	if (!d)
	{
		...
		enhance_strlcpymt(pack.login, pTab->login, sizeof(pack.login));
		db_clientdesc->DBPacket(HEADER_GD_LOGOUT, dwHandle, &pack, sizeof(pack));
		return;
	}

	if (strcmp(pTab->status, "OK")) // OK°Ў ѕЖґПёй
	{
	...
	if (!bFound) // ДіёЇЕН°Ў ѕшАёёй ·ЈґэЗС Б¦±№Аё·О єёіЅґЩ.. -_-
	{
		TPacketGCEmpire pe;
...
void CInputDB::PlayerCreateSuccess(LPDESC d, const char * data)
{
	...
	if (china_event_server)
	{
		...
		//№«»з: АЪАО°©+3,Г¶ЖнЕх±ё+3,±ЭЖнЅЕ№Я+3,іІёёµµ+3,№й±Эёс°ЙАМ+3, ИжґЬ±Н°ЙАМ+3, јТ»кєО+3, їА°ўЖР+3, ИжґЬЖИВо+3 
		//АЪ°ґЈєїµё°+3,ї¬ИЇµО°З+3,±ЭЖнЅЕ№Я+3,ё¶ѕИµµ+3,И­ѕИ±Г+3,їБёс°ЙАМ+3, їБ±Н°ЙАМ+3, їА°ўЖР+3, ИжґЬЖИВо+3 
		//јц¶уЈєАЅѕз°©+3,ѕЦИсЕх±ё+3,±ЭЖнЅЕ№Я+3,іІёёµµ+3,БшБЦёс°ЙАМ+3, №й±Э±Н°ЙАМ+3, їА°ўЖР+3, ИжґЬЖИВо+3
		//№«ґзЈєј­ГµАЗ+3,ЕВА»ёр+3,±ЭЖнЅЕ№Я+3,АЪё°ј±+3,ёЕИ­·Й+3,БшБЦёс°ЙАМ+3, №й±Э±Н°ЙАМ+3, їА°ўЖР+3, ИжґЬЖИВо+3
...
void CInputDB::ChangeName(LPDESC d, const char * data)
{
	...
	for (size_t i = 0; i < PLAYER_PER_ACCOUNT; ++i)
		...
		{
			enhance_strlcpymt(r.players[i].szName, p->name, sizeof(r.players[i].szName));
			...
			pgc.pid = p->pid;
			enhance_strlcpymt(pgc.name, p->name, sizeof(pgc.name));
...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	if (lMapIndex == 0)
	{
		...
		if (lMapIndex == 0) // БВЗҐё¦ ГЈА» јц ѕшґЩ.
		{
	...
	// Private ёКїЎ АЦѕъґВµҐ, Private ёКАМ »з¶уБш »уЕВ¶уёй Гв±ё·О µ№ѕЖ°ЎѕЯ ЗСґЩ.
	// ----
	// ±ЩµҐ Гв±ё·О µ№ѕЖ°ЎѕЯ ЗСґЩёйј­... їЦ Гв±ё°Ў ѕЖґП¶у private map »уїЎ ґлААµЗґВ pulic mapАЗ А§ДЎё¦ ГЈіД°н...
	// їЄ»зё¦ ёрёЈґП... ¶З ЗПµеДЪµщ ЗСґЩ.
	// ѕЖ±Нµї±јАМёй, Гв±ё·О...
	// by rtsummit
	...
	
	{
		...
		enhance_strlcpymt(p.szName, ch->GetName(), sizeof(p.szName));
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґЩёҐ°Е·ЎГўАМ ї­ё°»уЕВїЎј­ґВ Гў°нё¦ ї­јц°Ў ѕшЅАґПґЩ." ) );
...
//
// @version	05/06/20 Bang2ni - ReqSafeboxLoad АЗ ГлјТ
//
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Гў°н> Гў°н єс№Р№шИЈ°Ў єЇ°жµЗѕъЅАґПґЩ."));
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Гў°н> ±вБё єс№Р№шИЈ°Ў ЖІ·ИЅАґПґЩ."));
	}
}
...
void CInputDB::LoginAlready(LPDESC d, const char * c_pData)
{
	...
	// INTERNATIONAL_VERSION АМ№М БўјУБЯАМёй БўјУ ІчАЅ
	{ 
		...
		else
		{
			...
			enhance_strlcpymt(pgg.szLogin, p->szLogin, sizeof(pgg.szLogin));
...
void CInputDB::AuthLogin(LPDESC d, const char * c_pData)
{
	...
	if (bResult)
	{
		// Panama ѕПИЈИ­ ЖСїЎ ЗКїдЗС Е° єёі»±в
		SendPanamaList(d);
		ptoc.dwLoginKey = d->GetLoginKey();

		//NOTE: AuthSucessєёґЩ ёХАъ єёі»ѕЯБц ѕИ±Ч·Їёй PHASE Close°Ў µЗј­ єёі»БцБц ѕКґВґЩ.-_-
...
void CInputDB::Notice(const char * c_pData)
{
	...
	char szBuf[256+1];
	enhance_strlcpymt(szBuf, c_pData, sizeof(szBuf));
...
////////////////////////////////////////////////////////////////////
// Analyze
// @version	05/06/10 Bang2ni - ѕЖАМЕЫ °Ў°ЭБ¤єё ё®ЅєЖ® ЖРЕ¶(HEADER_DG_MYSHOP_PRICELIST_RES) Гіё®·зЖѕ ГЯ°Ў.
////////////////////////////////////////////////////////////////////
...
void CInputDB::AddMonarchMoney(LPDESC d, const char * data )
{
	...
	{
		if (number(1, 100) > 95) 
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЗцАз %s ±№°нїЎґВ %u АЗ µ·АМ АЦЅАґПґЩ"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::DecMonarchMoney(LPDESC d, const char * data)
{
	...
	if (ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЗцАз %s ±№°нїЎґВ %d АЗ µ·АМ АЦЅАґПґЩ"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::TakeMonarchMoney(LPDESC d, const char * data)
{
	...
	if (!CMonarch::instance().DecMoney(Money, Empire))
	{
		...
		LPCHARACTER ch = d->GetCharacter();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±№°нїЎ µ·АМ єОБ·ЗП°ЕіЄ µ·А» °ЎБ®їГјц ѕшґВ »уИІАФґПґЩ"));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_TEXT("%sАЗ ±єБЦ°Ў %s ґФАё·О ±іГјµЗѕъЅАґПґЩ."), EMPIRE_NAME(info->bEmpire), info->szName);
	SendNotice(notice);
}
...
{	
	LPDESC d = DESC_MANAGER::instance().FindByLoginName(data->login);	//loginБ¤єё
	...
	else
	{
		...
		{
			LPCHARACTER ch = d->GetCharacter();	
			ch->SetItemAward_vnum(data->vnum);	// ch їЎ АУЅГ АъАеЗШіщґЩ°Ў QuestLoad ЗФјцїЎј­ Гіё®
			ch->SetItemAward_cmd(data->command);		

			if(d->IsPhase(PHASE_GAME))			//°ФАУЖдАМБоАП¶§
			{
				quest::CQuestManager::instance().ItemInformer(ch->GetPlayerID(),ch->GetItemAward_vnum());	//questmanager ИЈГв
			}
		}
	}
}
