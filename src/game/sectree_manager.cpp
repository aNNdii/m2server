...
{
    // Е¬¶уАМѕрЖ®їЎ°Ф №Э°ж 150m ДіёЇЕНАЗ Б¤єёё¦ БЦ±вА§ЗШ
    // 3x3Д­ -> 5x5 Д­Аё·О БЦєЇsectree И®ґл(ЗС±№)
...
#define NUM_OF_NEIGHBORS	(NEIGHBOR_LENGTH * NEIGHBOR_LENGTH - 1)
	...
	//
	// ёрµз sectreeїЎ ґлЗШ БЦА§ sectreeµй ё®ЅєЖ®ё¦ ёёµзґЩ.
	//
	MapType::iterator it = map_.begin();
	...
	while (it != map_.end())
	{
		...
		tree->m_neighbor_list.push_back(tree); // АЪЅЕА» іЦґВґЩ.
...
// -----------------------------------------------------------------------------
// Setting.txt ·О єОЕН SECTREE ёёµй±в
// -----------------------------------------------------------------------------
...
// Fix me
// ЗцАз Town.txtїЎј­ x, yё¦ ±ЧіЙ №Ю°н, ±Ч°Й АМ ДЪµе і»їЎј­ base БВЗҐё¦ ґхЗШБЦ±в ¶§№®їЎ
// ґЩёҐ ёКїЎ АЦґВ ЕёїоАё·О Аэґл АМµїЗТ јц ѕш°Ф µЗѕоАЦґЩ.
// ѕХїЎ mapАМ¶у°ЕіЄ, ±вЕё ґЩёҐ ЅДє°АЪ°Ў АЦАёёй,
// ґЩёҐ ёКАЗ ЕёїоАё·Оµµ АМµїЗТ јц АЦ°Ф ЗПАЪ.
// by rtsummit
...
bool SECTREE_MANAGER::LoadAttribute(LPSECTREE_MAP pkMapSectree, const char * c_pszFileName, TMapSetting & r_setting)
{
	...
	for (int y = 0; y < iHeight; ++y)
		...
		{
			// UNION Аё·О БВЗҐё¦ ЗХГДёёµз DWORD°ЄА» ѕЖАМµр·О »зїлЗСґЩ.
			SECTREEID id;
...
bool SECTREE_MANAGER::GetRecallPositionByEmpire(int iMapIndex, BYTE bEmpire, PIXEL_POSITION & r_pos)
{
	...
	// 10000А» іСґВ ёКАє АОЅєЕПЅє ґшАьїЎёё ЗСБ¤µЗѕоАЦґЩ.
	if (iMapIndex >= 10000)
	{
		iMapIndex /= 10000;
	}
...
const TMapRegion * SECTREE_MANAGER::FindRegionByPartialName(const char* szMapName)
{
	...
	while (it != m_vec_mapRegion.end())
	{
		...
		if (rRegion.strMapName.find(szMapName))
			return &rRegion; // ДіЅМ ЗШј­ єьёЈ°Ф ЗПАЪ
	}
...
int SECTREE_MANAGER::Build(const char * c_pszListFileName, const char* c_pszMapBasePath)
{
	...
	while (fgets(buf, 256, fp))
	{
		*strrchr(buf, '\n') = '\0';
		...
		// ёХАъ АМ ј­№цїЎј­ АМ ёКАЗ ёуЅєЕНё¦ ЅєЖщЗШѕЯ ЗПґВ°Ў И®АО ЗСґЩ.
...
{
	if (lMapIndex >= 10000) // 10000№ш АМ»уАЗ ёКАє ѕшґЩ. (И¤Ає АМ№М private АМґЩ)
		return 0;
...
{
	if (lMapIndex < 10000) // private map Ає АОµ¦Ѕє°Ў 10000 АМ»у АМґЩ.
		return;
	...
	// АМ ёК А§їЎ ЗцАз БёАзЗПґВ °НµйА» АьєО ѕшѕШґЩ.
	// WARNING:
	// АМ ёКїЎ АЦБцёё ѕо¶І SectreeїЎµµ БёАзЗПБц ѕКА» јц АЦАЅ
	// µы¶уј­ ї©±вј­ delete ЗТ јц ѕшАё№З·О ЖчАОЕН°Ў ±ъБъ јц АЦАёґП
	// є°µµ Гіё®ё¦ ЗШѕЯЗФ
...
TAreaMap& SECTREE_MANAGER::GetDungeonArea(long lMapIndex)
{
	...
	if (it == m_map_pkArea.end())
		return m_map_pkArea[-1]; // АУЅГ·О єу Areaё¦ ё®ЕП
...
bool SECTREE_MANAGER::ForAttrRegion(long lMapIndex, long lStartX, long lStartY, long lEndX, long lEndY, long lRotate, DWORD dwAttr, EAttrRegionMode mode)
{
	...
	//
	// їµїЄАЗ БВЗҐё¦ Cell АЗ Е©±вїЎ ёВГз И®АеЗСґЩ.
	//
