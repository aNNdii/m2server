...
enum
{
	...
	HEADER_CG_TEXT				= 64,	// @ ·О ЅГАЫµЗёй ЕШЅєЖ®ё¦ ЖДЅМЗСґЩ.
	HEADER_CG_WARP				= 65,
	HEADER_CG_SCRIPT_BUTTON			= 66,
	HEADER_CG_MESSENGER				= 67,

	HEADER_CG_MALL_CHECKOUT			= 69,
	HEADER_CG_SAFEBOX_CHECKIN			= 70,	// ѕЖАМЕЫА» Гў°нїЎ іЦ†љґВґЩ.
	HEADER_CG_SAFEBOX_CHECKOUT			= 71,	// ѕЖАМЕЫА» Гў°н·О єОЕН »©їВґЩ.
...
//	HEADER_CG_ROULETTE				= 200,
	...
	HEADER_GG_GUILD				= 6,
	HEADER_GG_DISCONNECT			= 7,	// ґ©±є°ЎАЗ БўјУА» °­Б¦·О ІчА» ¶§
	HEADER_GG_SHOUT				= 8,
	HEADER_GG_SETUP				= 9,
...
{
	BYTE header;
	DWORD dwFromPID; // Аъ А§ДЎ·О їцЗБЗП·БґВ »з¶ч
	DWORD dwTargetPID; // ГЈґВ »з¶ч
...
// А§ДЎ µї±вИ­
typedef struct command_sync_position	// °ЎєЇ ЖРЕ¶
{
	BYTE	bHeader;
	WORD	wSize;
...
/* Г¤ЖГ (3) */
typedef struct command_chat	// °ЎєЇ ЖРЕ¶
{
	BYTE	header;
	WORD	size;
	BYTE	type;
...
typedef struct packet_chat	// °ЎєЇ ЖРЕ¶
{
	BYTE	header;
	WORD	size;
	BYTE	type;
...
typedef struct packet_whisper	// °ЎєЇ ЖРЕ¶
{
	BYTE	bHeader;
	WORD	wSize;
	BYTE	bType;
...
typedef struct packet_shop_start_ex // ґЩАЅїЎ TSubPacketShopTab* shop_tabs АМ µы¶уїИ.
{
...
typedef struct packet_shop	// °ЎєЇ ЖРЕ¶
{
	BYTE        header;
	WORD	size; 
	BYTE        subheader;
...
// А§ДЎ µї±вИ­ ЖРЕ¶АЗ bCount ёёЕ­ єЩґВ ґЬА§
...
// А§ДЎ µї±вИ­
typedef struct packet_sync_position	// °ЎєЇ ЖРЕ¶
{
	BYTE	bHeader;
	WORD	wSize;	// °іјц = (wSize - sizeof(TPacketGCSyncPosition)) / sizeof(TPacketGCSyncPositionElement) 
...
{
	BYTE	header;
	WORD	wSize;	// DWORD°Ў ёо°і? °іјц = (wSize - sizeof(TPacketGCPVPList)) / 4
} TPacketGCDuelStart;
...
typedef struct packet_mark_idxlist
{
	...
	DWORD	bufSize;
	WORD	count;
	//µЪїЎ size * (WORD + WORD)ёёЕ­ µҐАМЕН єЩАЅ
...
typedef struct packet_mark_block
{
	...
	BYTE	imgIdx;
	DWORD	count;
	// µЪїЎ 64 x 48 x ЗИјїЕ©±в(4№ЩАМЖ®) = 12288ёёЕ­ µҐАМЕН єЩАЅ
} TPacketGCMarkBlock;
...
//TODO :  ѕЖїм ВЇіЄ..°ЎєЇЖРЕ¶ »зАМБо №ЮѕЖµйАПјц АЦ°Ф ЖРЕ¶ ЗЪµй·Ї Refactoring ЗПАЪ. 
typedef struct SPacketGCHybridCryptKeys
{
	SPacketGCHybridCryptKeys() : m_pStream(NULL) {}
	~SPacketGCHybridCryptKeys()
	{
		//GCC їЎј± NULL delete ЗШµµ ±¦ВъіЄ? АПґЬ ѕИАьЗП°Ф NULL ГјЕ© ЗПАЪ. ( ±ЩµҐ АМ°Е C++ ЗҐБШѕЖґПѕъіЄ --a )
		if( m_pStream )
		{
			delete[] m_pStream;
			m_pStream = NULL;
	...
	BYTE	bHeader;
	WORD    uDynamicPacketSize; // єфѕоёФА» Е¬¶у  DynamicPacketHeader ±ёБ¶¶§№®їЎ ёВГзБаѕЯЗСґЩ -_-;
	int		KeyStreamLen;
	BYTE*   pDataKeyStream;
