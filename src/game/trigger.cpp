...
/*
 * ёуЅєЕН AI ЗФјцµйА» BattleAI Е¬·ЎЅє·О јцБ¤
 */
int OnIdleDefault(TRIGGERPARAM)
{
	if (ch->OnIdle())
...
class FuncFindMobVictim
{
			...
			m_pkChrBuilding(NULL)
	{
		...
		bool operator () (LPENTITY ent)
		{
			...
			if (pkChr->IsAffectFlag(AFF_TERROR) && m_pkChr->IsImmune(IMMUNE_TERROR) == false )	// °шЖч Гіё®
			{
		...
		{
			// ±ЩГіїЎ °З№°АМ АЦ°н ЗЗ°Ў ё№АМ АЦґЩёй °З№°А» °ш°ЭЗСґЩ. °З№°ёё АЦѕоµµ °З№°А» °ш°Э
