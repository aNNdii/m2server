...
namespace quest 
{
	...
	int pc_warp_local(lua_State * L)
	{
		...
		{
			pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("°Е·ЎГў,Гў°н µоА» ї¬ »уЕВїЎј­ґВ ґЩёҐ°чАё·О АМµїЗТјц ѕшЅАґПґЩ" ));
		...
		{
			pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("°Е·Ў ИД 1єР АМі»їЎґВ ґЩёҐБцїЄАё·О АМµї ЗТ јц ѕшЅАґПґЩ." ) );
			return;
		}
	...
	int pc_give_item_from_special_item_group(lua_State* L)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("µ· %d іЙА» И№µжЗЯЅАґПґЩ."), dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("іЄ№«їЎј­ єОЕН ЅЕєсЗС єыАМ іЄїЙґПґЩ."));
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%dАЗ °жЗиДЎё¦ И№µжЗЯЅАґПґЩ."), dwCounts[i]);
				}
			}
		}
		return 0;
	...
	int pc_give_item(lua_State* L)
	{
		...
		if (lua_isnumber(L,2)) // №шИЈАО°жїм №шИЈ·О БШґЩ.
	...
	int pc_give_or_drop_item(lua_State* L)
	{
		...
		if (lua_isnumber(L, 1)) // №шИЈАО°жїм №шИЈ·О БШґЩ.
		{
	...
	int pc_change_sp(lua_State * L)
	{
		...
		if (val > 0) // Бх°ЎЅГЕ°ґВ °НАМ№З·О №«Б¶°З јє°ш ё®ЕП
	...
	// 20050725.myevan.АєµРАЗ ёБЕд »зїлБЯ ИҐј® јц·ГЅГ ј±ѕЗДЎ°Ў µО№и јТёрµЗґВ №ц±Ч°Ў №Я»эЗШ
	// ЅЗБ¦ ј±ѕЗДЎё¦ АМїлЗШ °и»кА» ЗП°Ф ЗСґЩ.
	...
	int pc_mount(lua_State* L)
	{
		...
		// ё»АМ јТИЇµЗѕо µы¶уґЩґПґВ »уЕВ¶уёй ё»єОЕН ѕшѕЪ
		if (ch->GetHorse())
...
teleport_area:
	...
	{
		// ё®ЕП°Є
		//		0: »хАМё§А» јіБ¤ЗС µЪ ·О±ЧѕЖїфА» ѕИЗЯАЅ
		//		1: ЅєЕ©ёіЖ®їЎј­ №®АЪї­АМ іСѕоїАБц ѕКѕТАЅ
		//		2: check_name А» Ел°ъЗПБц ёшЗЯАЅ
		//		3: АМ№М °°Ає АМё§АМ »зїлБЯ
		//		4: јє°ш
		//		5: ЗШґз ±вґЙ БцїшЗПБц ѕКАЅ
		if ( LC_IsEurope() )
		{
			lua_pushnumber(L, 5);
			return 1;
		...
		if ( pmsg->Get()->uiNumRows > 0 )
		{
			...
			// АМ№М ЗШґз АМё§А» °ЎБш ДіёЇЕН°Ў АЦАЅ
			if ( count != 0 )
			{
				lua_pushnumber(L, 3);
				return 1;
	...
	int pc_get_socket_items( lua_State* L )
	{
		...
		// їлИҐј® ЅЅ·ФАє ЗТ ЗКїд ѕшА» µн.
		// АМ ЗФјцґВ Е»ј®ј­їл ЗФјцАО µн ЗПґЩ.
	...
	int pc_get_informer_type(lua_State* L)	//µ¶АП ј±№° ±вґЙ
	{
		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] = 
		{
			...
			{ "get_informer_type",	pc_get_informer_type	},	//µ¶АП ј±№° ±вґЙ
			{ "get_informer_item",  pc_get_informer_item	},

			{ "give_award",			pc_give_award			},	//АПє» °иБ¤ґз ЗС№шѕї ±Э±« Бц±Ю
			{ "give_award_socket",	pc_give_award_socket	},	//ёф АОєҐЕдё®їЎ ѕЖАМЕЫ Бц±Ю. јТДП јіБ¤А» А§ЗС ЗФјц.

			{ "get_killee_drop_pct",	pc_get_killee_drop_pct	}, /* mob_vnum.kill АМєҐЖ®їЎј­ killeeїН pcїНАЗ level ВчАМ, pcАЗ ЗБё®№Мѕц µе¶ш·ь µоµоА» °н·БЗС ѕЖАМЕЫ µе¶ш И®·ь.
																    * return °ЄАє (єРАЪ, єРёр).
																    * (ё»АМ є№АвЗСµҐ, CreateDropItemАЗ GetDropPctАЗ iDeltaPercent, iRandRangeё¦ returnЗСґЩ°н єёёй µК.)
																	* (АМ ё»АМ ґх ѕо·Бїп¶уіЄ ¤Р¤Р)
																	* БЦАЗ»зЗЧ : kill eventїЎј­ёё »зїлЗТ °Н!
																	*/
