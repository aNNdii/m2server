...
namespace quest
{
	...
	int affect_add(lua_State * L)
	{
		...
		if (ch->FindAffect(AFFECT_QUEST_START_IDX, applyOn)) // ДщЅєЖ®·О АОЗШ °°Ає °чїЎ Иї°ъ°Ў °Й·БАЦАёёй ЅєЕµ
			return 0;
	...
	int affect_remove_bad(lua_State * L) // іЄ»Ы Иї°ъё¦ ѕшѕЪ
	{
	...
	int affect_remove_good(lua_State * L) // ББАє Иї°ъё¦ ѕшѕЪ
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->RemoveGoodAffect();
		return 0;
	...
	int affect_remove_hair(lua_State * L) // Змѕо Иї°ъё¦ ѕшѕШґЩ.
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	// ЗцАз ДіёЇЕН°Ў AFFECT_TYPE affectё¦ °®°нАЦАёёй bApplyOn °ЄА» №ЭИЇЗП°н ѕшАёёй nilА» №ЭИЇЗПґВ ЗФјц.
