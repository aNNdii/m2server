...
namespace quest
{
	...
	int dungeon_exit(lua_State* L) // ґшАьїЎ µйѕоїА±в Аь А§ДЎ·О єёії
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int dungeon_exit_all(lua_State* L) // ґшАьїЎ АЦґВ ёрµз »з¶чА» ґшАьїЎ µйѕоїА±в Аь А§ДЎ·О єёії
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
	...
	int dungeon_exit_all_by_item_group (lua_State* L) // ЖЇБ¤ ѕЖАМЕЫ ±Ч·мїЎ јУЗС ѕЖАМЕЫАМ ѕшґВ»з¶чАє °­Ер
	{
		if (!lua_isstring(L, 1))
		{
			sys_log(0, "QUEST wrong set flag");
	...
	int dungeon_delete_item_in_item_group_from_all(lua_State* L) // ЖЇБ¤ ѕЖАМЕЫА» ґшАь і» pcїЎ°Фј­ »иБ¦.
	{
		if (!lua_isstring(L, 1))
		{
			sys_log(0, "QUEST wrong set flag");
