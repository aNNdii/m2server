...
bool CPolymorphUtils::PolymorphCharacter(LPCHARACTER pChar, LPITEM pItem, const CMob* pMob)
{
	...
	// єЇЅЕ И®·ь = ДіёЇЕН ·№є§ - ёч ·№є§ + µР°©ј­ ·№є§ + 29 + µР°© ЅєЕі ·№є§
	iPolyPercent = pChar->GetLevel() - pMob->m_table.bLevel + pItem->GetSocket(2) + (29 + bySkillLevel);

	if (iPolyPercent <= 0)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("µР°©їЎ ЅЗЖР ЗПїґЅАґПґЩ"));
		return false;
	}
	else
	{
...
{
	// јТДП0                јТДП1       јТДП2
	// µР°©ЗТ ёуЅєЕН №шИЈ   јц·ГБ¤µµ    µР°©ј­ ·№є§
	if (pChar == NULL)
		return false;
	...
	pItem->SetSocket(0, dwMobVnum);			// µР°©ЗТ ёуЅєЕН №шИЈ
	pItem->SetSocket(1, dwPracticeCount);		// јц·ГЗШѕЯЗТ ИЅјц
	pItem->SetSocket(2, BookLevel);			// јц·Г·№є§
	return true;
}
