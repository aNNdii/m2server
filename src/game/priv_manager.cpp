...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		...
		{
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеАЗ %sАМ %d%% Бх°ЎЗЯЅАґПґЩ!"), g->GetName(), GetPrivName(type), value);
			SendNotice(buf);
		}
		else
		{
			char buf[100];
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеАЗ %sАМ Б¤»уАё·О µ№ѕЖїФЅАґПґЩ."), g->GetName(), GetPrivName(type));
			SendNotice(buf);
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%sАЗ %sАМ %d%% Бх°ЎЗЯЅАґПґЩ!"), GetEmpireName(empire), GetPrivName(type), value);
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), LC_TEXT("%sАЗ %sАМ Б¤»уАё·О µ№ѕЖїФЅАґПґЩ."), GetEmpireName(empire), GetPrivName(type));
...
void CPrivManager::RemoveCharacterPriv(DWORD pid, BYTE type)
{
	...
	PrivCharMap::const_iterator it = m_aPrivChar[type].find(pid);
...
{
	// ДіёЇЕНАЗ єЇ°ж јцДЎ°Ў -¶уёй №«Б¶°З -ёё АыїлµЗ°Ф
...
int CPrivManager::GetPrivByGuild(DWORD guild_id, BYTE type)
{
	...
	PrivGuildMap::const_iterator itFind = m_aPrivGuild[type].find(guild_id);
...
const SPrivGuildData* CPrivManager::GetPrivByGuildEx( DWORD dwGuildID, BYTE byType ) const
{
	if ( byType >= MAX_PRIV_NUM )
		return NULL;

	PrivGuildMap::const_iterator itFind = m_aPrivGuild[byType].find(dwGuildID);
