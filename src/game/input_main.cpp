...
void SendBlockChatInfo(LPCHARACTER ch, int sec)
{
	if (sec <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Г¤ЖГ ±ЭБц »уЕВАФґПґЩ."));
		return;
	}
	...
	if (hour > 0 && min > 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d ЅГ°Ј %d єР %d ГК µїѕИ Г¤ЖГ±ЭБц »уЕВАФґПґЩ"), hour, min, sec);
	else if (hour > 0 && min == 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d ЅГ°Ј %d ГК µїѕИ Г¤ЖГ±ЭБц »уЕВАФґПґЩ"), hour, sec);
	else if (hour == 0 && min > 0)
		snprintf(buf, sizeof(buf), LC_TEXT("%d єР %d ГК µїѕИ Г¤ЖГ±ЭБц »уЕВАФґПґЩ"), min, sec);
	else
		snprintf(buf, sizeof(buf), LC_TEXT("%d ГК µїѕИ Г¤ЖГ±ЭБц »уЕВАФґПґЩ"), sec);
...
int GetTextTag(const char * src, int maxLen, int & tagLen, std::string & extraInfo)
{
	...
	else if (*cur == 'H') // hyperlink |Hitem:10000:0:0:0:0|h[АМё§]|h
	{
		tagLen = 2;
...
int ProcessTextTag(LPCHARACTER ch, const char * c_pszText, size_t len)
{
	...
	//0 : Б¤»уАыАё·О »зїл
	//1 : ±Э°­°ж єОБ·
	//2 : ±Э°­°жАМ АЦАёіЄ, °іАО»уБЎїЎј­ »зїлБЯ
	//3 : ±іИЇБЯ
	//4 : їЎ·Ї
	int hyperlinks;
	bool colored;
	...
	else
	{
		...
		else
		{
			...
			if (g_bEmpireWhisper)
				if (!ch->IsEquipUniqueGroup(UNIQUE_GROUP_RING_OF_LANGUAGE))
					if (!(pkChr && pkChr->IsEquipUniqueGroup(UNIQUE_GROUP_RING_OF_LANGUAGE)))
						if (bOpponentEmpire != ch->GetEmpire() && ch->GetEmpire() && bOpponentEmpire // ј­·О Б¦±№АМ ґЩёЈёйј­
								&& ch->GetGMLevel() == GM_PLAYER && gm_get_level(pinfo->szNameTo) == GM_PLAYER) // µСґЩ АП№Э ЗГ·№АМѕоАМёй
							// АМё§ №ЫїЎ ёрёЈґП gm_get_level ЗФјцё¦ »зїл
						{
							if (!pkChr)
							{
								// ґЩёҐ ј­№цїЎ АЦАёґП Б¦±№ ЗҐЅГёё ЗСґЩ. bTypeАЗ »уА§ 4єсЖ®ё¦ Empire№шИЈ·О »зїлЗСґЩ.
								bType = ch->GetEmpire() << 4;
							}
							else
							{
								ConvertEmpireText(ch->GetEmpire(), buf, buflen, 10 + 2 * pkChr->GetSkillPower(SKILL_LANGUAGE1 + ch->GetEmpire() - 1)/*єЇИЇИ®·ь*/);
							}
						}
			...
			if (0!=processReturn)
			{
				if (ch->GetDesc())
				{
					...
					if (pTable)
					{
						char buf[128];
						int len;
						if (3==processReturn) //±іИЇБЯ
							len = snprintf(buf, sizeof(buf), LC_TEXT("ґЩёҐ °Е·ЎБЯ(Гў°н,±іИЇ,»уБЎ)їЎґВ °іАО»уБЎА» »зїлЗТ јц ѕшЅАґПґЩ."), pTable->szLocaleName);
						else
							len = snprintf(buf, sizeof(buf), LC_TEXT("%sАМ ЗКїдЗХґПґЩ."), pTable->szLocaleName);
				...
				// ё±·ЎАМ »уЕВАП јц АЦАё№З·О ё±·ЎАМё¦ З®ѕоБШґЩ.
				pkDesc->SetRelay("");
				return (iExtraLen);
			}
			...
			if (buflen > 0)
			{
				...
				// desc->BufferedPacketА» ЗПБц ѕК°н №цЖЫїЎ ЅбѕЯЗПґВ АМАЇґВ 
				// P2P relayµЗѕо ЖРЕ¶АМ ДёЅ¶И­ µЙ јц АЦ±в ¶§№®АМґЩ.
				TEMP_BUFFER tmpbuf;
...
struct FEmpireChatPacket
{
	...
	void operator () (LPDESC d)
	{
		...
		{
			// »з¶чё¶ґЩ ЅєЕі·№є§АМ ґЩёЈґП ёЕ№ш ЗШѕЯЗХґПґЩ
...
struct FYmirChatPacket
{
	...
	FYmirChatPacket(packet_chat& p, const char* chat, size_t len_chat, const char* name, size_t len_name, int iMapIndex, BYTE empire, bool ring)
		...
		m_ring(ring)
	{
		m_len_orig_msg = snprintf(m_orig_msg, sizeof(m_orig_msg), "%s : %s", m_szName, m_szChat) + 1; // іО №®АЪ ЖчЗФ

		if (m_len_orig_msg < 0 || m_len_orig_msg >= (int) sizeof(m_orig_msg))
			m_len_orig_msg = sizeof(m_orig_msg) - 1;

		m_len_conv_msg = snprintf(m_conv_msg, sizeof(m_conv_msg), "??? : %s", m_szChat) + 1; // іО №®АЪ №МЖчЗФ

		if (m_len_conv_msg < 0 || m_len_conv_msg >= (int) sizeof(m_conv_msg))
			m_len_conv_msg = sizeof(m_conv_msg) - 1;

		ConvertEmpireText(m_bEmpire, m_conv_msg + 6, m_len_conv_msg - 6, 10); // 6Ає "??? : "АЗ ±жАМ
	}
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (0!=processReturn)
	{
		...
		{
			if (3==processReturn) //±іИЇБЯ
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґЩёҐ °Е·ЎБЯ(Гў°н,±іИЇ,»уБЎ)їЎґВ °іАО»уБЎА» »зїлЗТ јц ѕшЅАґПґЩ."), pTable->szLocaleName);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%sАМ ЗКїдЗХґПґЩ."), pTable->szLocaleName);
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("їЬДЎ±вґВ ·№є§ %d АМ»уёё »зїл °ЎґЙ ЗХґПґЩ."), SHOUT_LIMIT_LEVEL);
			return (iExtraLen);
		}
	...
	switch (pinfo->type)
	{
		...
		case CHAT_TYPE_PARTY:
			{
				if (!ch->GetParty())
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЖДЖј БЯАМ ѕЖґХґПґЩ."));
				else
				{
					TEMP_BUFFER tbuf;
		...
		case CHAT_TYPE_GUILD:
			{
				if (!ch->GetGuild())
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±жµеїЎ °ЎАФЗПБц ѕКѕТЅАґПґЩ."));
				else
...
void CInputMain::ItemDrop(LPCHARACTER ch, const char * data)
{
	...
	// ї¤Е©°Ў 0єёґЩ Е©ёй ї¤Е©ё¦ №цё®ґВ °Н АМґЩ.
	if (pinfo->gold > 0)
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_VID:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»уґл№жАМ ёЮЅЕБ® ГЯ°Ў °ЕєО »уЕВАФґПґЩ."));
					return sizeof(TPacketCGMessengerAddByVID);
				}
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ёЮЅЕБ®> їоїµАЪґВ ёЮЅЕБ®їЎ ГЯ°ЎЗТ јц ѕшЅАґПґЩ."));
					return sizeof(TPacketCGMessengerAddByVID);
				}

				if (ch->GetDesc() == d) // АЪЅЕАє ГЯ°ЎЗТ јц ѕшґЩ.
					return sizeof(TPacketCGMessengerAddByVID);
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_NAME:
			{
				...
				if (!tch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ґФАє БўјУµЗ АЦБц ѕКЅАґПґЩ."), name);
				else
				{
					if (tch == ch) // АЪЅЕАє ГЯ°ЎЗТ јц ѕшґЩ.
						return CHARACTER_NAME_MAX_LEN;
					...
					{
						// ёЮЅЕАъ°Ў ДіёЇЕНґЬА§°Ў µЗёйј­ єЇ°ж
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
	{
		...
		{
			to_ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°Е·Ў ИД %dГК АМі»їЎ Гў°нё¦ ї­јц ѕшЅАґПґЩ."), g_nPortalLimitTime);
			return;
		}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°Е·Ў ИД %dГК АМі»їЎ Гў°нё¦ ї­јц ѕшЅАґПґЩ."), g_nPortalLimitTime);
		return;
	}
	...
	switch (pinfo->sub_header)
	{
		case EXCHANGE_SUBHEADER_CG_START:	// arg1 == vid of target character
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±єБЦїНґВ °Е·Ўё¦ ЗТјц°Ў ѕшЅАґПґЩ"), g_nPortalLimitTime);
						return;
					}
					//END_MONARCH_LIMIT
					*/
					if (iPulse - ch->GetSafeboxLoadTime() < PASSES_PER_SEC(g_nPortalLimitTime))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Гў°нё¦ ї¬ИД %dГК АМі»їЎґВ °Е·Ўё¦ ЗТјц ѕшЅАґПґЩ."), g_nPortalLimitTime);
					...
					if (iPulse - to_ch->GetSafeboxLoadTime() < PASSES_PER_SEC(g_nPortalLimitTime))
					{
						to_ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Гў°нё¦ ї¬ИД %dГК АМі»їЎґВ °Е·Ўё¦ ЗТјц ѕшЅАґПґЩ."), g_nPortalLimitTime);
					...
					{	
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ѕЧјц°Ў 20ѕп іЙА» ГК°ъЗПї© °Е·Ўё¦ ЗТјц°Ў ѕшЅАґПґЩ.."));
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґЩёҐ °Е·ЎБЯАП°жїм °іАО»уБЎА» ї­јц°Ў ѕшЅАґПґЩ."));
						return;
					}
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»уґл№жАЗ ГС±ЭѕЧАМ 20ѕп іЙА» ГК°ъЗПї© °Е·Ўё¦ ЗТјц°Ў ѕшЅАґПґЩ.."));
...
{
	//	БЧ°ЕіЄ ±вАэ »уЕВїЎј­ґВ °ш°ЭЗТ јц ѕшАё№З·О, skipЗСґЩ.
	//	АМ·ё°Ф ЗПБц ё»°н, CHRACTER::CanMove()їЎ 
	//	if (IsStun() || IsDead()) return false;
	//	ё¦ ГЯ°ЎЗПґВ°Ф ёВґЩ°н »э°ўЗПіЄ,
	//	АМ№М ґЩёҐ єОєРїЎј­ CanMove()ґВ IsStun(), IsDead()°ъ
	//	µ¶ёіАыАё·О ГјЕ©ЗП°н АЦ±в ¶§№®їЎ јцБ¤їЎ АЗЗС їµЗвА»
	//	ГЦјТИ­ЗП±в А§ЗШ АМ·ё°Ф ¶«»§ ДЪµеё¦ ЅбіхґВґЩ.
	if (ch->IsStun() || ch->IsDead())
		return false;
	int ComboInterval = dwTime - ch->GetLastComboTime();
	int HackScalar = 0; // ±вє» ЅєД®¶у ґЬА§ 1
#if 0	
	sys_log(0, "COMBO: %s arg:%u seq:%u delta:%d checkspeedhack:%d",
			ch->GetName(), bArg, ch->GetComboSequence(), ComboInterval - ch->GetValidComboInterval(), CheckSpeedHack);
#endif
	// bArg 14 ~ 21№ш ±оБц ГС 8ДЮєё °ЎґЙ
	// 1. Г№ ДЮєё(14)ґВ АПБ¤ ЅГ°Ј АМИДїЎ №Эє№ °ЎґЙ
	// 2. 15 ~ 21№шАє №Эє№ єТ°ЎґЙ
	// 3. Вч·Кґл·О Бх°ЎЗСґЩ.
	if (bArg == 14)
	{
		if (CheckSpeedHack && ComboInterval > 0 && ComboInterval < ch->GetValidComboInterval() - COMBO_HACK_ALLOWABLE_MS)
		{
			// FIXME Г№№шВ° ДЮєёґВ АМ»уЗП°Ф »Ўё® їГ јц°Ў АЦѕој­ 300Аё·О іЄґ® -_-;
			// ґЩјцАЗ ёуЅєЕНїЎ АЗЗШ ґЩїоµЗґВ »уИІїЎј­ °ш°ЭА» ЗПёй
			// Г№№шВ° ДЮєё°Ў ёЕїм АыАє АОЕН№ъ·О µйѕоїАґВ »уИІ №Я»э.
			// АМ·О АОЗШ ДЮєёЗЩАё·О ЖЁ±вґВ °жїм°Ў АЦѕо ґЩАЅ ДЪµе єс И°јєИ­.
	...
	else if (bArg == 13) // ±вє» °ш°Э (µР°©(Polymorph)ЗЯА» ¶§ їВґЩ)
	{
		if (CheckSpeedHack && ComboInterval > 0 && ComboInterval < ch->GetValidComboInterval() - COMBO_HACK_ALLOWABLE_MS)
		{
			// ґЩјцАЗ ёуЅєЕНїЎ АЗЗШ ґЩїоµЗґВ »уИІїЎј­ °ш°ЭА» ЗПёй
		...
		if (ch->GetRaceNum() >= MAIN_RACE_MAX_NUM)
		{
			...
			{
				// Б¤»уАы °и»кАМ¶уёй 1000.fё¦ °цЗШѕЯ ЗПБцёё Е¬¶уАМѕрЖ®°Ў ѕЦґПёЮАМјЗ јУµµАЗ 90%їЎј­
				// ґЩАЅ ѕЦґПёЮАМјЗ єн·»µщА» ЗгїлЗП№З·О 900.fё¦ °цЗСґЩ.
		...
		{
			// ё»АМ ѕИµЗґВ ДЮєё°Ў їФґЩ ЗШДїАП °ЎґЙјє?
			...
			// А§ ДЪµе·О АОЗШ, Жъё®ёрЗБё¦ ЗЄґВ БЯїЎ °ш°Э ЗПёй,
			// °ЎІы ЗЩАё·О АОЅДЗПґВ °жїм°Ў АЦґЩ.

			// АЪјјИч ё»Зфёй,
			// ј­№цїЎј­ poly 0ё¦ Гіё®ЗЯБцёё,
			// Е¬¶уїЎј­ ±Ч ЖРЕ¶А» №Ю±в АьїЎ, ёчА» °ш°Э. <- Бп, ёчАО »уЕВїЎј­ °ш°Э.
			//
			// ±Ч·Їёй Е¬¶уїЎј­ґВ ј­№цїЎ ёч »уЕВ·О °ш°ЭЗЯґЩґВ ДїёЗµеё¦ єёі»°н (arg == 13)
			//
			// ј­№цїЎј­ґВ raceґВ АО°ЈАОµҐ °ш°ЭЗьЕВґВ ёчАО ірАМґЩ! ¶у°н ЗПї© ЗЩГјЕ©ё¦ ЗЯґЩ.

			// »зЅЗ °ш°Э ЖРЕПїЎ ґлЗС °НАє Е¬¶уАМѕрЖ®їЎј­ ЖЗґЬЗШј­ єёіѕ °НАМ ѕЖґП¶у,
			// ј­№цїЎј­ ЖЗґЬЗШѕЯ ЗТ °НАОµҐ... їЦ АМ·ё°Ф ЗШіщА»±о...
			// by rtsummit
		}
	}
	else
	...
	if (HackScalar)
	{
		// ё»їЎ Её°ЕіЄ і»·ИА» ¶§ 1.5ГК°Ј °ш°ЭАє ЗЩАё·О °ЈБЦЗПБц ѕКµЗ °ш°Э·ВАє ѕш°Ф ЗПґВ Гіё®
...
//	if (!test_server)	//2012.05.15 ±иїлїн : ЕЧј·їЎј­ (№«Аы»уЕВ·О) ґЩјц ёуЅєЕН »уґл·О ґЩїоµЗёйј­ °ш°ЭЅГ ДЮєёЗЩАё·О БЧґВ №®Б¦°Ў АЦѕъґЩ.
	{
		...
		if (((false == ch->IsRiding() && fDist > 25) || fDist > 40) && OXEVENT_MAP_INDEX != ch->GetMapIndex())
		{
			if( false == LC_IsEurope() )
			{
				...
				if (warpPos.x == 0 && warpPos.y == 0)
					LogManager::instance().HackLog("Teleport", ch); // єОБ¤И®ЗТ јц АЦАЅ
			}
		...
		//
		// ЅєЗЗµеЗЩ(SPEEDHACK) Check
		//
		DWORD dwCurTime = get_dword_time();
		// ЅГ°ЈА» SyncЗП°н 7ГК ИД єОЕН °Л»зЗСґЩ. (20090702 АМАьїЈ 5ГКїґАЅ)
		...
		if (CheckSpeedHack)
		{
			...
			// ЅГ°ЈАМ ґК°Ф°ЈґЩ. АПґЬ ·О±Чёё ЗШµРґЩ. БшВҐ АМ·± »з¶чµйАМ ё№АєБц ГјЕ©ЗШѕЯЗФ. TODO
			if (iDelta >= 30000)
			{
				sys_log(0, "SPEEDHACK: slow timer name %s delta %d", ch->GetName(), iDelta);
				ch->GetDesc()->DelayedDisconnect(3);
			}
			// 1ГКїЎ 20msec »Ўё® °ЎґВ°Е ±оБцґВ АМЗШЗСґЩ.
		...
		//
		// ДЮєёЗЩ №Ч ЅєЗЗµеЗЩ ГјЕ©
		//
		if (pinfo->bFunc == FUNC_COMBO && g_bCheckMultiHack)
		{
			CheckComboHack(ch, pinfo->bArg, pinfo->dwTime, CheckSpeedHack); // ДЮєё ГјЕ©
		}
	}
	...
	if (pinfo->bFunc == FUNC_MOVE)
	{
		...
		ch->SetRotation(pinfo->bRot * 5);	// БЯє№ ДЪµе
		ch->ResetStopTime();				// ""
...
int CInputMain::SyncPosition(LPCHARACTER ch, const char * c_pcData, size_t uiBytes)
{
	...
	for (int i = 0; i < iCount; ++i, ++e)
	{
		...
		// victim°ъАЗ °Её®°Ў 2500 + a АМ»уАМёй ЗЩАё·О °ЈБЦ.
		//	°Её® ВьБ¶ : Е¬¶уАМѕрЖ®АЗ __GetSkillTargetRange, __GetBowRange ЗФјц
		//	2500 : ЅєЕі protoїЎј­ °ЎАе »з°Её®°Ў ±д ЅєЕіАЗ »з°Её®, ¶ЗґВ И°АЗ »з°Её®
		//	a = POINT_BOW_DISTANCE °Є... АОµҐ ЅЗБ¦·О »зїлЗПґВ °ЄАОБцґВ АЯ ёрёЈ°ЪАЅ. ѕЖАМЕЫАМіЄ ЖчјЗ, ЅєЕі, ДщЅєЖ®їЎґВ ѕшґВµҐ...
		//		±Ч·Ўµµ И¤ЅГіЄ ЗПґВ ё¶АЅїЎ №цЖЫ·О »зїлЗТ °вЗШј­ 1000.f ·О µТ...
		if (fDistWithSyncOwner > fLimitDistWithSyncOwner)
		{
			// g_iSyncHackLimitCount№ш ±оБцґВ єББЬ.
		...
		// °°Ає АЇАъё¦ g_lValidSyncInterval ms АМі»їЎ ґЩЅГ SyncPositionЗП·Б°н ЗПёй ЗЩАё·О °ЈБЦ.
...
void CInputMain::ScriptAnswer(LPCHARACTER ch, const void* c_pData)
{
	...
	if (p->answer > 250) // ґЩАЅ №цЖ°їЎ ґлЗС ААґдАё·О їВ ЖРЕ¶АО °жїм
	{
		quest::CQuestManager::Instance().Resume(ch->GetPlayerID());
	}
	else // ј±ЕГ №цЖ°А» °с¶уј­ їВ ЖРЕ¶АО °жїм
	{
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Гў°н> Гў°н·О їЕ±ж јц ѕшґВ ѕЖАМЕЫ АФґПґЩ."));
	    return;
	}

	if (!pkSafebox->IsEmpty(p->bSafePos, pkItem->GetSize()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Гў°н> їЕ±ж јц ѕшґВ А§ДЎАФґПґЩ."));
		return;
	}

	if (pkItem->GetVnum() == UNIQUE_ITEM_SAFEBOX_EXPAND)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Гў°н> АМ ѕЖАМЕЫАє іЦА» јц ѕшЅАґПґЩ."));
		return;
	}
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	// ѕЖАМЕЫ ёфїЎј­ АОєҐАё·О їЕ±вґВ єОєРїЎј­ їлИҐј® ЖЇјц Гіё®
	// (ёфїЎј­ ёёµеґВ ѕЖАМЕЫАє item_protoїЎ Б¤АЗµИґл·О јУјєАМ єЩ±в ¶§№®їЎ,
	//  їлИҐј®АЗ °жїм, АМ Гіё®ё¦ ЗПБц ѕКАёёй јУјєАМ ЗПіЄµµ єЩБц ѕК°Ф µИґЩ.)
	if (pkItem->IsDragonSoul())
	{
		if (bMall)
		{
	...
	else
	{
		...
		if (bMall)
		{
			...
			// 100% И®·ь·О јУјєАМ єЩѕоѕЯ ЗПґВµҐ ѕИ єЩѕоАЦґЩёй »х·О єЩИщґЩ. ...............
...
void CInputMain::PartyInviteAnswer(LPCHARACTER ch, const char * c_pData)
{
	...
	// pInviter °Ў ch їЎ°Ф ЖДЖј їдГ»А» ЗЯѕъґЩ.

	if (!pInviter)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїдГ»А» ЗС ДіёЇЕНё¦ ГЈА»јц ѕшЅАґПґЩ."));
	else if (!p->accept)
...
void CInputMain::PartySetState(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ј­№ц №®Б¦·О ЖДЖј °ь·Г Гіё®ё¦ ЗТ јц ѕшЅАґПґЩ."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ё®ґхёё єЇ°жЗТ јц АЦЅАґПґЩ."));
		return;
	}

	if (!ch->GetParty()->IsMember(p->pid))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> »уЕВё¦ єЇ°жЗП·БґВ »з¶чАМ ЖДЖјїшАМ ѕЖґХґПґЩ."));
		return;
	}
	...
	switch (p->byRole)
	{
		...
		case PARTY_ROLE_DEFENDER:
			...
			/* else
			   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ѕоЕВДї јіБ¤їЎ ЅЗЖРЗПїґЅАґПґЩ.")); */
			break;
...
void CInputMain::PartyRemove(LPCHARACTER ch, const char* c_pData)
{
	...
	if (ch->GetDungeon())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ґшАь ѕИїЎј­ґВ ЖДЖјїЎј­ ГЯ№жЗТ јц ѕшЅАґПґЩ."));
		return;
	}
	...
	if (pParty->GetLeaderPID() == ch->GetPlayerID())
	{
		if (ch->GetDungeon())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ґшБЇі»їЎј­ґВ ЖДЖјїшА» ГЯ№жЗТ јц ѕшЅАґПґЩ."));
		}
		else
		{
			...
			else
			{
				...
				if (B)
				{
					...
					B->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїЎј­ ГЯ№жґзЗПјМЅАґПґЩ."));
					//pParty->Unlink(B);
	...
	else
	{
		...
		if (p->pid == ch->GetPlayerID())
		{
			if (ch->GetDungeon())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ґшБЇі»їЎј­ґВ ЖДЖјё¦ іЄ°Ґ јц ѕшЅАґПґЩ."));
			}
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїЎј­ іЄ°ЎјМЅАґПґЩ."));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ґЩёҐ ЖДЖјїшА» Е»ЕрЅГЕі јц ѕшЅАґПґЩ."));
		}
	}
}
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("АыЗХЗПБц ѕКАє ±жµе АМё§ АФґПґЩ."));
		return;
	}
	...
	if (dwGuildID)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> [%s] ±жµе°Ў »эјєµЗѕъЅАґПґЩ."), cp.name);
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµе »эјєїЎ ЅЗЖРЗПїґЅАґПґЩ."));
}
...
void CInputMain::PartyUseSkill(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖј ±вјъАє ЖДЖјАеёё »зїлЗТ јц АЦЅАґПґЩ."));
		return;
	}
		...
		case PARTY_SKILL_WARP:
			{
				...
				if (pch)
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> јТИЇЗП·БґВ ґл»уА» ГЈА» јц ѕшЅАґПґЩ."));
			}
			break;
	}
}
...
int CInputMain::Guild(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (NULL == pGuild)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеїЎ јУЗШАЦБц ѕКЅАґПґЩ."));
			return SubPacketLen;
		}
	}
	...
	switch (SubHeader)
	{
		case GUILD_SUBHEADER_CG_DEPOSIT_MONEY:
			{
				// by mhh : ±жµеАЪ±ЭАє ґзєР°Ј іЦА» јц ѕшґЩ.
				return SubPacketLen;
				...
				if (gold < 0)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> АЯёшµИ ±ЭѕЧАФґПґЩ."));
					return SubPacketLen;
				}

				if (ch->GetGold() < gold)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> °ЎБц°н АЦґВ µ·АМ єОБ·ЗХґПґЩ."));
					return SubPacketLen;
				}
		...
		case GUILD_SUBHEADER_CG_WITHDRAW_MONEY:
			{
				// by mhh : ±жµеАЪ±ЭАє ґзєР°Ј »¬ јц ѕшґЩ.
				return SubPacketLen;
		...
		case GUILD_SUBHEADER_CG_ADD_MEMBER:
			{
				...
				if (!newmember)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±Ч·ЇЗС »з¶чА» ГЈА» јц ѕшЅАґПґЩ."));
					return SubPacketLen;
				}
				...
				if (LC_IsCanada() == true)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ѕЖБч °ЎАФЗТ јц ѕшґВ ДіёЇЕНАФґПґЩ"));
						return SubPacketLen;
					}
				}
		...
		case GUILD_SUBHEADER_CG_REMOVE_MEMBER:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАь БЯїЎґВ ±жµеїшА» Е»ЕрЅГЕі јц ѕшЅАґПґЩ."));
					return SubPacketLen;
				}
				...
				if (member)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл№жАМ °°Ає ±жµе°Ў ѕЖґХґПґЩ."));
						return SubPacketLen;
					}

					if (!pGuild->HasGradeAuth(m->grade, GUILD_AUTH_REMOVE_MEMBER))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеїшА» °­Б¦ Е»Ер ЅГЕі ±ЗЗСАМ ѕшЅАґПґЩ."));
						return SubPacketLen;
					}
				...
				else
				{
					...
					if (pGuild->RequestRemoveMember(pid))
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеїшА» °­Б¦ Е»Ер ЅГДЧЅАґПґЩ."));
					else
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±Ч·ЇЗС »з¶чА» ГЈА» јц ѕшЅАґПґЩ."));
				}
			}
		...
		case GUILD_SUBHEADER_CG_CHANGE_GRADE_NAME:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> БчА§ АМё§А» єЇ°жЗТ ±ЗЗСАМ ѕшЅАґПґЩ."));
				}
				else if (*c_pData == GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАеАЗ БчА§ АМё§Ає єЇ°жЗТ јц ѕшЅАґПґЩ."));
				}
				else if (!check_name(gradename))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> АыЗХЗПБц ѕКАє БчА§ АМё§ АФґПґЩ."));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_CHANGE_GRADE_AUTHORITY:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> БчА§ ±ЗЗСА» єЇ°жЗТ ±ЗЗСАМ ѕшЅАґПґЩ."));
				}
				else if (*c_pData == GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАеАЗ ±ЗЗСАє єЇ°жЗТ јц ѕшЅАґПґЩ."));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_OFFER:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµе°Ў АМ№М ГЦ°н ·№є§АФґПґЩ."));
				}
				else
				{
					offer /= 100;
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> %uАЗ °жЗиДЎё¦ ЕхАЪЗПїґЅАґПґЩ."), offer);
					}
					else
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> °жЗиДЎ ЕхАЪїЎ ЅЗЖРЗПїґЅАґПґЩ."));
					}
				}
			}
			return SubPacketLen;
		...
		case GUILD_SUBHEADER_CG_CHARGE_GSP:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> µ·АМ єОБ·ЗХґПґЩ."));
					return SubPacketLen;
				}

				if (!pGuild->ChargeSP(ch, offer))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> їлЅЕ·В Иёє№їЎ ЅЗЖРЗПїґЅАґПґЩ."));
				}
			}
			return SubPacketLen;
		...
		case GUILD_SUBHEADER_CG_POST_COMMENT:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> °шБц±ЫА» АЫјєЗТ ±ЗЗСАМ ѕшЅАґПґЩ."));
				}
				else
				{
		...
		case GUILD_SUBHEADER_CG_CHANGE_MEMBER_GRADE:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> БчА§ё¦ єЇ°жЗТ ±ЗЗСАМ ѕшЅАґПґЩ."));
				else if (ch->GetPlayerID() == pid)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАеАЗ БчА§ґВ єЇ°жЗТ јц ѕшЅАґПґЩ."));
				else if (grade == 1)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАеАё·О БчА§ё¦ єЇ°жЗТ јц ѕшЅАґПґЩ."));
				else
		...
		case GUILD_SUBHEADER_CG_CHANGE_MEMBER_GENERAL:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> Ае±єА» БцБ¤ЗТ ±ЗЗСАМ ѕшЅАґПґЩ."));
				}
				else
				{
					if (!pGuild->ChangeMemberGeneral(pid, is_general))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ґхАМ»у Аејцё¦ БцБ¤ЗТ јц ѕшЅАґПґЩ."));
					}
				}
			}
			return SubPacketLen;
...
void CInputMain::ItemGive(LPCHARACTER ch, const char* c_pData)
{
	...
	if (to_ch)
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ѕЖАМЕЫА» °ЗіЧБЩ јц ѕшЅАґПґЩ."));
}
...
void CInputMain::Hack(LPCHARACTER ch, const char * c_pData)
{
	...
	// ЗцАз Е¬¶уАМѕрЖ®їЎј­ АМ ЖРЕ¶А» єёі»ґВ °жїм°Ў ѕшАё№З·О №«Б¶°З Ічµµ·П ЗСґЩ
	ch->GetDesc()->SetPhase(PHASE_CLOSE);
}
...
int CInputMain::MyShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	if (ch->GetGold() >= GOLD_MAX)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("јТАЇ µ·АМ 20ѕпіЙА» іСѕо °Е·Ўё¦ ЗЫјц°Ў ѕшЅАґПґЩ."));
...
void CInputMain::Refine(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("Гў°н,°Е·ЎГўµоАМ ї­ё° »уЕВїЎј­ґВ °і·®А» ЗТјц°Ў ѕшЅАґПґЩ"));
		ch->ClearRefineMode();
		return;
	}
	...
	else if (p->type == REFINE_TYPE_MONEY_ONLY)
	{
		...
		if (NULL != item)
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "»з±Н Еёїц їП·б єё»уАє ЗС№ш±оБц »зїл°ЎґЙЗХґПґЩ.");
				}
			}
		}
	}
		...
		case HEADER_CG_MOVE:
			...
			if (LC_IsEurope())
			{
				if (g_bCheckClientVersion)
				{
					...
					if (version > date)
					{
						ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Е¬¶уАМѕрЖ® №цАьАМ ЖІ·Б ·О±ЧѕЖїф µЛґПґЩ. Б¤»уАыАё·О ЖРДЎ ИД БўјУЗПјјїд."));
