...
class MessengerManager : public singleton<MessengerManager>
{
	...
	public:
		...
		void	__AddToList(const std::string& account, const std::string& companion);	// ЅЗБ¦ m_Relation, m_InverseRelation јцБ¤ЗПґВ ёЮјТµе
		void	AddToList(const std::string& account, const std::string& companion);

		void	__RemoveFromList(const std::string& account, const std::string& companion); // ЅЗБ¦ m_Relation, m_InverseRelation јцБ¤ЗПґВ ёЮјТµе
