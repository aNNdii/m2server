...
namespace mining
{
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°о±ЄАМ јц·Гµµ°Ў ГЦґл(%d)°Ў µЗѕъЅАґПґЩ."), Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°о±ЄАМ°Ў ГЦґл јц·ГµµїЎ µµґЮЗПїґЅАґПґЩ."));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("іЄ№«ІЫё¦ ЕлЗШ ґЩАЅ ·№є§АЗ °о±ЄАМ·О ѕч±Ч·№АМµе ЗТ јц АЦЅАґПґЩ."));
			}
			else
			{
				Pick_IncCurExp(pick);	

				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°о±ЄАМАЗ јц·Гµµ°Ў Бх°ЎЗПїґЅАґПґЩ! (%d/%d)"),
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°о±ЄАМё¦ µй°н АЦБц ѕКѕЖј­ Д¶ јц ѕшЅАґПґЩ."));
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґхАМ»у Дііѕ јц ѕшЅАґПґЩ."));
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Г¤±¤їЎ јє°шЗПїґЅАґПґЩ."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Г¤±¤їЎ ЅЗЖРЗПїґЅАґПґЩ."));
		}
