...
bool timed_event_cancel(LPCHARACTER ch)
{
	...
	/* RECALL_DELAY
	   ВчИД АьЕх·О АОЗШ ±НИЇєО µф·№АМ°Ў ГлјТ µЗѕоѕЯ ЗТ °жїм БЦј® ЗШБ¦
...
// ЅЗБ¦ GET_BATTLE_VICTIMА» NULL·О ёёµй°н АМєҐЖ®ё¦ ДµЅЅ ЅГЕІґЩ.
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	//[ mob -> PC ] їшјТ јУјє №жѕо Аыїл
	//2013/01/17
	//ёуЅєЕН јУјє°ш°Э µҐ№МБцАЗ 30%їЎ ЗШґзЗПґВ јцДЎїЎёё АъЗЧАМ АыїлµК.
	if (pkAttacker->IsNPC() && pkVictim->IsPC())
	{
...
int battle_hit(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int & iRetDam)
{
	...
	//ГЦБѕАыАО µҐ№МБц єёБ¤. (2011ів 2їщ ЗцАз ґлїХ°Е№МїЎ°Фёё Аыїл.)
