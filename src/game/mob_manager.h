...
{
	DWORD	dwTiming; // ЅєЕі »зїл ИД ЅЗБ¦·О µҐ№МБц ёФИъ¶§±оБц ±вґЩё®ґВ ЅГ°Ј (ms)
	DWORD	dwHitDistance; // ЅєЕі »зїлЅГ ЅЗБ¦·О ЅєЕі °и»кАМ µЗґВ °Её® (Аь№ж ёоcm)
...
class CMobInstance
{
	public:
		...
		PIXEL_POSITION	m_posLastAttacked;	// ё¶Бцё· ёВАє А§ДЎ
		DWORD		m_dwLastAttackedTime;	// ё¶Бцё· ёВАє ЅГ°Ј
...
class CMobGroupGroup
{
	public:
		...
		DWORD GetMember()
		{
			...
			std::vector<int>::iterator it = lower_bound(m_vec_iProbs.begin(), m_vec_iProbs.end(), n);
