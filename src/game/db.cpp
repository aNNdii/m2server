		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					else
					{
						if (LC_IsEurope())
						{
							//stBlockData >= 0 == іЇВҐ°Ў BlockDate єёґЩ №М·Ў 
				...
				if (ch)
				{
					...
					else
					{
						...
						if (pkItem)
						{
							...
							//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ѕЖАМЕЫ И№µж: %s"), pkItem->GetName());
...
void VCardUse(LPCHARACTER CardOwner, LPCHARACTER CardTaker, LPITEM item)
{
	...
	CardTaker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%dєРАЗ °бБ¦ЅГ°ЈАМ ГЯ°Ў µЗѕъЅАґПґЩ. (°бБ¦№шИЈ %d)"), item->GetSocket(1) / 60, item->GetSocket(0));
