...
struct FHealMyEmpire 
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±єБЦ°Ў і»ё° Гає№Аё·О ёрµз їЎіКБц°Ў °Ўµж Г¤їцБэґПґЩ"));
...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,DWORD price)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d ГК ИДїЎ ±єБЦАЗ Гає№А» »зїлЗТ јц АЦЅАґПґЩ"), ch->GetMCLTime(CHARACTER::MI_HEAL));
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±№°нїЎ µ·АМ єОБ·ЗХґПґЩ. ЗцАз : %u ЗКїд±ЭѕЧ : %u"), GetMoney(Empire), price);
		return 0;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[TEST_ONLY]ЗцАз ±№°н : %d "), GetMoney(Empire) - price);
	return 1;
}
...
bool CMonarch::SendtoDBDecMoney(int Money, BYTE bEmpire, LPCHARACTER ch)
{
	...
	// ЅЗБ¦ БЩАМґВ єОєРАє ј­№цїЎ °¬ґЩ їВ ґЩАЅїЎ Гіё®µИґЩ
	int nEmpire = bEmpire;	
