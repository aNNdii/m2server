...
enhance_strlcpymt(pck.szName, info->szTargetDesc, sizeof(pck.szName));
		...
		int iSendFlag)
{
	...
	ListEventMap::const_iterator it = m_map_kListEvent.find(dwPID);
	...
	if (it != m_map_kListEvent.end())
	{
		...
		while (it2 != it->second.end())
		{
			...
			if (existInfo->dwQuestIndex == dwQuestIndex && !strcmp(existInfo->szTargetName, c_pszTargetName))
			{
				...
				if (c_pszTargetDesc)
				{
					enhance_strlcpymt(existInfo->szTargetDesc, c_pszTargetDesc, sizeof(existInfo->szTargetDesc));
				}
				else
				{
	...
	if (c_pszTargetDesc)
	{
		enhance_strlcpymt(newInfo->szTargetDesc, c_pszTargetDesc, sizeof(newInfo->szTargetDesc));
	}
	else
	{
	...
	enhance_strlcpymt(newInfo->szTargetName, c_pszTargetName, sizeof(newInfo->szTargetName));
	newInfo->iType = iType;
	newInfo->iArg1 = iArg1;
	newInfo->iArg2 = iArg2;
...
{
	ListEventMap::iterator it = m_map_kListEvent.find(dwPID);
