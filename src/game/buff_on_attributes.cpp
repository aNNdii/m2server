...
void CBuffOnAttributes::RemoveBuffFromItem(LPITEM pItem)
{
	...
	if (NULL != pItem)
	{
		...
		for (int j = 0; j < m; j++)
		{
			...
			// m_map_additional_attrsїЎј­ ЗШґз attribute typeїЎ ґлЗС °ЄА» Б¦°ЕЗП°н,
			// єЇ°жµИ °ЄАЗ (m_bBuffValue)%ёёЕ­АЗ №цЗБ Иї°ъ °ЁјТ
...
void CBuffOnAttributes::AddBuffFromItem(LPITEM pItem)
{
	...
	if (NULL != pItem)
	{
		...
		for (int j = 0; j < m; j++)
		{
			...
			// m_map_additional_attrsїЎј­ ЗШґз attribute typeїЎ ґлЗС °ЄАМ ѕшґЩёй ГЯ°Ў.
			// ГЯ°ЎµИ °ЄАЗ (m_bBuffValue)%ёёЕ­АЗ №цЗБ Иї°ъ ГЯ°Ў
			if (it == m_map_additional_attrs.end())
			{
			...
			// m_map_additional_attrsїЎј­ ЗШґз attribute typeїЎ ґлЗС °ЄАМ АЦґЩёй, ±Ч °ЄА» Бх°ЎЅГЕ°°н,
			// єЇ°жµИ °ЄАЗ (m_bBuffValue)%ёёЕ­АЗ №цЗБ Иї°ъ ГЯ°Ў
			else
			{
				int& sum_of_attr_value = it->second;
				int old_value = sum_of_attr_value * m_bBuffValue / 100;
...
void CBuffOnAttributes::ChangeBuffValue(BYTE bNewValue)
{
	...
	{
		// ±вБёїЎ, m_map_additional_attrsАЗ °ЄАЗ (m_bBuffValue)%ёёЕ­АМ №цЗБ·О µйѕо°Ў АЦѕъАё№З·О,
		// (bNewValue)%ёёЕ­Аё·О °ЄА» єЇ°жЗФ.
