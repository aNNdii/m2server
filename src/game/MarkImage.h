...
struct SGuildMark
{
	...
	///////////////////////////////////////////////////////////////////////////////
	Pixel m_apxBuf[SIZE];	// ЅЗБ¦ АМ№МБц
...
struct SGuildMarkBlock
{
	...
	BYTE 	m_abCompBuf[MAX_COMP_SIZE];	// ѕРГаµИ µҐАМЕН
	lzo_uint m_sizeCompBuf;	// ѕРГаµИ Е©±в
	DWORD	m_crc;			// ѕРГаµИ µҐАМЕНАЗ CRC
...
class CGuildMarkImage
{
	public:
		...
		bool SaveBlockFromCompressedData(DWORD posBlock, const BYTE * pbComp, DWORD dwCompSize); // ј­№ц -> Е¬¶уАМѕрЖ®

		DWORD GetEmptyPosition(); // єу ё¶Е© А§ДЎё¦ ѕтґВґЩ.
