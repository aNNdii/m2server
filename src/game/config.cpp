...
int		save_event_second_cycle = passes_per_sec * 120;	// 3єР
...
//ЅГѕЯ = VIEW_RANGE + VIEW_BONUS_RANGE
//VIEW_BONUSE_RANGE : Е¬¶уАМѕрЖ®їН ЅГѕЯ Гіё®їЎј­іК№« µь ¶іѕоБъ°жїм №®Б¦°Ў №Я»эЗТјцАЦѕо 500CMАЗ ї©єРА» ЗЧ»уБШґЩ.
int VIEW_RANGE = 5000;
...
unsigned int g_uiSpamBlockDuration = 60 * 15; // ±вє» 15єР
unsigned int g_uiSpamBlockScore = 100; // ±вє» 100БЎ
unsigned int g_uiSpamReloadCycle = 60 * 10; // ±вє» 10єР
...
bool            g_protectNormalPlayer   = false;        // №ь№эАЪ°Ў "ЖтИ­ёрµе" АО АП№ЭАЇАъё¦ °ш°ЭЗПБц ёшЗФ
bool            g_noticeBattleZone      = false;        // БЯёіБцґлїЎ АФАеЗПёй ѕИі»ёЮјјБцё¦ ѕЛ·ББЬ
...
void config_init(const string& st_localeServiceName)
{
	...
	// ... ѕЖ... db_portґВ АМ№М АЦґВµҐ... іЧАМ№Ц ѕоВоЗШѕЯЗФ...
	int mysql_db_port[2];
	...
	*log_db = '\0';
	...
	// DBїЎј­ ·ОДЙАПБ¤єёё¦ јјЖГЗП±вА§ЗШј­ґВ ґЩёҐ јјЖГ°ЄєёґЩ ј±ЗаµЗѕој­
	// DBБ¤єёёё АРѕоїН ·ОДЙАП јјЖГА» ЗСИД ґЩёҐ јјЖГА» АыїлЅГДСѕЯЗСґЩ.
	// АМАЇґВ ·ОДЙАП°ь·ГµИ ГК±вИ­ ·зЖѕАМ °ч°чїЎ БёАзЗП±в ¶§№®.
	...
	// Common DB °Ў Locale Б¤єёё¦ °ЎБц°н АЦ±в ¶§№®їЎ °ЎАе ёХАъ БўјУЗШѕЯ ЗСґЩ.
	...
	// ·ОДЙАП Б¤єёё¦ °ЎБ®їААЪ 
	// <°ж°н> Дхё®№®їЎ Аэґл Б¶°З№®(WHERE) ґЮБц ё¶јјїд. (ґЩёҐ БцїЄїЎј­ №®Б¦°Ў »э±жјц АЦЅАґПґЩ)
	{
		char szQuery[512];
	...
	// ·ОДЙАП Б¤єёё¦ COMMON SQLїЎ јјЖГЗШБШґЩ.
	// Вь°н·О g_stLocale Б¤єёґВ LocaleService_Init() і»єОїЎј­ јјЖГµИґЩ.
	...
	if (false == g_bAuthServer) // АОБх ј­№ц°Ў ѕЖґТ °жїм
	{
		// Log DB БўјУ
	...
	// ЅєЖ®ёµ єс±іАЗ №®Б¦·О АОЗШј­ AccountDB::instance().SetLocale(g_stLocale) ИДєОЕН ЗСґЩ.
	// №°·Р ±№і»ґВ є°·О №®Б¦°Ў ѕИµИґЩ(ЗШїЬ°Ў №®Б¦)
	{
		char szQuery[256];
		...
		for (int job = 0; job < JOB_MAX_NUM * 2; ++job)
		{
			...
			// јјЖГАМ ѕИµЗѕоАЦАёёй ±вє»ЕЧАМєнА» »зїлЗСґЩ.
			if (pMsg->Get()->uiNumRows == 0)
			{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("spam_block_reload_cycle")
		{
			...
			g_uiSpamReloadCycle = MAX(60, g_uiSpamReloadCycle); // ГЦјТ 1єР
		}
...
void CheckClientVersion()
{
	...
	while (it != set.end())
	{
		...
		if (version > date)
		{
			d->GetCharacter()->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Е¬¶уАМѕрЖ® №цАьАМ ЖІ·Б ·О±ЧѕЖїф µЛґПґЩ. Б¤»уАыАё·О ЖРДЎ ИД БўјУЗПјјїд."));
