...
/*
   int iMinLevel;	// ЕѕЅВЗТ јц АЦґВ ГЦјТ ·№є§
   int iNPCRace;
   int iMaxHealth;	// ё»АЗ ГЦґл Гј·В
   int iMaxStamina;	// ё»АЗ ГЦґл ЅєЕЧ№МіК
   int iST;
   int iDX;
   int iHT;
   int iIQ;
 ...
 */
{
	...
	{ 25,	20101,	3,	4,	26,	35,	18,	9,	54,	43,	64,	32 },	// 1 (ГК±Ю)
	...
	{ 35,	20104,	18,	30,	40,	53,	27,	13,	69,	55,	82,	41 },	// 11 (БЯ±Ю)
	...
	{ 50,	20107,	35,	120,	53,	71,	36,	18,	84,	67,	100,	50 },	// 21 (°н±Ю)
...
bool CHorseRider::ReviveHorse()
{
	...
	// 2005.03.24.ipkn.ё» »мё°ИД ґЩЅГ БЧґВ Зц»у јцБ¤
...
void CHorseRider::FeedHorse()
{
	...
	{
		UpdateHorseHealth(+1);
		// 20050324. ipkn ё» ёФїґА»¶§µµ Гј·В °ЁјТ µф·№АМё¦ ґГё°ґЩ.
		ResetHorseHealthDropTime();
	}
}
...
void CHorseRider::UpdateHorseDataByLogoff(DWORD dwLogoffTime)
{
	...
	if (dwLogoffTime >= 12 * 60)
		UpdateHorseStamina(dwLogoffTime / 12 / 60, false); // ·О±ЧїАЗБ 12єРґз 1ѕї Иёє№
}
