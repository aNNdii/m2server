	...
	else
	{
		if (pkKiller->GetJob() == JOB_SHAMAN || (pkKiller->GetJob() == JOB_SURA && pkKiller->GetSkillGroup() == 2))
		{
			...
			if (bAttacking)
				...
				iAmount = 10 + GetMaxSP() * 3 / 100; // Жт»уЅГ
		...
		else
		{
			...
			else
			{
				...
				if (pkKiller->GetHP() < pkKiller->GetMaxHP())
					iAmount = 2 + (pkKiller->GetMaxSP() / 100); // ЗЗ ґЩ ѕИГЎА»¶§
				else
					iAmount = 9 + (pkKiller->GetMaxSP() / 100); // ±вє»
			}
...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("їлЅЕАЗ °ЎИЈ·О °жЗиДЎ°Ў ¶іѕоБцБц ѕКѕТЅАґПґЩ."));
		return;
	}
	...
	if (IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_DEATH_PENALTY))
	{
		...
		if (LC_IsYMIR()) // Гµё¶ №цАьїЎј­ґВ ѕрБ¦іЄ їлЅЕАЗ °ЎИЈ ѕЖАМЕЫА» ГјЕ©ЗСґЩ.
		{
		...
		else if (!bTown) // ±№Б¦ №цАьїЎј­ґВ Б¦АЪё® єОИ°ЅГёё їлЅЕАЗ °ЎИЈё¦ »зїлЗСґЩ. (ё¶А» є№±НЅГґВ °жЗиДЎ ЖРіОЖј ѕшАЅ)
		{
			if (FindAffect(AFFECT_NO_DEATH_PENALTY))
			{
				sys_log(0, "NO_DEATH_PENALTY_AFFECT(%s)", GetName());
...
void CHARACTER::Stun()
{
	...
	event_cancel(&m_pkRecoveryEvent); // Иёє№ АМєҐЖ®ё¦ БЧАОґЩ.
...
void CHARACTER::RewardGold(LPCHARACTER pkAttacker)
{
	...
	if (1 == number(1, 50000)) // 1/50000 И®·ь·О µ·АМ 10№и
		iGoldMultipler *= 10;
	else if (1 == number(1, 10000)) // 1/10000 И®·ь·О µ·АМ 5№и
		iGoldMultipler *= 5;
	...
	// MOB_RANK°Ў BOSSєёґЩ іфАёёй №«Б¶°З µ·ЖшЕє
	if (GetMobRank() >= MOB_RANK_BOSS && !IsStone() && GetMobTable().dwGoldMax != 0)
	{
		if (1 == number(1, iGold10DropPct))
			iGoldMultipler *= 10; // 1% И®·ь·О µ· 10№и
		...
		for (int i = 0; i < iSplitCount; ++i)
		{
			...
			// NOTE: µ· ЖшЕєАє Б¦ 3АЗ јХ Гіё®ё¦ ЗПБц ѕКАЅ
	...
	// 1% И®·ь·О µ·А» 10°і ¶іѕо ¶Яё°ґЩ. (10№и µе·УАУ)
...
{
	// °іАО»уБЎА» ї¬ »уЕВїЎј­ґВ ѕЖАМЕЫА» µе·УЗПБц ѕКґВґЩ.
	if (GetMyShop())
		return;
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	m_dwKillerPID = 0; // №ЭµеЅГ ГК±вИ­ ЗШѕЯЗФ DO NOT DELETE THIS LINE UNLESS YOU ARE 1000000% SURE
	...
	if (pkKiller && IsPC())
	{
		...
		else
		{
			...
			if (GetEmpire() != pkKiller->GetEmpire())
			{
				...
				{
					// TODO : АФ±ё·О іЇё®ґВ ДЪµеё¦ іЦѕоѕЯ ЗСґЩ.
				}
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("їлЅЕАЗ єёИЈ·О ѕЖАМЕЫАМ ¶іѕоБцБц ѕКѕТЅАґПґЩ."));
					else
					{
	...
	event_cancel(&m_pkStunEvent); // ±вАэ АМєҐЖ®ґВ БЧАОґЩ.
	...
	{
		// °ЎµеїЎ°Ф °ш°Э№ЮАє ёуЅєЕНґВ єё»уАМ ѕшѕоѕЯ ЗСґЩ.
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			{
				// єОИ°ЗПґВ ёуЅєЕНґВ єё»уА» БЦБц ѕКґВґЩ.
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАьБЯїЎґВ »зіЙїЎ µыёҐ АМАНАМ ѕшЅАґПґЩ."));
				}
			}
		}
	}
	...
	if (GetDesc() != NULL) {
		//
		// Е¬¶уАМѕрЖ®їЎ їЎЖеЖ® ЖРЕ¶А» ґЩЅГ єёіЅґЩ.
		//
	...
	// Dead АМєҐЖ® »эјє,
	//
	// Dead АМєҐЖ®їЎј­ґВ ёуЅєЕНАЗ °жїм ёоГК ИДїЎ Destroy µЗµµ·П ЗШБЦёз,
	// PCАЗ °жїм 3єР АЦґЩ°Ў ё¶А»їЎј­ іЄїАµµ·П ЗШ БШґЩ. 3єР і»їЎґВ АЇАъ·ОєОЕН
	// ё¶А»їЎј­ ЅГАЫЗТ °ЗБц, ї©±вј­ ЅГАЫЗТ °ЗБц °бБ¤А» №ЮґВґЩ.
	if (isDuel == false)
	{
		if (m_pkDeadEvent)
		{
...
//
// CHARACTER::Damage ёЮјТµеґВ this°Ў µҐ№МБцё¦ АФ°Ф ЗСґЩ.
//
// Arguments
//    pAttacker		: °ш°ЭАЪ
//    dam		: µҐ№МБц
//    EDamageType	: ѕо¶І ЗьЅДАЗ °ш°ЭАО°Ў?
//    
// Return value
//    true		: dead
//    false		: not dead yet
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	//
	// ё¶№эЗь ЅєЕі°ъ, ·№АОБцЗь ЅєЕіАє(±ГАЪ°ґ) Е©ё®ЖјДГ°ъ, °ьЕл°ш°Э °и»кА» ЗСґЩ.
	// їш·ЎґВ ЗПБц ѕКѕЖѕЯ ЗПґВµҐ Nerf(ґЩїо№л·±Ѕє)ЖРДЎё¦ ЗТ јц ѕшѕој­ Е©ё®ЖјДГ°ъ
	// °ьЕл°ш°ЭАЗ їш·Ў °ЄА» ѕІБц ѕК°н, /2 АМ»уЗПї© АыїлЗСґЩ.
	// 
	// №«»з АМѕЯ±в°Ў ё№ѕЖј­ №Рё® ЅєЕіµµ ГЯ°Ў
	//
	// 20091109 : №«»з°Ў °б°ъАыАё·О ѕцГ»іЄ°Ф °­ЗШБш °НАё·О °б·РіІ, µ¶АП ±вБШ №«»з єсАІ 70% А°№Ъ
	//
	if (type == DAMAGE_TYPE_MELEE || type == DAMAGE_TYPE_RANGE || type == DAMAGE_TYPE_MAGIC)
	{
		if (pAttacker)
		{
			...
			if (iCriticalPct)
			{
				if (iCriticalPct >= 10) // 10єёґЩ Е©ёй 5% + (4ё¶ґЩ 1%ѕї Бх°Ў), µы¶уј­ јцДЎ°Ў 50АМёй 20%
					iCriticalPct = 5 + (iCriticalPct - 10) / 4;
				else // 10єёґЩ АЫАёёй ґЬјшИч №ЭАё·О ±рАЅ, 10 = 5%
					iCriticalPct /= 2;

				//Е©ё®ЖјДГ АъЗЧ °Є Аыїл.
			...
			if (iPenetratePct)
			{
				...
				if (iPenetratePct >= 10)
				{
					// 10єёґЩ Е©ёй 5% + (4ё¶ґЩ 1%ѕї Бх°Ў), µы¶уј­ јцДЎ°Ў 50АМёй 20%
					iPenetratePct = 5 + (iPenetratePct - 10) / 4;
				}
				else
				{
					// 10єёґЩ АЫАёёй ґЬјшИч №ЭАё·О ±рАЅ, 10 = 5%
					iPenetratePct /= 2;
				}
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°ьЕл ГЯ°Ў µҐ№МБц %d"), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	// 
	// ДЮєё °ш°Э, И° °ш°Э, Бп ЖтЕё АП ¶§ёё јУјє°ЄµйА» °и»кА» ЗСґЩ.
	//
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		if (type == DAMAGE_TYPE_NORMAL)
		{
			// ±ЩБў ЖтЕёАП °жїм ё·А» јц АЦАЅ
			if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s єн·°! (%d%%)"), GetName(), GetPoint(POINT_BLOCK));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s єн·°! (%d%%)"), GetName(), GetPoint(POINT_BLOCK));
				}
		...
		{
			// їш°Её® ЖтЕёАЗ °жїм ЗЗЗТ јц АЦАЅ
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ИёЗЗ! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s ИёЗЗ! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
				}
		...
		if (pAttacker)
		{
			if (type == DAMAGE_TYPE_NORMAL)
			{
				...
				if (GetPoint(POINT_REFLECT_MELEE))
				{
					...
					// NOTE: °ш°ЭАЪ°Ў IMMUNE_REFLECT јУјєА» °®°нАЦґЩёй №Э»зё¦ ѕИ ЗПґВ °Ф 
					// ѕЖґП¶у 1/3 µҐ№МБц·О °нБ¤ЗШј­ µйѕо°Ўµµ·П ±вИ№їЎј­ їдГ».
			...
			// ДҐ ¶§ё¶ґЩ HPИёє№
			if (pAttacker->GetPoint(POINT_HIT_HP_RECOVERY) && number(0, 4) > 0) // 80% И®·ь
			{
			...
			// ДҐ ¶§ё¶ґЩ SPИёє№
			if (pAttacker->GetPoint(POINT_HIT_SP_RECOVERY) && number(0, 4) > 0) // 80% И®·ь
			{
	...
	//
	// ЖтЕё ¶ЗґВ ЅєЕі·О АОЗС єёіКЅє ЗЗЗШ/№жѕо °и»к
	// 
	switch (type)
	{
	...
	if (IsAffectFlag(AFF_MANASHIELD))
	{
		...
		{
			// Б¤ЅЕ·ВАМ ёрАЪ¶уј­ ЗЗ°Ў ґх ±пї©ѕЯЗТ‹љ
	...
	if (pAttacker)
	{
		...
		//
		// Б¦±№Аё·О АОЗС єёіКЅє (ЗС±№ їГµе №цАьёё Аыїл)
		//
		...
		if (pAttacker->IsPC())
		{
			...
			// ґЩёҐ Б¦±№ »з¶чАО °жїм µҐ№МБц 10% °ЁјТ
			if (iEmpire && iMapEmpire && iEmpire != iMapEmpire)
			{
				int percent = 10;
	...
	if (pAttacker)
	{
		// DEATH BLOW : И®·ь АыАё·О 4№и ЗЗЗШ (!? ЗцАз АМєҐЖ®іЄ °шјєАьїл ёуЅєЕНёё »зїлЗФ)
		...
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s[%d]°Ў °ш°Э А§ДЎ: %d %d"), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
	...
	if (pAttacker && dam > 0 && IsNPC())
	{
		...
		StartRecoveryEvent(); // ёуЅєЕНґВ µҐ№МБцё¦ АФАёёй Иёє№А» ЅГАЫЗСґЩ.
		//puRest21.Pop();
...
{
	if (pkKiller->GetDungeon()) // ґшБЇі»їЎј± ёёµО°ЎіЄїАБцѕКґВґЩ
		return;
}
...
*/
...

	{
		...
		// єОєО°Ў ј­·О ЖДЖјБЯАМёй ±ЭЅЅАМ їАёҐґЩ
		if (you)
		{
			// 1ѕпАМ 100%
...
typedef struct SDamageInfo
{
	...
	inline void Distribute(LPCHARACTER ch, int iExp)
	{
		...
		else if (pParty)
		{
			...
			// °жЗиДЎ ёфѕЖБЦ±в (ЖДЖј°Ў И№µжЗС °жЗиДЎё¦ 5% »©ј­ ёХАъ БЬ)
...
LPCHARACTER CHARACTER::DistributeExp()
{
	...
	// АПґЬ БЦА§їЎ ѕшґВ »з¶чА» °Й·Ї іЅґЩ. (50m)
	...
	if (iTotalDam == 0)	// µҐ№МБц БШ°Ф 0АМёй ё®ЕП
		return NULL;

	if (m_pkChrStone)	// µ№АМ АЦА» °жїм °жЗиДЎАЗ №ЭА» µ№їЎ°Ф іС±дґЩ.
	{
	...
	// Б¦АП µҐ№МБцё¦ ё№АМ БШ »з¶чАМ HP Иёє№А» ЗСґЩ.
	DistributeHP(pkChrMostAttacked);	// ёёµО ЅГЅєЕЫ

	{
		// Б¦АП µҐ№МБцё¦ ё№АМ БШ »з¶чАМіЄ ЖДЖј°Ў ГС °жЗиДЎАЗ 20% + АЪ±в°Ў ¶§ё°ёёЕ­АЗ °жЗиДЎё¦ ёФґВґЩ.
...
LPCHARACTER CHARACTER::GetNearestVictim(LPCHARACTER pkChr)
{
	...
	// АПґЬ БЦА§їЎ ѕшґВ »з¶чА» °Й·Ї іЅґЩ.
	while (it != m_map_kDamage.end())
	{
		const VID & c_VID = it->first;
		++it;
...
void CHARACTER::SetVictim(LPCHARACTER pkVictim)
{
	...
	else
	{
		if (m_kVIDVictim != pkVictim->GetVID())
			MonsterLog("°ш°Э ґл»уА» јіБ¤: %s", pkVictim->GetName());
...
LPCHARACTER CHARACTER::GetProtege() const // єёИЈЗШѕЯ ЗТ ґл»уА» ё®ЕП
{
	if (m_pkChrStone)
		return m_pkChrStone;
...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 50) // АУЅГ·О 50% И®·ь·О АыА» ІшѕоїВґЩ
...
{
	// ЖЇБ¤ °ш°ЭЕёАФїЎ µы¶у ґх їГ¶у°ЈґЩ
	switch (type)
	{
		case DAMAGE_TYPE_NORMAL_RANGE:
			dam = (int) (dam*1.2f);
	...
	// °ш°ЭАЪ°Ў ЗцАз ґл»уАО °жїм єёіКЅєё¦ БШґЩ.
...
{
	if (get_dword_time() - m_dwLastVictimSetTime < 3000) // 3ГКґВ ±вґЩ·БѕЯЗСґЩ
		return;
