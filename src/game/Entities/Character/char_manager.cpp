...
#endif
{
	...
	// ґшАьїЎ јТјУµИ ёуЅєЕНґВ ґшАьїЎј­µµ »иБ¦ЗПµµ·П.
...
{
	// їЦ±ё ЅєЖщЗТБцё»Бцё¦ °бБ¤ЗТ јц АЦ°ФЗФ
	{
	...
	// ЗШЕВё¦ ЅєЖщЗТБц ё»Бцё¦ °бБ¤ЗТ јц АЦ°Ф ЗФ
	{
...
LPCHARACTER CHARACTER_MANAGER::SpawnMobRange(DWORD dwVnum, long lMapIndex, int sx, int sy, int ex, int ey, bool bIsException, bool bSpawnMotion, bool bAggressive )
{
	...
	if (pkMob->m_table.bType == CHAR_TYPE_STONE)	// µ№Ає №«Б¶°З SPAWN ёрјЗАМ АЦґЩ.
		bSpawnMotion = true;
...
bool CHARACTER_MANAGER::SpawnMoveGroup(DWORD dwVnum, long lMapIndex, int sx, int sy, int ex, int ey, int tx, int ty, LPREGEN pkRegen, bool bAggressive_)
{
	...
	for (DWORD i = 0; i < c_rdwMembers.size(); ++i)
	{
		...
		if (!tch)
		{
			if (i == 0)	// ёшёёµз ёуЅєЕН°Ў ґлАеАП °жїмїЎґВ ±ЧіЙ ЅЗЖР
				return false;
...
//		for_each_pc(bind2nd(mem_fun(&CHARACTER::UpdateCharacter), iPulse));
	...
	// ЕЧЅєЖ® ј­№цїЎј­ґВ 60ГКё¶ґЩ ДіёЇЕН °іјцё¦ јѕґЩ
...
void CHARACTER_MANAGER::RegisterRaceNumMap(LPCHARACTER ch)
{
	...
	if (m_set_dwRegisteredRaceNum.find(dwVnum) != m_set_dwRegisteredRaceNum.end()) // µо·ПµИ №шИЈ АМёй
	{
...
{
	// Begin АМ ИДїЎ BeginА» ¶З ЗПґВ °жїмїЎ Flush ЗПБц ѕКґВ ±вґЙ БцїшА» А§ЗШ
	// АМ№М ЅГАЫµЗѕоАЦАёёй false ё®ЕП Гіё®
	if (m_bUsePendingDestroy)
		return false;
...
void CHARACTER_MANAGER::FlushPendingDestroy()
{
	...
	m_bUsePendingDestroy = false; // ЗГ·Ў±Чё¦ ёХАъ јіБ¤ЗШѕЯ ЅЗБ¦ Destroy Гіё®°Ў µК
