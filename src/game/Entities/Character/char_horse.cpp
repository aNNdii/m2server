...
bool CHARACTER::StartRiding()
{
	if (IsDead() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ѕІ·ЇБш »уЕВїЎј­ґВ ё»їЎ Е» јц ѕшЅАґПґЩ."));
		return false;
	}
	
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("єЇЅЕ »уЕВїЎј­ґВ ё»їЎ Е» јц ѕшЅАґПґЩ."));
		return false;
	}

	// ЕОЅГµµ АФАє »уЕВАЗ ё» Её±в ±ЭБц
	LPITEM armor = GetWear(WEAR_BODY);

	if (armor && (armor->GetVnum() >= 11901 && armor->GetVnum() <= 11904))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ї№є№А» АФАє »уЕВїЎј­ ё»А» Е» јц ѕшЅАґПґЩ."));
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ё»А» јТАЇЗП°н АЦБц ѕКЅАґПґЩ."));
		else if (GetHorseHealth() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ё»АМ БЧѕоАЦґВ »уЕВ АФґПґЩ."));
		else if (GetHorseStamina() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ё»АЗ ЅєЕЧ№МіК°Ў єОБ·ЗПї© ё»А» Е» јц ѕшЅАґПґЩ."));
...
bool CHARACTER::StopRiding()
{
	if (CHorseRider::StopRiding())
	{
		...
		if (!IsDead() && !IsStun())
		{
			...
			// [NOTE] ё»їЎј­ і»ё± ¶© АЪ±в°Ў ЕАґш°Й јТИЇЗПµµ·П јцБ¤
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		//NOTE : summonЗЯґВµҐ АМ№М horse°Ў АЦАёёй ѕЖ№«°Нµµ ѕИЗСґЩ.
		if( m_chHorse != NULL )
			return;
		...
		if (!m_chHorse)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ё» јТИЇїЎ ЅЗЖРЗПїґЅАґПґЩ."));
			return;
		}

		if (GetHorseHealth() <= 0)
		{
			// БЧАє°ЕГі·і АЦ°Ф ЗПґВ Гіё®
			m_chHorse->SetPosition(POS_DEAD);

			// АПБ¤ЅГ°ЈАЦґЩ »з¶уБц°Ф ЗПАЪ.
		...
		else
		{
			...
			m_chHorse->m_stName += LC_TEXT("ґФАЗ ё»");
		}
...
0:	 ~ <= 10%
				...
				m_bSendHorseStaminaGrade != iStaminaGrade)
		{
			...
			// FIX : Е¬¶уАМѕрЖ®їЎ "ё» »уЕВ №цЗБ" ѕЖАМДЬА» ЗҐЅГЗПБц ѕКА» ёсАыАё·О ЗФјц ГКАФїЎ returnЗФАё·ОЅб ѕЖ·Ў ДЪµеё¦ №«ЅГЗСґЩёй
			// ё»А» №«ЗСґл·О јТИЇЗПґВ №«ЅГ№«ЅГЗС №ц±Ч°Ў »э±и.. Б¤И®ЗС їшАОАє ЖДѕЗ ѕИЗШєБј­ ёрё§.
