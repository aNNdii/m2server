...
ACMD(do_inputall)
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ён·Йѕоё¦ ёрµО АФ·ВЗПјјїд."));
}
...
ACMD (do_full_set);
// Бч±є°ъ ·№є§їЎ µыёҐ ГЦ°н ѕЖАМЕЫ
ACMD (do_item_full_set);
// Бч±єїЎ µыёҐ ГЦ°н їЙјЗАЗ јУјє јВЖГ
ACMD (do_attr_full_set);
// ёрµз ЅєЕі ё¶ЅєЕН
ACMD (do_all_skill_master);
// ѕЖАМЕЫ Вшїл. iconАМ ѕшѕо Е¬¶уїЎј­ И®АО ЗТ јц ѕшґВ ѕЖАМЕЫ ВшїлА» А§ЗШ ёёµк.
ACMD (do_use_item);
ACMD (do_dragon_soul);
ACMD (do_ds_list);
...
{
	{ "!RESERVED!",	NULL,			0,			POS_DEAD,	GM_IMPLEMENTOR	}, /* №ЭµеЅГ АМ °НАМ ГіАЅАМѕоѕЯ ЗСґЩ. */
	...
	{ "mob_ld",		do_mob_ld,			0,			POS_DEAD,	GM_HIGH_WIZARD	}, /* ёчАЗ А§ДЎїН №жЗвА» јіБ¤ЗШ јТИЇ /mob_ld vnum x y dir */
	...
	{ "·О±Чё¦єёї©Ба",	do_detaillog,		0,			POS_DEAD,	GM_LOW_WIZARD	},
	{ "ёуЅєЕНєёї©Ба",	do_monsterlog,		0,			POS_DEAD,	GM_LOW_WIZARD	},
	...
	{ "БЦ»зА§",				do_dice,				0,	POS_DEAD,	GM_PLAYER		},
	...
	{ "\n",		NULL,			0,			POS_DEAD,	GM_IMPLEMENTOR	}  /* №ЭµеЅГ АМ °НАМ ё¶Бцё·АМѕоѕЯ ЗСґЩ. */
};
...
void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	char cmd[128 + 1];  // buffer overflow №®Б¦°Ў »э±вБц ѕКµµ·П АПєО·Ї ±жАМё¦ ВЄ°Ф АвАЅ
	char new_line[256 + 1];
	const char * line;
	int icmd;
	...
	for (icmd = 1; *cmd_info[icmd].command != '\n'; ++icmd)
	{
		...
		{
			if (!strcmp(cmd_info[icmd].command, cmd)) // do_cmdґВ ёрµз ён·Йѕоё¦ ГДѕЯ ЗТ јц АЦґЩ.
				break;
		}
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		...
		{
			case POS_MOUNTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Еє »уЕВїЎј­ґВ ЗТ јц ѕшЅАґПґЩ."));
				break;

			case POS_DEAD:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ѕІ·ЇБш »уЕВїЎј­ґВ ЗТ јц ѕшЅАґПґЩ."));
				break;

			case POS_SLEEPING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ІЮјУїЎј­ ѕо¶»°Фїд?"));
				break;

			case POS_RESTING:
			case POS_SITTING:
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ёХАъ АПѕо іЄјјїд."));
				break;
				/*
				   case POS_FIGHTING:
				   ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ёсјыА» °Й°н АьЕх БЯ АФґПґЩ. БэБЯ ЗПјјїд."));
				   break;
				 */
			default:
	...
	if (strncmp("phase", cmd_info[icmd].command, 5) != 0) // Ичµз ён·Йѕо Гіё® 
