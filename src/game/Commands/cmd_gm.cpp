...
/*
   = {
   { "A1|їµѕИАѕјє",		0, 1,  4693, 9642 },
   { "A3|АЪѕзЗц",		0, 3,  3608, 8776 },

   { "B1|Б¶ѕИАѕјє",		0, 21,  557, 1579 },
   { "B3|є№Б¤Зц",		0, 23, 1385, 2349 },

   { "C1|Жт№«Аѕјє",		0, 41, 9696, 2784 },
   { "C3|№Ъ¶уЗц",		0, 43, 8731, 2426 },

// Snow
{ "Snow|ј­ЗС»к",		1, 61, 4342, 2906 },
{ "Snow|ј­ЗС»к",		2, 61, 3752, 1749 },
{ "Snow|ј­ЗС»к",		3, 61, 4918, 1736 },

// Flame
{ "Flame|µµї°И­Бц|И­ї°",	1, 62, 5994, 7563 },
{ "Flame|µµї°И­Бц|И­ї°",	2, 62, 5978, 6222 },
{ "Flame|µµї°И­Бц|И­ї°",	3, 62, 7307, 6898 },

// Desert
{ "Desert|їµєс»зё·|»зё·",	1, 63, 2178, 6272 },
{ "Desert|їµєс»зё·|»зё·",	2, 63, 2219, 5027 },
{ "Desert|їµєс»зё·|»зё·",	3, 63, 3440, 5025 },

// Threeway
{ "Three|ЅВ·ж°о",		1, 64, 4021, 6739 },
{ "Three|ЅВ·ж°о",		2, 64, 2704, 7399 },
{ "Three|ЅВ·ж°о",		3, 64, 3213, 8080 },

// №Р±і»зїш
{ "Milgyo|№Р±і»зїш",	1, 65, 5536, 1436 },
{ "Milgyo|№Р±і»зїш",	2, 65, 5536, 1436 },
{ "Milgyo|№Р±і»зїш",	3, 65, 5536, 1436 },

// »з±НЕёїцАФ±ё
{ "»з±НЕёїцАФ±ё",		1, 65, 5905, 1108 },
{ "»з±НЕёїцАФ±ё",		2, 65, 5905, 1108 },
{ "»з±НЕёїцАФ±ё",		3, 65, 5905, 1108 },
	...
	if (item)
	{
		if (item->IsDragonSoul())
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "АОєҐАМ И°јєИ­ µЗБц ѕКАЅ.");
				}
				else
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!m_bAll && iDist >= 1000)	// 10№МЕН АМ»уїЎ АЦґВ °НµйАє purge ЗПБц ѕКґВґЩ.
			return;
...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("(%s) ±жµе°Ў »эјєµЗѕъЅАґПґЩ. [АУЅГ]"), cp.name);
}
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("ЗцАз АьАпБЯАО ±жµе АФґПґЩ"));
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s: БёАзЗПБц ѕКґВ ±жµе АФґПґЩ."), arg1);
	}
}
...
/**
 * @version 05/06/08	Bang2ni - ±жµе єёіКЅє ДщЅєЖ® БшЗа ѕИµЗґВ №®Б¦ јцБ¤.(ЅєЕ©ёіЖ®°Ў АЫјєѕИµК.)
 * 			          quest/priv_guild.quest ·О єОЕН ЅєЕ©ёіЖ® АРѕоїА°Ф јцБ¤µК
 */
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±Ч·± АМё§ ¶ЗґВ №шИЈАЗ ±жµе°Ў ѕшЅАґПґЩ."));
		else
		{
			char buf[1024+1];
...
{
	// GMАМ ѕЖґП°ЕіЄ block_chat_privilege°Ў ѕшґВ »з¶чАє ён·Йѕо »зїл єТ°Ў
	if (!ch || (ch->GetGMLevel() < GM_HIGH_WIZARD && ch->GetQuestFlag("chat_privilege.block") <= 0))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±Ч·± ён·ЙѕоґВ ѕшЅАґПґЩ"));
		return;
	}
...
ACMD(do_block_chat)
{
	...
	if (lBlockDuration < 0)
	{
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "АЯёшµИ ЗьЅДАЗ ЅГ°ЈАФґПґЩ. h, m, sё¦ єЩї©ј­ БцБ¤ЗШ БЦЅКЅГїА.");
			ch->ChatPacket(CHAT_TYPE_INFO, "ї№) 10s, 10m, 1m 30s");
		}
		return;
	}
...
ACMD(do_build)
{
	...
	// NOTE: Б¶°З ГјЕ©µйАє Е¬¶уАМѕрЖ®їН ј­№ц°Ў ЗФІІ ЗП±в ¶§№®їЎ №®Б¦°Ў АЦА» ¶§ґВ
	//       ёЮјјБцё¦ АьјЫЗПБц ѕК°н їЎ·Їё¦ Гв·ВЗСґЩ.
	if (!pkLand)
	{
	...
	{
		// ЗГ·№АМѕо°Ў БэА» БцА» ¶§ґВ ¶ҐАМ і»І«Бц И®АОЗШѕЯ ЗСґЩ.
	...
	switch (LOWER(*arg1))
	{
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("БёАзЗПБц ѕКґВ °З№°АФґПґЩ."));
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°°АМ БцА» јц ѕшґВ Бѕ·щАЗ °З№°АМ БцѕоБ® АЦЅАґПґЩ."));
						return;
					}
				}

				// °З№° БѕјУјє ГјЕ© (АМ °З№°АМ БцѕоБ® АЦѕоѕЯЗФ)
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°ЗјіїЎ ЗКїдЗС °З№°АМ БцѕоБ® АЦБц ѕКЅАґПґЩ."));
							return;
						}
					}
				}
				...
				{
					// GMАМ ѕЖґТ°жїмёё (ЕЧј·їЎј­ґВ GMµµ јТёр)
					// °Зјі єсїл ГјЕ©
					if (t->dwPrice > BUILDING_MAX_PRICE)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°З№° єсїл Б¤єё АМ»уАё·О °Зјі АЫѕчїЎ ЅЗЖРЗЯЅАґПґЩ."));
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°Зјі єсїлАМ єОБ·ЗХґПґЩ."));
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("АЪАз°Ў єОБ·ЗПї© °ЗјіЗТ јц ѕшЅАґПґЩ."));
							return;
						}
					}
				}
				...
				// 20050811.myevan.°З№° ИёАь ±вґЙ єААО ЗШБ¦
				/*
				   if (x_rot != 0.0f || y_rot != 0.0f || z_rot != 0.0f)
				   {
				   ch->ChatPacket(CHAT_TYPE_INFO, "°З№° ИёАь ±вґЙАє ѕЖБч Б¦°шµЗБц ѕКЅАґПґЩ");
				   return;
				   }
				 */
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°З№°А» БцА» јц ѕшґВ А§ДЎАФґПґЩ."));
					return;
				}

				if (test_server || GMLevel == GM_PLAYER)
					// °Зјі Аз·б јТёрЗП±в (ЕЧј·їЎј­ґВ GMµµ јТёр)
				{
					// °Зјі єсїл јТёр
		...
		case 'W' :
			// ґгАе јјїм±в
			// build (w)all ґгАе№шИЈ ґгАеЕ©±в ґл№®µї ґл№®ј­ ґл№®іІ ґл№®єП
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("БёАзЗПБц ѕКґВ ДіёЇЕН АФґПґЩ."));
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·Г °­Б¦ Бѕ·б ЅЗЖР"));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·Г °­Б¦ Бѕ·б јє°ш"));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїЎј­ іЄ°ЎјМЅАґПґЩ."));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїЎј­ іЄ°ЎјМЅАґПґЩ."));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАМ јє°шАыАё·О ЅГАЫ µЗѕъЅАґПґЩ."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·Г ЅГАЫїЎ №®Б¦°Ў АЦЅАґПґЩ."));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ґл·ГАЪ°Ў ѕшЅАґПґЩ."));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("µР°© БЯїЎґВ ґЙ·ВА» їГё± јц ѕшЅАґПґЩ."));
		return;
	}
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("іІАє ЅєЕИ ЖчАОЖ®°Ў ѕшЅАґПґЩ."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("іІАє ЅєЕИ ЖчАОЖ®°Ў АыЅАґПґЩ."));
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°ЄА» АЯёш АФ·ВЗПїґЅАґПґЩ."));
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ён·ЙѕоАЗ ј­єк ДїёЗµе°Ў АЯёш µЗѕъЅАґПґЩ."));
			return;
			break;
	}
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЗГ·№АМѕо %d їН ЗГ·№АМѕо  %dё¦ ЖДИҐЅГЕµґПґЩ.."), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°ўБ¦±№ БшЗа Б¤єё"));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ј±ЕГ ёК Б¤єё јєБц %d Ел·О %d %d %d"), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3)); 
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("іЄАЗ »п°Её® БшЗаБ¤єё"));
...
ACMD(do_check_monarch_money)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "±№°н: %d їш", NationMoney);
}
...
ACMD (do_attr_full_set)
{
	...
	case JOB_SHAMAN:
		{
			// №«»з ёц»§ јВЖГ.
			// АМ°Нёё іЄїН АЦѕој­ АУЅГ·О ёрµз Бч±є ґЩ АМ·± јУјє µыё§.
...
ACMD (do_use_item)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "ѕЖАМЕЫАМ ѕшѕој­ ВшїлЗТ јц ѕшѕо.");
	}
}
