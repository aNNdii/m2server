...
void CPVP::Win(DWORD dwPID)
{
	...
	m_players[iSlot].bAgree = true; // АЪµїАё·О µїАЗ
...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	{
		// є№јцЗТ јц АЦАёёй №Щ·О ЅОїт!
		if (pkPVP->Agree(pkChr->GetPlayerID()))
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%sґФ°ъАЗ ґл°б ЅГАЫ!"), pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%sґФ°ъАЗ ґл°б ЅГАЫ!"), pkVictim->GetName());
		}
		return;
	}
	...
	snprintf(msg, sizeof(msg), LC_TEXT("%sґФАМ ґл°бЅЕГ»А» ЗЯЅАґПґЩ. ЅВі«ЗП·Бёй ґл°бµїАЗё¦ ЗПјјїд."), pkChr->GetName());

	pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
	pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%sїЎ°Ф ґл°бЅЕГ»А» ЗЯЅАґПґЩ."), pkVictim->GetName());
	...
	if (pkVictimDesc)
	{
		...
		enhance_strlcpymt(pack.szNameFrom, pkChr->GetName(), sizeof(pack.szNameFrom));
...
// ё®ЕП°Є: 0 = PK, 1 = PVP
// PVPё¦ ё®ЕПЗПёй °жЗиДЎіЄ ѕЖАМЕЫА» ¶і±ё°н PKёй ¶і±ёБц ѕКґВґЩ.
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkChr == pkVictim)  // і»°Ў іЇ ДҐ¶у°н ЗПіЧ -_-
		return false;
	...
	else
	{
		switch( pkChr->GetMountVnum() )
		{
			...
			case 20211:
			case 20212:
			case 20119:		// ¶уё¶ґЬ Ижё¶
			case 20219:		// ¶уё¶ґЬ Ижё¶ Е¬·Р (ЗТ·ОА©їл)
			case 20220:		// Е©ё®Ѕєё¶Ѕє Е»°Н
			case 20221:		// Аь°© №йїх
			case 20222:		// Аь°© ЖТґх
			case 20120:
			case 20121:
			...
			case 20124:
			case 20125:
			case 20214:		// і­ЖшЗС Аь°©јшјш·П	
			case 20215:		// їлёНЗС Аь°©јшјш·П	
			case 20217:		// і­ЖшЗС Аь°©ѕПјш·П	
			case 20218:		// їлёНЗС Аь°©ѕПјш·П
			case 20224:		// і­ЖшЗС Аь°©ј®·жАЪ
			case 20225:		// їлёНЗС Аь°©ј®·жАЪ
			case 20226:		//	АЇґПДЬ
			case 20227:
				break;
	...
	else
	{
		...
		if (pkChr->GetAlignment() < 0 && pkVictim->GetAlignment() >= 0)
		{
		    ...
		    {
			// №ь№эАЪґВ ЖтИ­ёрµеАО ВшЗС»з¶чА» °ш°ЭЗТ јц ѕшґЩ.
...
void CPVPManager::Process()
{
	...
	while (it != m_map_pkPVP.end())
	{
		...
		if (get_dword_time() - pvp->GetLastFightTime() > 600000) // 10єР АМ»у ЅОїтАМ ѕшѕъАёёй
		{
			pvp->Packet(true);
			Delete(pvp);
		}
