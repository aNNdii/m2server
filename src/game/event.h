...
extern void		event_cancel(LPEVENT * event);			// АМєҐЖ® ГлјТ
extern long		event_processing_time(LPEVENT event);	// јцЗа ЅГ°Ј ё®ЕП
extern long		event_time(LPEVENT event);			// іІАє ЅГ°Ј ё®ЕП
extern void		event_reset_time(LPEVENT event, long when);	// ЅЗЗа ЅГ°Ј Аз јіБ¤
