...
namespace marriage
{
	...
	struct TMarriageItemBonusByGrade
	{
		...
		{ 71069,	{ 4,	5,	6,	8,  } }, // °ьЕл Бх°Ў
		{ 71070,	{ 10,	12,	15,	20, } }, // °жЗиДЎ Бх°Ў
		{ 71071,	{ 4,	5,	6,	8,  } }, // Е©ё®ЖјДГ Бх°Ў
		{ 71072,	{ -4,	-5,	-6,	-8, } }, // »уґл№ж °ш°Э·В °ЁјТ
		{ 71073,	{ 20,	25,	30,	40, } }, // °ш°Э·В Бх°Ў (Аэґл°Є)
		{ 71074,	{ 12,	16,	20,	30, } }, // №жѕо·В Бх°Ў (Аэґл°Є)

		//{ 71069,	1,	2,	3,	6,	8,  }, // °ьЕл Бх°Ў
		//{ 71070,	5,	7,	10,	15,	20, }, // °жЗиДЎ Бх°Ў
		//{ 71071,	1,	2,	3,	6,	8,  }, // Е©ё®ЖјДГ Бх°Ў
		//{ 71072,	5,	10,	15,	20,	30, }, // »уґл№жАМ АФАє µҐ№МБцё¦ іЄїЎ°Ф·О
		//{ 71073,	10,	15,	20,	25,	40, }, // °ш°Э·В Бх°Ў (Аэґл°Є)
		//{ 71074,	5,	10,	15,	20,	30, }, // №жѕо·В Бх°Ў (Аэґл°Є)
	};
	...
	void SendLoverInfo(LPCHARACTER ch, const string& lover_name, int love_point)
	{
		...
		enhance_strlcpymt(p.name, lover_name.c_str(), sizeof(p.name));
	...
	int TMarriage::GetMarriagePoint()
	{
		...
		// їшѕУАЗ ±кЕР »зїлБЯАП ¶§ :
		// іЇВҐїЎ АЗЗС їµЗв 80% ЗП·зґз 8%
		// АьЕхїЎ АЗЗС їµЗв 80%
		// ЕдЕ» 100%

		// єс»зїлБЯАП ¶§ : 
		// іЇВҐїЎ АЗЗС їµЗв 60% ЗП·зґз 6%
		// АьЕхїЎ АЗЗС їµЗв 60%
		// ЕдЕ» 100%
	...
	int TMarriage::GetBonus(DWORD dwItemVnum, bool bShare, LPCHARACTER me)
	{
		...
		// БЦєЇїЎ ѕшА»¶§ґВ АЪ±в ±вґЙёё АыїлµИґЩ.

		// ЗШґз ѕЖАМЕЫАМ ѕо¶І ±вґЙА» ЗПґВБц ГЈґВґЩ.
	...
	void TMarriage::Login(LPCHARACTER ch)
	{
		...
		// µС ґЩ АМ ЗБ·ОјјЅєїЎ ·О±ЧАО БЯАМёй ЖчАОЕНё¦ ї¬°бЗП°н АМєҐЖ® №Я»э
		if (IsOnline())
		{
		...
		// µС ґЩ ·О±ЧАО µЗѕо АЦґЩёй ЖРЕ¶А» єёіЅґЩ.
		if (is_married)
		{
			LPDESC d1, d2;
			CCI * pkCCI;
	...
	{
		MariageMap::const_iterator it = m_MarriageByPID.find(dwPlayerID);
	...
	void CManager::RequestAdd(DWORD dwPID1, DWORD dwPID2, const char* szName1, const char* szName2)
	{
		...
		p.dwPID1 = dwPID1;
		p.dwPID2 = dwPID2;
		enhance_strlcpymt(p.szName1, szName1, sizeof(p.szName1));
		enhance_strlcpymt(p.szName2, szName2, sizeof(p.szName2));
