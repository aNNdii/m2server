...
CGuild::CGuild(TGuildCreateParameter & cp)
{
	...
	enhance_strlcpymt(m_data.grade_array[0].grade_name, LC_TEXT("±жµеАе"), sizeof(m_data.grade_array[0].grade_name));
	...
	{
		enhance_strlcpymt(m_data.grade_array[i].grade_name, LC_TEXT("±жµеїш"), sizeof(m_data.grade_array[i].grade_name));
...
void CGuild::SendListPacket(LPCHARACTER ch)
{
	/*
	   ...
	   [
	   ...
	   name_flag 1 - АМё§А» єёі»ґАіД ѕИєёі»ґАіД
	...
	for (TGuildMemberContainer::iterator it = m_member.begin(); it != m_member.end(); ++it)
	{
		...
		enhance_strlcpymt(c, it->second.name.c_str(), MIN(sizeof(c), it->second.name.length() + 1));
	...
	*/
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		if (grade >= 1 && grade <= 15)
		{
			...
			enhance_strlcpymt(m_data.grade_array[grade-1].grade_name, name, sizeof(m_data.grade_array[grade-1].grade_name));
...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	...
	enhance_strlcpymt(m_data.name, row[3], sizeof(m_data.name));
...
void CGuild::__P2PUpdateGrade(SQLMsg* pmsg)
{
	if (pmsg->Get()->uiNumRows)
	{
		...
		// µо±Ю ёнДЄАМ ЗцАзїН ґЩёЈґЩёй ѕчµҐАМЖ®
		if (0 != strcmp(m_data.grade_array[grade].grade_name, name))
		{
			...
			pack2.grade = grade + 1;
			enhance_strlcpymt(pack2.grade_name, name, sizeof(pack2.grade_name));
...
void CGuild::ChangeGradeName(BYTE grade, const char* grade_name)
{
	...
	grade--;
	enhance_strlcpymt(m_data.grade_array[grade].grade_name, grade_name, sizeof(m_data.grade_array[grade].grade_name));
	...
	pack.size+=sizeof(pack2);
	pack2.grade = grade+1;
	enhance_strlcpymt(pack2.grade_name,grade_name, sizeof(pack2.grade_name));
...
void CGuild::SendGuildInfoPacket(LPCHARACTER ch)
{
	...
	enhance_strlcpymt(pack_sub.name, m_data.name, sizeof(pack_sub.name));
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> Б¦°шЗП°нАЪ ЗПґВ °жЗиДЎ°Ў іІАє °жЗиДЎєёґЩ ё№ЅАґПґЩ."));
		return false;
	}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »иБ¦ЗТ јц ѕшґВ ±ЫАФґПґЩ."));
	else
...
void CGuild::RefreshCommentForce(DWORD player_id)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; i++)
	{
		...
		enhance_strlcpymt(szName, row[1], sizeof(szName));
		enhance_strlcpymt(szContent, row[2], sizeof(szContent));

		d->BufferedPacket(&id, sizeof(id));
		d->BufferedPacket(szName, sizeof(szName));

		if (i == pmsg->Get()->uiNumRows - 1)
			d->Packet(szContent, sizeof(szContent)); // ё¶Бцё· БЩАМёй єёі»±в
		else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	{
		// АМ№М °Й·Б АЦАё№З·О »зїлЗПБц ѕКАЅ.
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> їлЅЕ·ВАМ єОБ·ЗХґПґЩ. (%d, %d)"), GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ДрЕёАУАМ іЎіЄБц ѕКѕЖ ±жµе ЅєЕіА» »зїлЗТ јц ѕшЅАґПґЩ."));
		return;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> %d ЅєЕіА» »зїлЗФ (%d, %d) to %u"), dwVnum, GetSP(), iNeededSP, pid);

	switch (dwVnum)
	{
		case GUILD_SKILL_TELEPORT:
			// ЗцАз ј­№цїЎ АЦґВ »з¶чА» ёХАъ ЅГµµ.
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл°Ў %d Г¤іОїЎ АЦЅАґПґЩ. (ЗцАз Г¤іО %d)"), pcci->bChannel, g_bChannel);
					}
					else
					{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл°Ў їВ¶уАО »уЕВ°Ў ѕЖґХґПґЩ."));
			}
			break;
		...
		default:
			{
				/*if (ch->GetPlayerID() != GetMasterPID())
				  {
				  ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАеёё ±жµе ЅєЕіА» »зїлЗТ јц АЦЅАґПґЩ."));
				  return;
				  }*/

				if (!UnderAnyWar())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµе ЅєЕіАє ±жµеАь БЯїЎёё »зїлЗТ јц АЦЅАґПґЩ."));
					return;
				}
...
void CGuild::Chat(const char* c_pszText)
{
	...
	p1.dwGuild = GetID();
	enhance_strlcpymt(p2.szText, c_pszText, sizeof(p2.szText));
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> %uАЗ їлЅЕ·ВА» Иёє№ЗПїґЅАґПґЩ."), iSP);
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> АбЅГИДїЎ АМїлЗШБЦЅКЅГїА"));
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	if (ch->GetPlayerID() != GetMasterPID())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµе ±Э°нїЎј± ±жµеАеёё Гв±ЭЗТ јц АЦЅАґПґЩ."));
		return;
	}
...
{
	DWORD	dwInviteePID;		///< ГКґл№ЮАє character АЗ PID
	DWORD	dwGuildID;		///< ГКґлЗС Guild АЗ ID
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл№жАМ ГКґл ЅЕГ»А» №ЮА» јц ѕшґВ »уЕВАФґПґЩ."));
	    return;
	}
	...
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл№жАМ ±жµе ГКґл °ЕєО »уЕВАФґПґЩ.") );
		return;
	} 
	else if ( !HasGradeAuth( GetMember( pchInviter->GetPlayerID() )->grade, GUILD_AUTH_ADD_MEMBER ) ) 
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеїшА» ГКґлЗТ ±ЗЗСАМ ѕшЅАґПґЩ.") );
		return;
	} 
	else if ( pchInvitee->GetEmpire() != pchInviter->GetEmpire() ) 
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ґЩёҐ Б¦±№ »з¶чА» ±жµеїЎ ГКґлЗТ јц ѕшЅАґПґЩ.") );
		return;
	}
	...
	switch ( errcode ) 
	{
		...
		case GERR_WITHDRAWPENALTY:
						pchInviter->ChatPacket( CHAT_TYPE_INFO, 
								LC_TEXT("<±жµе> Е»ЕрЗС ИД %dАПАМ БціЄБц ѕКАє »з¶чАє ±жµеїЎ ГКґлЗТ јц ѕшЅАґПґЩ."), 
								quest::CQuestManager::instance().GetEventFlag( "guild_withdraw_delay" ) );
						return;
		case GERR_COMMISSIONPENALTY:
						pchInviter->ChatPacket( CHAT_TYPE_INFO, 
								LC_TEXT("<±жµе> ±жµеё¦ ЗШ»кЗС Бц %dАПАМ БціЄБц ѕКАє »з¶чАє ±жµеїЎ ГКґлЗТ јц ѕшЅАґПґЩ."), 
								quest::CQuestManager::instance().GetEventFlag( "guild_disband_delay") );
						return;
		case GERR_ALREADYJOIN:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл№жАМ АМ№М ґЩёҐ ±жµеїЎ јУЗШАЦЅАґПґЩ.")); return;
		case GERR_GUILDISFULL:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ГЦґл ±жµеїш јцё¦ ГК°ъЗЯЅАґПґЩ.")); return;
		case GERR_GUILD_IS_IN_WAR : pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ЗцАз ±жµе°Ў АьАп БЯ АФґПґЩ.") ); return;
		case GERR_INVITE_LIMIT : pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ЗцАз ЅЕ±Ф °ЎАФ Б¦ЗС »уЕВ АФґПґЩ.") ); return;
	...
	switch ( errcode ) 
	{
		...
		case GERR_COMMISSIONPENALTY:
						pchInvitee->ChatPacket( CHAT_TYPE_INFO, 
								...
								quest::CQuestManager::instance().GetEventFlag( "guild_disband_delay") );
						return;
		case GERR_ALREADYJOIN:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> »уґл№жАМ АМ№М ґЩёҐ ±жµеїЎ јУЗШАЦЅАґПґЩ.")); return;
		case GERR_GUILDISFULL:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ГЦґл ±жµеїш јцё¦ ГК°ъЗЯЅАґПґЩ.")); return;
		case GERR_GUILD_IS_IN_WAR : pchInvitee->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ЗцАз ±жµе°Ў АьАп БЯ АФґПґЩ.") ); return;
		case GERR_INVITE_LIMIT : pchInvitee->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±жµе> ЗцАз ЅЕ±Ф °ЎАФ Б¦ЗС »уЕВ АФґПґЩ.") ); return;
