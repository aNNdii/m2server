...
{
	DWORD pid; // player ЕЧАМєнАЗ id; primary key
	BYTE grade; // ±жµе»уАЗ ЗГ·№АМѕоАЗ °и±Ю 1 to 15 (1АМ ВЇ)
	BYTE is_general;
	BYTE job;
	BYTE level;
	DWORD offer_exp; // °шЗеЗС °жЗиДЎ
	BYTE _dummy;
...
{
	char grade_name[GUILD_GRADE_NAME_MAX_LEN+1]; // 8+1 ±жµеАе, ±жµеїш µоАЗ АМё§
	BYTE auth_flag;
} TGuildGrade;
...
class CGuild
{
	public:
		...
		void		P2PChat(const char* c_pszText); // ±жµе Г¤ЖГ
		...
		void		RecvWithdrawMoneyGive(int iChangeGold); // bGive==1 АМёй ±жµеАеїЎ°Ф БЦґВ °Й ЅГµµЗП°н јє°шЅЗЖРё¦ µрєсїЎ°Ф єёіЅґЩ
		...
		bool		UnderWar(DWORD guild_id); // АьАпБЯАО°Ў?
		...
		// GUILD_JOIN_BUG_FIX
		/// character їЎ°Ф ±жµе°ЎАФ ГКґлё¦ ЗСґЩ.
		/**
		 * @param	pchInviter ГКґлЗС character.
		 * @param	pchInvitee ГКґлЗТ character.
		 *
		 * ГКґлЗП°ЕіЄ №ЮА»јц ѕшґВ »уЕВ¶уёй ЗШґзЗПґВ Г¤ЖГ ёЮјјБцё¦ АьјЫЗСґЩ.
		 */
		void		Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee );

		/// ±жµеГКґлїЎ ґлЗС »уґл character АЗ јц¶фА» Гіё®ЗСґЩ.
		/**
		 * @param	pchInvitee ГКґл№ЮАє character
		 *
		 * ±жµеїЎ °ЎАФ°ЎґЙЗС »уЕВ°Ў ѕЖґП¶уёй ЗШґзЗПґВ Г¤ЖГ ёЮјјБцё¦ АьјЫЗСґЩ.
		 */
		void		InviteAccept( LPCHARACTER pchInvitee );

		/// ±жµеГКґлїЎ ґлЗС »уґл character АЗ °ЕєОё¦ Гіё®ЗСґЩ.
		/**
		 * @param	dwPID ГКґл№ЮАє character АЗ PID
		 */
	...
	private:
		...
		// GUILD_JOIN_BUG_FIX
		/// ±жµе °ЎАФА» ЗТ јц ѕшА» °жїмАЗ їЎ·ЇДЪµе.
		enum GuildJoinErrCode {
			GERR_NONE			= 0,	///< Гіё®јє°ш
			GERR_WITHDRAWPENALTY,		///< Е»ЕрИД °ЎАФ°ЎґЙЗС ЅГ°ЈАМ БціЄБц ѕКАЅ
			GERR_COMMISSIONPENALTY,		///< ЗШ»кИД °ЎАФ°ЎґЙЗС ЅГ°ЈАМ БціЄБц ѕКАЅ
			GERR_ALREADYJOIN,			///< ±жµе°ЎАФ ґл»у ДіёЇЕН°Ў АМ№М ±жµеїЎ °ЎАФЗШ АЦАЅ
			GERR_GUILDISFULL,			///< ±жµеАОїш Б¦ЗС ГК°ъ
			GERR_GUILD_IS_IN_WAR,		///< ±жµе°Ў ЗцАз АьАпБЯ
			GERR_INVITE_LIMIT,			///< ±жµеїш °ЎАФ Б¦ЗС »уЕВ
			GERR_MAX				///< Error code ГЦ°нДЎ. АМ ѕХїЎ Error code ё¦ ГЯ°ЎЗСґЩ.
		};

		/// ±жµеїЎ °ЎАФ °ЎґЙЗС Б¶°ЗА» °Л»зЗСґЩ.
		/**
		 * @param [in]	pchInvitee ГКґл№ЮґВ character
		 * @return	GuildJoinErrCode
		 */
		GuildJoinErrCode	VerifyGuildJoinableCondition( const LPCHARACTER pchInvitee );

		typedef std::map< DWORD, LPEVENT >	EventMap;
		EventMap	m_GuildInviteEventMap;	///< ±жµе ГКГ» Event map. key: ГКґл№ЮАє ДіёЇЕНАЗ PID
		// END_OF_GUILD_JOIN_BUG_FIX
};
