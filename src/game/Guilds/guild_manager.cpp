...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµе АМё§АМ АыЗХЗПБц ѕКЅАґПґЩ."));
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> АМ№М °°Ає АМё§АЗ ±жµе°Ў АЦЅАґПґЩ."));
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеё¦ »эјєЗТ јц ѕшЅАґПґЩ."));
		return 0;
	}
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµе°Ў %s ±жµеїЎ ј±АьЖч°нё¦ ЗПїґЅАґПґЩ!"), TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
		}
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> %s ±жµе°Ў ±жµеАьА» °ЕєОЗПїґЅАґПґЩ."), g1->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеїН %s ±жµе°Ў АбЅГ ИД АьАпА» ЅГАЫЗХґПґЩ!"), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	char buf[256];
	snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеїН %s ±жµе°Ў АьАпА» ЅГАЫЗПїґЅАґПґЩ!"), g1->GetName(), g2->GetName());
	SendNotice(buf);
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеїН %s ±жµе »зАМАЗ АьАпАМ №«ЅВєО·О іЎіµЅАґПґЩ."), g1->GetName(), g2->GetName());
		}
		else
		{
			if ( g1->GetWarScoreAgainstTo( g2->GetID() ) > g2->GetWarScoreAgainstTo( g1->GetID() ) )
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµе°Ў %s ±жµеїНАЗ АьАпїЎј­ ЅВё® ЗЯЅАґПґЩ."), g1->GetName(), g2->GetName());
			}
			else
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµе°Ў %s ±жµеїНАЗ АьАпїЎј­ ЅВё® ЗЯЅАґПґЩ."), g2->GetName(), g1->GetName());
			}
		}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАьАМ ГлјТ µЗѕъЅАґПґЩ."));
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАьАМ ГлјТ µЗѕъЅАґПґЩ."));
	}

	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf), LC_TEXT("%s ±жµеїН %s ±жµе »зАМАЗ АьАпАМ ГлјТµЗѕъЅАґПґЩ."), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
