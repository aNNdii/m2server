...
void CGuild::GuildWarPacket(DWORD dwOppGID, BYTE bWarType, BYTE bWarState)
{
	...
	for (TGuildMemberOnlineContainer::const_iterator it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
	{
		...
		if (bWarState == GUILD_WAR_ON_WAR)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±жµе> ±жµеАьБЯїЎґВ »зіЙїЎ µыёҐ АМАНАМ ѕшЅАґПґЩ."));
...
bool CGuild::CanStartWar(BYTE bGuildWarType) // ЕёАФїЎ µы¶у ґЩёҐ Б¶°ЗАМ »э±ж јцµµ АЦАЅ
{
	if (bGuildWarType >= GUILD_WAR_TYPE_MAX_NUM)
		return false;

	// ЕЧЅєЖ®ЅГїЎґВ АОїшјцё¦ И®АОЗПБц ѕКґВґЩ.
...
void CGuild::RequestDeclareWar(DWORD dwOppGID, BYTE type)
{
	...
	if (it == m_EnemyGuild.end())
	{
		if (!GuildWar_IsWarMap(type))
		{
			...
			map_allow_log();
			NotifyGuildMaster(LC_TEXT("АьАп ј­№ц°Ў ї­·БАЦБц ѕКѕЖ ±жµеАьА» ЅГАЫЗТ јц ѕшЅАґПґЩ."));
			return;
		}

		// ЖРЕ¶ єёі»±в to another server
		TPacketGuildWar p;
		p.bType = type;
	...
	{	
		...
		case GUILD_WAR_SEND_DECLARE:
			{
				NotifyGuildMaster(LC_TEXT("АМ№М ј±АьЖч°н БЯАО ±жµеАФґПґЩ."));
			}
			break;
		default:
...
bool CGuild::WaitStartWar(DWORD dwOppGID)
{
	...
	//»уґл№ж ±жµе TGuildWar ё¦ ѕтѕоїВґЩ.
	TEnemyGuildContainer::iterator it = m_EnemyGuild.find(dwOppGID);
	if (it == m_EnemyGuild.end())
	{
	...
	//»уґлЖнАЗ ±жµе Е¬·ЎЅє ЖчАОЕНё¦ ѕтѕоїА°н
...
void CGuild::GuildWarEntryAccept(DWORD dwOppGID, LPCHARACTER ch)
{
	...
	if (gw.state != GUILD_WAR_ON_WAR)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("АМ№М АьАпАМ іЎіµЅАґПґЩ."));
		return;
	}
...
{ 
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("<±жµе> ·Ўґх БЎјц°Ў %d БЎАМ µЗѕъЅАґПґЩ"), point);
		for (TGuildMemberOnlineContainer::const_iterator it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
		{
			LPCHARACTER ch = (*it);
