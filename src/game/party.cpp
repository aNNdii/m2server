...
bool CPartyManager::SetParty(LPCHARACTER ch)	// PCёё »зїлЗШѕЯ ЗСґЩ!!
{
...
void CParty::Destroy()
{
	...
	// PC°Ў ёёµз ЖДЖјёй ЖДЖјёЕґПАъїЎ ёКїЎј­ PIDё¦ »иБ¦ЗШѕЯ ЗСґЩ.
	if (m_bPCParty)
	{
	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖј°Ў ЗШ»к µЗѕъЅАґПґЩ."));
			}
			else
			{
				// NPCАП °жїм АПБ¤ ЅГ°Ј ИД АьЕх БЯАМ ѕЖґТ ¶§ »з¶уБц°Ф ЗПґВ АМєҐЖ®ё¦ ЅГАЫЅГЕІґЩ.
...
void CParty::Join(DWORD dwPID)
{
	...
	if (m_bPCParty)
	{
		...
		p.dwPID = dwPID;
		p.bState = PARTY_ROLE_NORMAL; // #0000790: [M2EU] CZ Е©·ЎЅ¬ Бх°Ў: ГК±вИ­ БЯїд! 
...
void CParty::Link(LPCHARACTER pkChr)
{
	...
	// ЗГ·№АМѕо ЖДЖјАП °жїм ѕчµҐАМЖ® АМєҐЖ® »эјє
...
void CParty::Unlink(LPCHARACTER pkChr)
{
	...
	if (pkChr->IsPC())
	{
		...
		//SendPartyUnlinkAllToOne(pkChr); // Іч±вґВ °НАМ№З·О ±ёБц Unlink ЖРЕ¶А» єёіѕ ЗКїд ѕшґЩ.
		...
		if (it->second.bRole == PARTY_ROLE_LEADER)
		{
			...
			{
				// TODO: ґшБЇїЎ АЦАёёй іЄёУБцµµ іЄ°ЈґЩ
				FExitDungeon f;
				ForEachNearMember(f);
			}
		}
...
void CParty::SendPartyJoinOneToAll(DWORD pid)
{
	...
	p.pid = pid;
	enhance_strlcpymt(p.name, r.strName.c_str(), sizeof(p.name));
...
void CParty::SendPartyJoinAllToOne(LPCHARACTER ch)
{
	...
	{
		p.pid = it->first;
		enhance_strlcpymt(p.name, it->second.strName.c_str(), sizeof(p.name));
...
void CParty::SendMessage(LPCHARACTER ch, BYTE bMsg, DWORD dwArg1, DWORD dwArg2)
{
	...
	switch (bMsg)
	{
		...
		case PM_ATTACKED_BY:	// °ш°Э №ЮѕТАЅ, ё®ґхїЎ°Ф µµїтА» їдГ»
			{
				// ё®ґх°Ў ѕшА» ¶§
...
{
	// XXX DELETEME Е¬¶уАМѕрЖ® їП·бµЙ¶§±оБц
	{
		return;
	}
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> јТИЇЗП·БґВ ґл»уА» ГЈА» јц ѕшЅАґПґЩ."));
		return;
	}
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ґл»уА» јТИЇЗТ јц ѕшЅАґПґЩ."));
		return;
	}
	...
	if (n == 0)
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ЖДЖј> ЖДЖјїшА» ЗцАз А§ДЎ·О јТИЇЗТ јц ѕшЅАґПґЩ."));
	else
	{
		int i = number(0, n - 1);
...
void CParty::Update()
{
	...
	if (!m_bPartyHealReady)
	{
		...
		m_bCanUsePartyHeal = m_iLeadership >= 18; // ЕлјЦ·В 18 АМ»уАє ИъА» »зїлЗТ јц АЦАЅ.

		// ЕлјЦ·В 40АМ»уАє ЖДЖј Иъ ДрЕёАУАМ АыґЩ.
		...
		if (m_bCanUsePartyHeal)
		{
			if (get_dword_time() > m_dwPartyHealTime + PartyHealCoolTime)
			{
				...
				// send heal ready
				if (0) // XXX  DELETEME Е¬¶уАМѕрЖ® їП·бµЙ¶§±оБц
...
void CParty::UpdateOnlineState(DWORD dwPID, const char* name)
{
	...
	p.pid = dwPID;
	r.strName = name;
	enhance_strlcpymt(p.name, name, sizeof(p.name));
...
BYTE CParty::GetMemberMaxLevel()
{
	...
	TMemberMap::const_iterator it = m_memberMap.begin();
	while (it != m_memberMap.end())
	{
		if (!it->second.bLevel)
		{
