...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»уґл№жАМ ДЈ±ё ГЯ°Ўё¦ №ЮА» јц ѕшґВ »уЕВАФґПґЩ."));
	    return;
	}
...
void MessengerManager::__AddToList(const std::string& account, const std::string& companion)
{
	...
	if (d)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ёЮЅЕБ®> %s ґФА» ДЈ±ё·О ГЯ°ЎЗПїґЅАґПґЩ."), companion.c_str());
	}
...
void MessengerManager::AddToList(const std::string& account, const std::string& companion)
{
	...
	enhance_strlcpymt(p2ppck.szAccount, account.c_str(), sizeof(p2ppck.szAccount));
	enhance_strlcpymt(p2ppck.szCompanion, companion.c_str(), sizeof(p2ppck.szCompanion));
...
void MessengerManager::__RemoveFromList(const std::string& account, const std::string& companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ёЮЅЕБ®> %s ґФА» ёЮЅЕАъїЎј­ »иБ¦ЗПїґЅАґПґЩ."), companion.c_str());
}
