...
bool CLIENT_DESC::Connect(int iPhaseWhenSucceed)
{
	...
	if (get_global_time() - m_LastTryToConnectTime < 3)	// 3ГК
		return false;
		...
		case PHASE_DBCLIENT:
			{
				...
				memset(&p, 0, sizeof(p));
				enhance_strlcpymt(p.szPublicIP, g_szPublicIP, sizeof(p.szPublicIP));
				...
				if (!g_bAuthServer)
				{
					...
					if (p.dwLoginCount)
					{
						...
						for (it = c_set.begin(); it != c_set.end(); ++it)
						{
							...
							{
								pck.dwID = r.id;
								enhance_strlcpymt(pck.szLogin, r.login, sizeof(pck.szLogin));
								enhance_strlcpymt(pck.szSocialID, r.social_id, sizeof(pck.szSocialID));
								enhance_strlcpymt(pck.szHost, d->GetHostName(), sizeof(pck.szHost));
...
{
	enum {
		CHANNELSTATUS_UPDATE_PERIOD = 5*60*1000,	// 5єРё¶ґЩ
	};
