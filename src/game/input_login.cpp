...
static void _send_bonus_info(LPCHARACTER ch)
{
	...
	if (item_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, 
				LC_TEXT("ѕЖАМЕЫ µе·У·ь  %d%% ГЯ°Ў АМєҐЖ® БЯАФґПґЩ."), item_drop_bonus);
	}
	if (gold_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, 
				LC_TEXT("°сµе µе·У·ь %d%% ГЯ°Ў АМєҐЖ® БЯАФґПґЩ."), gold_drop_bonus);
	}
	if (gold10_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, 
				LC_TEXT("ґл№Ъ°сµе µе·У·ь %d%% ГЯ°Ў АМєҐЖ® БЯАФґПґЩ."), gold10_drop_bonus);
	}
	if (exp_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, 
				LC_TEXT("°жЗиДЎ %d%% ГЯ°Ў И№µж АМєҐЖ® БЯАФґПґЩ."), exp_bonus);
	}
}
...
static bool FN_is_battle_zone(LPCHARACTER ch)
{
	switch (ch->GetMapIndex())
	{
		...
		case 21:        // ГµБ¶ 1Вч ё¶А»
		case 23:        // ГµБ¶ 2Вч ё¶А»
		case 41:        // Бшіл 1Вч ё¶А»
		case 43:        // Бшіл 2Вч ё¶А»
		case 113:       // OX ёК
			return false;
	}
...
void CInputLogin::Login(LPDESC d, const char * data)
{
	...
	if (g_iUseLocale && !test_server)
	{
		...
		enhance_strlcpymt(failurePacket.szStatus, "VERSION", sizeof(failurePacket.szStatus));
	...
	{
		failurePacket.header = HEADER_GC_LOGIN_FAILURE;
		enhance_strlcpymt(failurePacket.szStatus, "SHUTDOWN", sizeof(failurePacket.szStatus));
		d->Packet(&failurePacket, sizeof(TPacketGCLoginFailure));
		return;
	}
	...
	if (g_iUserLimit > 0)
	{
		...
		{
			failurePacket.header = HEADER_GC_LOGIN_FAILURE;
			enhance_strlcpymt(failurePacket.szStatus, "FULL", sizeof(failurePacket.szStatus));
			d->Packet(&failurePacket, sizeof(TPacketGCLoginFailure));
			return;
		}
	}
	...
	enhance_strlcpymt(login_packet.login, login, sizeof(login_packet.login));
	enhance_strlcpymt(login_packet.passwd, pinfo->passwd, sizeof(login_packet.passwd));
...
void CInputLogin::LoginByKey(LPDESC d, const char * data)
{
	...
	enhance_strlcpymt(ptod.szLogin, login, sizeof(ptod.szLogin));
	ptod.dwLoginKey = pinfo->dwLoginKey;
	thecore_memcpy(ptod.adwClientKey, pinfo->adwClientKey, sizeof(DWORD) * 4);
	enhance_strlcpymt(ptod.szIP, d->GetHostName(), sizeof(ptod.szIP));
...
void CInputLogin::ChangeName(LPDESC d, const char * data)
{
	...
	enhance_strlcpymt(pdb.name, p->name, sizeof(pdb.name));
...
bool NewPlayerTable2(TPlayerTable * table, const char * name, BYTE race, BYTE shape, BYTE bEmpire)
{
	...
	table->level		= 1;
	table->job			= race;	// БчѕчґлЅЕ БѕБ·А» іЦґВґЩ
	table->voice		= 0;
	table->part_base	= shape;
...
void CInputLogin::CharacterCreate(LPDESC d, const char * data)
{
	...
	// »зїлЗТ јц ѕшґВ АМё§АМ°ЕіЄ, АЯёшµИ Жт»ує№АМёй »эјі ЅЗЖР
	...
	enhance_strlcpymt(player_create_packet.passwd, c_rAccountTable.passwd, sizeof(player_create_packet.passwd));
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	enhance_strlcpymt(player_delete_packet.private_code, pinfo->private_code, sizeof(player_delete_packet.private_code));
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if(ch->GetItemAward_cmd())																		//°ФАУЖдАМБо µйѕо°Ўёй
	...
	else if (ch->GetMapIndex() == 113)
	{
		...
		{
			// ox ёК БшАФ Зг°Ў°Ў іЄБц ѕКАЅ. ЗГ·№АМѕоёй ё¶А»·О єёі»АЪ
			if (ch->GetGMLevel() == GM_PLAYER)
				ch->WarpSet(EMPIRE_START_X(ch->GetEmpire()), EMPIRE_START_Y(ch->GetEmpire()));
		}
	}
	...
	// Г»јТів єёИЈ
	if (g_TeenDesc) // BufferedPacket »зїл ±ЭБц
	{
		TPacketGTLogin p;
	...
	// БЯёіёКїЎ µйѕо°¬А»¶§ ѕИі»ЗП±в
	if (g_noticeBattleZone)
	{
		if (FN_is_battle_zone(ch))
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("АМ ёКїЎј± °­Б¦АыАО ґлАьАМ АЦА»јц µµ АЦЅАґПґЩ."));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("АМ Б¶ЗЧїЎ µїАЗЗПБц ѕКА»ЅГ"));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("є»АОАЗ БЦјє №Ч єОјєАё·О µ№ѕЖ°ЎЅГ±в №Щ¶шґПґЩ."));
		}
	}
}
...
int CInputLogin::GuildSymbolUpload(LPDESC d, const char* c_pData, size_t uiBytes)
{
	...
	{
		// 64k єёґЩ Е« ±жµе ЅЙєјАє їГё±јцѕшґЩ
		// БўјУА» Іч°н №«ЅГ
		d->SetPhase(PHASE_CLOSE);
		return 0;
	}

	// ¶ҐА» јТАЇЗПБц ѕКАє ±жµеАО °жїм.
	if (!test_server)
...
void CInputLogin::GuildMarkCRCList(LPDESC d, const char* c_pData)
{
	...
	DWORD blockCount = 0;
	TEMP_BUFFER buf(1024 * 1024); // 1M №цЖЫ
