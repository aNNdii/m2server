...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
	...
	for (i = 0; i < size; ++i)
	{
		...
		// NOTE : QUEST_GIVE ЗГ·Ў±ЧґВ npc АМєҐЖ®·О №Я»э.
...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	if (LC_IsKorea() || LC_IsYMIR())
	{
		...
		{
			// јц·Гј­ё¦ ѕЖАМЕЫ №шИЈё¦ ґЩёЈ°Ф ёёµйѕо БШґЩ.
			DWORD dwSkillVnum;
	...
	//ГК±вИ­ ЗП°н. ЕЧАМєн јВЗП°н
	item->Initialize();
	item->SetProto(table);
	item->SetMaskVnum(dwMaskVnum);

	if (item->GetType() == ITEM_ELK) // µ·Ає ID°Ў ЗКїдѕш°н АъАеµµ ЗКїдѕшґЩ.
	...
	else
	{
		...
		if (item->GetType() == ITEM_UNIQUE) // АЇґПЕ© ѕЖАМЕЫАє »эјєЅГїЎ јТДПїЎ іІАєЅГ°ЈА» ±в·ПЗСґЩ.
		{
			if (item->GetValue(2) == 0)
				item->SetSocket(ITEM_SOCKET_UNIQUE_REMAIN_TIME, item->GetValue(0)); // °ФАУ ЅГ°Ј АЇґПЕ©
			else
			{
				//int globalTime = get_global_time();
				//int lastTime = item->GetValue(0);
				//int endTime = get_global_time() + item->GetValue(0);
				item->SetSocket(ITEM_SOCKET_UNIQUE_REMAIN_TIME, get_global_time() + item->GetValue(0)); // ЅЗЅГ°Ј АЇґПЕ©
			}
		}
	}
	...
	if (item->GetType() == ITEM_ELK) // µ·Ає ѕЖ№« Гіё®°Ў ЗКїдЗПБц ѕКАЅ
		;
	else if (item->IsStackable())  // ЗХДҐ јц АЦґВ ѕЖАМЕЫАЗ °жїм
	{
	...
	{
		// ѕЖАМЕЫ »эјє ЅГБЎєОЕН »зїлЗПБц ѕКѕЖµµ ЅГ°ЈАМ Вч°ЁµЗґВ №жЅД
		...
		// ±вБё АЇґПЕ© ѕЖАМЕЫГі·і ВшїлЅГїЎёё »зїл°ЎґЙ ЅГ°ЈАМ Вч°ЁµЗґВ №жЅД
		else if (LIMIT_TIMER_BASED_ON_WEAR == item->GetLimitType(i))
		{
			// АМ№М ВшїлБЯАО ѕЖАМЕЫАМёй ЕёАМёУё¦ ЅГАЫЗП°н, »х·О ёёµеґВ ѕЖАМЕЫАє »зїл °ЎґЙ ЅГ°ЈА» јјЖГЗШБШґЩ. (
			// ѕЖАМЕЫёф·О Бц±ЮЗПґВ °жїмїЎґВ АМ ·ОБчїЎ µйѕоїА±в АьїЎ Socket0 °ЄАМ јјЖГАМ µЗѕо АЦѕоѕЯ ЗСґЩ.
			...
			else if(0 == id)
			{
				...
				if (0 == duration)
					duration = 60 * 60 * 10;	// Б¤єё°Ў ѕЖ№«°Нµµ ѕшАёёй µрЖъЖ®·О 10ЅГ°Ј јјЖГ
	...
	if (id == 0) // »х·О ёёµеґВ ѕЖАМЕЫАП ¶§ёё Гіё®
	{
		// »х·ОГЯ°ЎµЗґВ ѕаГКµйАП°жїм јєґЙА» ґЩёЈ°ФГіё®
...
void ITEM_MANAGER::Update()
{
	...
	while (it != m_set_pkItemForDelayedSave.end())
	{
		...
		// SLOW_QUERY ЗГ·Ў±Ч°Ў АЦґВ °НАє Бѕ·бЅГїЎёё АъАеЗСґЩ.
...
void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason)
{
	...
	if ((o = item->GetOwner()))
	{
		...
		{
			// 20050613.ipkn.ЅГ°ЈБ¦ ѕЖАМЕЫАМ »уБЎїЎ АЦА» °жїм ЅГ°Јёё·бЅГ ј­№ц°Ў ґЩїоµИґЩ.
...
// 20050503.ipkn.
// iMinimum єёґЩ АЫАёёй iDefault јјЖГ (ґЬ, iMinimumАє 0єёґЩ ДїѕЯЗФ)
// 1, 0 ЅДАё·О ON/OFF µЗґВ №жЅДА» БцїшЗП±в А§ЗШ БёАз
int GetDropPerKillPct(int iMinimum, int iDefault, int iDeltaPercent, const char * c_pszFlag)
{
	...
	// ±вє» јјЖГАП¶§ (iDeltaPercent=100) 
	// 40000 iVal ё¶ё®ґз ЗПіЄ ґАі¦А» БЦ±в А§ЗС »ујцАУ
...
bool ITEM_MANAGER::CreateDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM>& vec_item)
{
	...
	// MobDropItem Group
	{
		...
		if ( it != m_map_pkMobItemGroup.end() )
		{
			...
			// 20050805.myevan.MobDropItem їЎ ѕЖАМЕЫАМ ѕшА» °жїм CMobItemGroup::GetOne() Бў±ЩЅГ №®Б¦ №Я»э јцБ¤
	...
	//
	// ЅВ·ж°о ГµАЗµї±ј 2ГюїЎј­ёё јц·ж№ж АФАе±З
	//
	...
	//
	// ЅВ·ж°о 1Гю, 2ГюїЎј­ёё 7,8 ЅєЕіАФ№®ј­ µе·У
	//
	if (LC_IsYMIR() || LC_IsKorea())
	{
		switch (pkKiller->GetMapIndex())
		{
			case 72:	// ГµАЗµї±ј 1Гю
			case 73:	// ГµАЗµї±ј 2Гю
				{
					int vnum = 0;

					if (2403 == pkChr->GetRaceNum())	// ГµАЗ№э»з
						vnum = 69200;	// 7ЅєЕі АФ№®ј­
					else if (2411 == pkChr->GetRaceNum())	// БшГµАЗєґ»з
						vnum = 69201;	// 8ЅєЕі АФ№®ј­
					else
						break;
...
// fixme
// А§АЗ °Н°ъ ЗФІІ quest·О »¬°Н »©єёАЪ. 
// АМ°Е іК№« ґх·ґАЭѕЖ...
// ”?. ЗПµеДЪµщ ЅИґЩ ¤М¤Р
// °и·® ѕЖАМЕЫ єё»у ЅГАЫ.
// by rtsummit °нДЎАЪ БшВҐ
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	// №Я·»ЕёАО µҐАМ АМєҐЖ®. OGEАЗ їд±ёїЎ µы¶у event ГЦјТ°ЄА» 1·О єЇ°ж.(ґЩёҐ АМєҐЖ®ґВ АПґЬ ±Чґл·О µТ.)
	...
	// new Е©ё®Ѕєё¶Ѕє АМєҐЖ®
	// 53002 : ѕЖ±в јш·П јТИЇ±З
...
// pkNewItemАё·О ёрµз јУјє°ъ јТДП °ЄµйА» ёс»зЗПґВ ЗФјц.
// ±вБёїЎ char_item.cpp ЖДАПїЎ АЦґш ·ОДГЗФјцАО TransformRefineItem ±Чґл·О є№»зЗФ
void ITEM_MANAGER::CopyAllAttrTo(LPITEM pkOldItem, LPITEM pkNewItem)
{
	...
	{
		// ї©±вј­ ±ъБшј®АМ АЪµїАыАё·О Г»јТ µК
		for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
		{
		...
		for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
		{
			...
			const int ITEM_BROKEN_METIN_VNUM = 28960; // АМ°З №№ ¶И°°Ає »ујц°Ў 3±єµҐіЄ АЦіД... ЗПіЄ·О ЗШіхБц¤Р¤Р¤Р іЄґВ ЖРЅє И«АМ ЗТІЁАУ
