...
void CItem::EncodeInsertPacket(LPENTITY ent)
{
	...
	if (m_pkOwnershipEvent != NULL)
	{
		...
		p.dwVID = m_dwVID;
		enhance_strlcpymt(p.szName, info->szOwnerName, sizeof(p.szName));
		...
		case ITEM_UNIQUE:
			{
				if (0 != GetSIGVnum())
				{
					...
					if (NULL == pAttrGroup)
						break;
					for (SpecialAttrInfoVector::const_iterator it = pAttrGroup->m_vecAttrs.begin(); it != pAttrGroup->m_vecAttrs.end(); it++)
					{
...
void CItem::SetOwnership(LPCHARACTER ch, int iSec)
{
	...
	item_event_info* info = AllocEventInfo<item_event_info>();
	enhance_strlcpymt(info->szOwnerName, ch->GetName(), sizeof(info->szOwnerName));
	info->item = this;
