...
class CItem : public CEntity
{
	...
	public:
		...
		// GetVnum°ъ GetOriginalVnumїЎ ґлЗС comment
		// GetVnumАє Masking µИ VnumАМґЩ. АМё¦ »зїлЗФАё·ОЅб, ѕЖАМЕЫАЗ ЅЗБ¦ VnumАє 10АМБцёё, VnumАМ 20АО °НГі·і µїАЫЗТ јц АЦґВ °НАМґЩ.
		// Masking °ЄАє ori_to_new.txtїЎј­ Б¤АЗµИ °ЄАМґЩ.
		// GetOriginalVnumАє ѕЖАМЕЫ °нАЇАЗ VnumАё·О, ·О±Ч іІ±ж ¶§, Е¬¶уАМѕрЖ®їЎ ѕЖАМЕЫ Б¤єё єёіѕ ¶§, АъАеЗТ ¶§ґВ АМ VnumА» »зїлЗПї©ѕЯ ЗСґЩ.
		// 
		...
		void		ModifyPoints(bool bAdd);	// ѕЖАМЕЫАЗ Иї°ъё¦ ДіёЇЕНїЎ єОї© ЗСґЩ. bAdd°Ў falseАМёй Б¦°ЕЗФ
		...
		int		GetAttributeSetIndex(); // јУјє єЩґВ°НА» БцБ¤ЗС №иї­АЗ ѕоґА АОµ¦Ѕєё¦ »зїлЗПґВБц µ№·ББШґЩ.
		...
		//			АПґЬ REAL_TIME°ъ TIMER_BASED_ON_WEAR ѕЖАМЕЫїЎ ґлЗШј­ёё Б¦ґл·О µїАЫЗФ.
		int			GetDuration();
		...
		// ѕЗјј»зё® ё¦ ѕЖАМЕЫїЎ №ЫѕТА»¶§ ЕёАМёУ µ№ѕЖ°ЎґВ°Н( ±ёё®, µо )
	...
	protected:
		...
		bool		OnAfterCreatedItem();			// ј­№ц»уїЎ ѕЖАМЕЫАМ ёрµз Б¤єёїН ЗФІІ їПАьИч »эјє(·Оµе)µИ ИД єТё®їмґВ ЗФјц.
	...
	public:
		...
		// їАё®БціО ѕЖАМЕЫїЎ, ±іИЇ ±ЭБц ЗГ·Ў±Чёё »иБ¦ЗС »х·Оїо ѕЖАМЕЫµйА» »х·Оїо ѕЖАМЕЫ ґлїЄїЎ ЗТґзЗПїґґЩ.
		...
		// Mask vnumАє ѕо¶І АМАЇ(ex. А§АЗ »уИІ)·О АОЗШ vnumАМ №ЩІоѕо µ№ѕЖ°ЎґВ ѕЖАМЕЫА» А§ЗШ АЦґЩ.
	...
	private:
		TItemTable const * m_pProto;		// ЗБ·ОЕд ЕёАЩ

		DWORD		m_dwVnum;
		LPCHARACTER	m_pOwner;

		BYTE		m_bWindow;		// ЗцАз ѕЖАМЕЫАМ А§ДЎЗС А©µµїм 
		DWORD		m_dwID;			// °нАЇ№шИЈ
		bool		m_bEquipped;	// АеВш µЗѕъґВ°Ў?
		DWORD		m_dwVID;		// VID
		WORD		m_wCell;		// А§ДЎ
		DWORD		m_dwCount;		// °іјц
		long		m_lFlag;		// ГЯ°Ў flag
		DWORD		m_dwLastOwnerPID;	// ё¶Бцё· °ЎБц°н АЦѕъґш »з¶чАЗ PID

		bool		m_bExchanging;	///< ЗцАз ±іИЇБЯ »уЕВ 

		long		m_alSockets[ITEM_SOCKET_MAX_NUM];	// ѕЖАМЕЫ јТД№
