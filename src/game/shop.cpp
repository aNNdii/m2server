...
void CShop::SetShopItems(TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (int i = 0; i < bItemCount; ++i)
	{
		...
		if (item.pkItem)
		{
			...
			item.count = pkItem->GetCount(); // PC јҐАЗ °жїм ѕЖАМЕЫ °іјцґВ БшВҐ ѕЖАМЕЫАЗ °іјцї©ѕЯ ЗСґЩ.
			item.price = pTable->price; // °Ў°Эµµ »зїлАЪ°Ў Б¤ЗСґл·О..
...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (m_pkPC) // ЗЗѕѕ°Ў їоїµЗПґВ јҐАє ЗЗѕѕ°Ў ЅЗБ¦ ѕЖАМЕЫА» °ЎБц°нАЦѕоѕЯ ЗСґЩ.
		item = r_item.pkItem;
	else
	...
	if (m_pkPC)
	{
		...
		if (iVal > 0)
			m_pkPC->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ЖЗёЕ±ЭѕЧАЗ %d %% °Ў јј±ЭАё·О іЄ°Ў°ФµЛґПґЩ"), iVal);
