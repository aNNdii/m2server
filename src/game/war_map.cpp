...
bool CWarMapManager::GetStartPosition(long lMapIndex, BYTE bIdx, PIXEL_POSITION & pos)
{
	...
	if (!pi)
	{
		...
		for (WarMapInfoMap::const_iterator it = m_map_kWarMapInfo.begin(); it != m_map_kWarMapInfo.end(); ++it)
		{
...
TWarMapInfo * CWarMapManager::GetWarMapInfo(long lMapIndex)
{
	...
	WarMapInfoMap::const_iterator it = m_map_kWarMapInfo.find(lMapIndex);
...
{
	WarMapMap::const_iterator it = m_mapWarMap.find(lMapIndex);
...
{
	WarMapMap::const_iterator it = m_mapWarMap.begin();
