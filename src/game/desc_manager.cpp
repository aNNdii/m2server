...
{
	Initialize();
	//NOTE : Destroy іЎїЎј­ Initialize ё¦ єОёЈґВ°З ¶З №«Ѕј БюАМіД..-_-; Б¤ё» 
...
LPDESC DESC_MANAGER::AcceptDesc(LPFDWATCH fdw, socket_t s)
{
	...
	enhance_strlcpymt(host, inet_ntoa(peer.sin_addr), sizeof(host));
	...
	if (!IsValidIP(admin_ip, host)) // admin_ip їЎ µо·ПµИ IP ґВ ГЦґл »зїлАЪ јцїЎ ±ёѕЦ№ЮБц ѕКґВґЩ.
	{
