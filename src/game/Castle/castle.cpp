...
#define EMPIRE_NONE		0	// ѕЖ№«±№°Ў ѕЖґФ
#define EMPIRE_RED		1	// ЅЕјц
#define EMPIRE_YELLOW	2	// ГµБ¶
#define EMPIRE_BLUE		3	// Бшіл


#define SIEGE_EVENT_PULSE	PASSES_PER_SEC(60*5)	// 5єР
...
EVENTFUNC(castle_siege_event)
{
	...
	// °шјє ЅГАЫИД 30єР АМі»¶уёй ѕИі»ёё ЗПАЪ
	if (info->pulse < PASSES_PER_SEC(30*60))
	{
		snprintf(buf, sizeof(buf), LC_TEXT("%sїЎј­ єАИ­ё¦ µС·ЇЅО°н АьЕх°Ў БшЗаБЯАФґПґЩ."),
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_STRUGGLE:
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%sАМ јцјєїЎ јє°шЗЯЅАґПґЩ."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);

				snprintf(buf, sizeof(buf), LC_TEXT("Бц±ЭєОЕН %sАє 30єР°Ј єАИ­ё¦ ЖД±«ЗПї© єё»уА» И№µж ЗТ јц АЦЅАґПґЩ."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);

				GET_SIEGE_STATE() = CASTLE_SIEGE_END;

				return PASSES_PER_SEC(60*30);	// 30єР
			}
			break;
		case CASTLE_SIEGE_END:
			BroadcastNotice(LC_TEXT("30єРАМ °ж°ъЗЯЅАґПґЩ.. єАИ­°Ў »з¶уБэґПґЩ."));
			castle_end_siege();
			break;
	}
	return 0;
...
EVENTFUNC(castle_stone_event)
{
	...
	if (info->spawn_count < (SPAWN_COUNT * 2))
	{
		...
		if (info->spawn_count < (SPAWN_COUNT * 2))
			return PASSES_PER_SEC(30 * 60);	// 30єР
		else
			return 0;
	}
...
void castle_start_siege(int empire, int tower_count)
{
	...
	/* °шјє ЕёАМёУ ЅГАЫ */
	{
		...
		GET_SIEGE_EVENT(empire) = event_create(castle_siege_event, info, /*5єР*/SIEGE_EVENT_PULSE);
	}

	/* ёЮЖѕј® јТИЇ ЕёАМёУ ЅГАЫ */
	{
		...
		GET_STONE_EVENT(empire) = event_create(castle_stone_event, info, /* 1ГК */PASSES_PER_SEC(1));
	}
}
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	int	spawn_count = MINMAX(MIN_CASTLE_TOWER, tower_count, MAX_CASTLE_TOWER);	// 5 ~ 10ё¶ё®
	...
	{
		char buf[1024];
		snprintf(buf, sizeof(buf), LC_TEXT("%sїЎ АьАпАЗ ЅГАЫА» ѕЛё®ґВ єАИ­°Ў іЄЕёіµЅАґПґЩ."), EMPIRE_NAME(empire));
		BroadcastNotice(buf);
	}
	return true;
}

/* °жєсєґё®ґх°Ў БЧАёёй ґЬјшЗП°Ф ЅЅ·Фёё єсїоґЩ. */
...
/* ИІ±Э µОІЁєс°Ў БЧАёёй killerїЎ°Ф 1ГµёёіЙ */
void castle_frog_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	DO_ALL_FROG(i)
	{
		if (ch == GET_FROG(empire, i))
		{
			...
			killer->PointChange(POINT_GOLD, 10000000 /*1Гµёё*/, true);
			//CMonarch::instance().SendtoDBAddMoney(30000000/*3Гµёё*/, killer->GetEmpire(), killer);
			castle_save();
			return;
		}
	}
...
/* єАИ­°Ў ёрµО БЧАёёй(?) °шјєАьАМ іЎі­ґЩ */
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_END:
			{
				int	siege_end = true;
				snprintf(buf, sizeof(buf), LC_TEXT("%sАМ єАИ­ё¦ ЖД±«ЗЯЅАґПґЩ."), EMPIRE_NAME(killer_empire));
				BroadcastNotice(buf);
				...
				if (siege_end)
				{
					...
					{
						snprintf(buf, sizeof(buf), LC_TEXT("%sАМ јцјєїЎ ЅЗЖРЗПї© АьАпїЎ ЖР№иЗПїґЅАґПґЩ.."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					else
					{
						snprintf(buf, sizeof(buf), LC_TEXT("%sАМ ёрµз єАИ­ё¦ ЖД±«ЗПїґЅАґПґЩ."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					castle_end_siege();
				}
...
int castle_cost_of_hiring_guard(DWORD group_vnum)
{
	...
	{
		/* ЗП±Ю */
		case 9501:	/* ЅЕјц Гў°жєс */
		case 9511:	/* Бшіл Гў°жєс */
		case 9521:	/* ГµБ¶ Гў°жєс */

		case 9502:	/* ЅЕјц И°°жєс */
		case 9512:	/* Бшіл И°°жєс */
		case 9522:	/* ГµБ¶ И°°жєс */
			return (100*10000);

		/* БЯ±Ю */
		case 9503:	/* ЅЕјц Гў°жєс */
		case 9513:	/* Бшіл Гў°жєс */
		case 9523:	/* ГµБ¶ Гў°жєс */

		case 9504:	/* ЅЕјц И°°жєс */
		case 9514:	/* Бшіл И°°жєс */
		case 9524:	/* ГµБ¶ И°°жєс */
			return (300*10000);

		/* »у±Ю */
		case 9505:	/* ЅЕјц Гў°жєс */
		case 9515:	/* Бшіл Гў°жєс */
		case 9525:	/* ГµБ¶ Гў°жєс */

		case 9506:	/* ЅЕјц И°°жєс */
		case 9516:	/* Бшіл И°°жєс */
		case 9526:	/* ГµБ¶ И°°жєс */
			return (1000*10000);
	}
...
bool castle_can_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	{
		// јцјєїЎ јє°шЗЯА»¶§ °°Ає Б¦±№ёё єАИ­ё¦ ДҐ јц АЦАЅ
...
bool castle_frog_to_empire_money(LPCHARACTER ch)
{
	...
	DO_ALL_FROG(i)
	{
		...
		GET_FROG(empire, i) = NULL; // µо·ПЗШБ¦
